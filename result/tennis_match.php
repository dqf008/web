...
$last_url = "/result/tennis_match.php?list_date=".date('Y-m-d',strtotime($date)-24*60*60)."&langx=zh-cn";
if( strtotime(date('Y-m-d', time())) > strtotime($date) ){
	$next_url = "/result/tennis_match.php?list_date=".date('Y-m-d',strtotime($date)+24*60*60)."&langx=zh-cn";
}
?> 
...
<div id="div_state" class="acc_leftMain">
    ...
    <div class="acc_state_head">
       ...
       <div  style="height:400px;">
		...
		$sql = 'select Match_Date,Match_Time, match_name,match_master,Match_MatchTime,match_guest,MB_Inball,TG_Inball from mydata4_db.tennis_match where MB_Inball is not null and  match_Date=:match_Date and match_js=1';//
