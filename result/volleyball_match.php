...
$last_url = "/result/volleyball_match.php?list_date=".date('Y-m-d',strtotime($date)-24*60*60)."&langx=zh-cn";
if( strtotime(date('Y-m-d', time())) > strtotime($date) ){
	$next_url = "/result/volleyball_match.php?list_date=".date('Y-m-d',strtotime($date)+24*60*60)."&langx=zh-cn";
}
?> 
       ...
       <div>
          <table cellpadding="0" cellspacing="0" border="0" id="results_tableLine" class="acc_results_table">
             <tr class="acc_results_tr_title">
                ...
                <td class="acc_results_teamw" style="width:75%"></td>
                <td class="acc_results_otherw" style="width:10px;">完赛</td>
             </tr>
		<?php 
		$params[':match_Date'] = date('m-d', strtotime($date));
		$sql = 'select Match_Date,Match_Time, match_name,match_master,match_guest,MB_Inball,TG_Inball,Match_MatchTime from mydata4_db.volleyball_match where MB_Inball is not null and  match_Date=:match_Date and match_js=1';//
		$stmt = $mydata1_db->prepare($sql);
		$stmt->execute($params);
		$rows = $stmt->fetch();

		if(!$rows){
			echo "<tr><td height='100' colspan='3' align='center' bgcolor='#FFFFFF'>暂无任何赛果</td></tr>";
		}else{
			do{
		...
		<tr class="acc_results_league">
		    <td colspan="3" ><span><?=$rows["match_name"]?></span></td>
		</tr>
		<?php
				}
		?>
			...
			<tr class="acc_result_tr_top" >
			    ...
			    <td class="acc_result_bg">
			    	<span class="BlackWord">
			    	<?=$rows["MB_Inball"]<0?"赛事无效":$rows["MB_Inball"]?>
			    	</span>
			    </td>
			 ...
			 <tr class="acc_result_tr_other">
			    ...
			    <td class="acc_result_bg"><span class="BlackWord"><?=$rows["TG_Inball"]<0?"赛事无效":$rows["TG_Inball"]?></span></td>
			 </tr>
		<?php
