angular.module('ionicz.bank')
    ...
    .controller('BankBaseCtrl', ['$rootScope', '$scope', '$log', 'Tools', 'My', '$ionicHistory', '$location', 'PATH', function($rootScope, $scope, $log, Tools, My, $ionicHistory, $location, PATH) {
        !$rootScope.appData.bank && ($rootScope.appData.bank = {});
        $log.debug("BankBaseCtrl...");
        $scope.back = Tools.back('/ucenter/index');
    }])

    .controller('BankController', ['$scope', '$log', '$location', 'Tools', 'My', function($scope, $log, $location, Tools, My) {
    ...
    .controller('MoneyController', ['$rootScope', '$scope', '$log', '$state', '$filter', '$location', 'My', 'Tools', 'md5', 'ionicDatePicker', function($rootScope, $scope, $log, $state, $filter, $location, My, Tools, md5, ionicDatePicker) {
                ...
                success: function(data) {
                    if(data&&data.status==1){
                        ...
                        Tools.ajax({
                            ...
                            url: '../app/ajax.php',
                            params: {
                                s: '/member/finance/deposit/list'
                            },
                            backdrop: true,
                            success: function(data) {
                                if(data&&data.errorCode==0){
                                    Object.keys(data.data).map(function (k) {
                                        if (k !== 'limit') {
                                            $scope.bankData[k] = data.data[k]
                                        }
                                    });
                                }
                            }
    ...
    .controller('TransController', ['$rootScope', '$scope', '$location', '$timeout', 'Tools', function($rootScope, $scope, $location, $timeout, Tools) {
    ...
    .controller('TransferController', ['$rootScope', '$scope', '$location', '$filter', 'Tools', 'ionicDatePicker', function($rootScope, $scope, $location, $filter, Tools, ionicDatePicker) {
        $scope.$on('$ionicView.afterEnter', function() {
        ...
        $scope.checkStep = function() {
            switch(!0){
                ...
                case parseFloat($scope.transferData.money)<=0:
                    Tools.tip('请输入有效转账金额');
                    break;
                case false && parseFloat($scope.transferData.money)<parseFloat($scope.min):
        ...
        $scope.transferSubmit = function() {
            Tools.ajax({
                method: 'POST',
                url: '../app/ajax.php?s=/member/finance/deposit/transfer',
                params: {
    ...
    .controller('DepositController', ['$rootScope', '$scope', '$location', '$filter', 'Tools', function($rootScope, $scope, $location, $filter, Tools) {
        $scope.$on('$ionicView.afterEnter', function() {
        ...
        $scope.min = 0;
        $scope.depositData = {};
        $scope.depositSubmit = function(event){
            var submit = false;
            switch(!0){
                ...
                default:
                    ...
                    //     success: function(data) {
                    //         if(data&&data.status==1){
                    //             Tools.alert('订单已创建，请继续', function(){
    ...
    .controller('QRCodeController', ['$rootScope', '$scope', '$location', '$filter', 'Tools', 'ionicDatePicker', function($rootScope, $scope, $location, $filter, Tools, ionicDatePicker) {
        $scope.$on('$ionicView.afterEnter', function() {
        ...
        $scope.checkStep = function() {
            switch(!0){
                ...
                case parseFloat($scope.transferData.money)<parseFloat($scope.min):
                    Tools.tip('转账金额不能小于 '+$filter('money')($scope.min)+' 元');
                    break;
                case false && parseFloat($scope.transferData.money)>parseFloat($scope.max):
        ...
        $scope.transferSubmit = function() {
            Tools.ajax({
                method: 'POST',
                url: '../app/ajax.php?s=/member/finance/deposit/qrcode',
                params: {
                    qrcode: !0,
                    code: $scope.transferData.info.code,
                    amount: parseFloat($scope.transferData.money),
    ...
    .controller('TwoInOneController', ['$rootScope', '$scope', '$location', '$filter', '$state', 'Tools', 'ionicDatePicker', function($rootScope, $scope, $location, $filter, $state, Tools, ionicDatePicker) {
        var type = $state.current.data.type;
        $scope.title = $state.current.data.title;
        $scope.$on('$ionicView.afterEnter', function() {
            if(!$rootScope.appData.bank.config||!$rootScope.appData.bank.config[type]){
                $location.path('/bank/money').search({type: "deposit"});
            }else{
                $scope.appData = $rootScope.appData.bank.config[type];
                $scope.min = $rootScope.appData.bank.config.min;
            }
        });
        $scope.min = 0;
        ...
        $scope.depositSubmit = function(event){
            ...
            switch(!0){
                ...
                default:
                    ...
                    //         action: 'bank',
                    //         type: 'online',
                    //         code: $scope.transferData.type.code,
                    //         amount: parseFloat($scope.transferData.money)
                    //     },
                    //     backdrop: true,
                    //     success: function(data) {
