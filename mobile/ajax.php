                                    ...
                                    case 'SHK3':
                                        ...
                                        if ($sum['count'] > 0) {
                                            ...
                                            while ($rows = $stmt->fetch()) {
                                                ...
                                                $rows['win'] = floatval($rows['win']);
                                                $rows['odds'] = floatval($rows['odds']);
                                                $detailContent =isset($contentText[$rows['content']])?$contentText[$rows['content']]:$rows['content'];
                                                $MOBILE['output']['msg'][] = [
                                                    'id' => $rows['id'],
                                                    'expect' => $rows['mid'],
                                                    'date' => $rows['bet_time'][0],
                                                    ...
                                                    'betMoney' => $rows['money'],
                                                    'winLoseMoney' => $rows['win'],
                                                    'betContent' => [$rows['btype'],$detailContent],
                                                    'odds' => $rows['odds'],
                                                    'status' => $rows['bet_ok'],
                                                ];
                                                $MOBILE['output']['lastId'] = $rows['id'];
                ...
                $MOBILE['output'] = [
                    ...
                    'msg' => [
                        ...
                        /*'qhk3'  => [
                            ...
                            // 'reset' => ['bigness', 'weitou', 'points', 'longcard', 'shortcard'],
                            'class'        => 'round',
                            'hasChangLong' => 0,
                            'hasLuZhu'     => 0,
                        ],
                ...
                switch ($_POST['gameType']) {
                    ...
                    case 'ffpcdd':
                        ...
                        if ($_POST['lastId'] > 0) {
                            $params[':id'] = $_POST['lastId'];
                            $sql = [
                                'SELECT COUNT(*) AS `count` FROM `lottery_k_pcdd` WHERE `ok`=1 AND `id`<:id  AND `fengpan` BETWEEN :qtime AND :etime',
                            ...
                            $sql = [
                                'SELECT COUNT(*) AS `count` FROM `lottery_k_pcdd` WHERE `ok`=1  AND `fengpan` BETWEEN :qtime AND :etime',
                                'SELECT * FROM `lottery_k_pcdd` WHERE `ok`=1  AND `fengpan` BETWEEN :qtime AND :etime ORDER BY `qihao` DESC LIMIT :limit',
                            ];
                        }
                        $stmt = $mydata1_db->prepare($sql[0]);
                        $stmt->execute($params);
                    ...
                    case 'jxk3':
                        ...
                        if ($rows['count'] > 0) {
                            ...
                            while ($rows = $stmt->fetch()) {
                                ...
                                $rows['openinfo'][] = $rows['openinfo'][0] > 10 ? '大' : '小';
                                $rows['openinfo'][] = fmod($rows['openinfo'][0], 2) == 0 ? '双' : '单';
                                $MOBILE['output']['msg'][] = [
                                    'issue' => $rows['qihao'],
                                    'opentime' => $rows['fengpan'],
