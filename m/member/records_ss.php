...
$p_type = 'ss';
$atype = $_GET['atype'];
if (!in_array($atype, [1, 2, 3, 4, 5, 6, 7])){
	$atype = '1';
}
  ...
  <body class="mWrap ui-page ui-page-theme-a ui-page-active" data-role="page" tabindex="0" style="min-height: 659px;"> 
	  ...
	  <section class="mContent clearfix" style="padding:0px;"> 
		  <div data-role="cotent"> 
			  <form action="records_ss.php" method="get" name="lt_form" data-role="none" data-ajax="false"> 
			  ...
			  if($atype>4){
					...
					$sql = 'SELECT COUNT(*) AS `count` FROM `c_bet_data` AS `c` LEFT JOIN `k_user` AS `u` ON `c`.`uid`=`u`.`uid` WHERE `u`.`uid`=:uid AND `c`.`type`=:type AND `c`.`addtime` BETWEEN :begin_time AND :end_time AND `c`.`status` BETWEEN 0 AND 1';
				...
				if ($sum>0){
					$sql	=	'SELECT `c`.* FROM `c_bet_data` AS `c` LEFT JOIN `k_user` AS `u` ON `c`.`uid`=`u`.`uid` WHERE `u`.`uid`=:uid AND `c`.`type`=:type AND `c`.`addtime` BETWEEN :begin_time AND :end_time AND `status` BETWEEN 0 AND 1 ORDER BY `c`.`id` desc LIMIT :index, :limit';
					...
					while ($rows = $query->fetch())
					{
						...
						if ($rows['status'] == 1){
							if ($rows['win'] == $rows['money'])
							{
								...
								$paicai = $rows['win'];
								$paycai_beizhu = '(<span style=\'color:#0000FF;\'>实际加款:' . sprintf('%.2f', $rows['win']) . '</span>)';
					  ...
					  <label> 
						  <div style="position:relative;padding-bottom:5px;">投注时间： 
							  <div style="position:absolute;top:0px;left:80px;"> 
								 <?=date('Y-m-d H:i:s',$rows['addtime']);?>(美东)<br> 
								 <?=date('Y-m-d H:i:s',$rows['addtime'] + 43200);?>(北京) 
							  </div> 
							  <div></div> 
						  </div> 
					  </label> 
				  ...
				  <li class="ui-li-static ui-body-inherit"> 
					  <label>下注单号：<?=$rows['id'];?></label> 
				  </li> 
				  <li class="ui-li-static ui-body-inherit"> 
					  <label>下注期号：第<?=$rows['value']['qishu'];?> 期</label> 
				  </li> 
				  <li class="ui-li-static ui-body-inherit"> 
					  <label> 
						  <div style="position:relative;padding-bottom:5px;">投注信息： 
							  <div style="position:absolute;top:0px;left:80px;"><?=$rows['value']['class'][0];?>【<font color="#FF0000"><?=$rows['value']['class'][1];?></font>】 
							  </div> 
							  <div></div> 
						  </div> 
					  </label> 
				  ...
				  <li class="ui-li-static ui-body-inherit"> 
					  <label>可赢：<?=sprintf('%.2f',$rows['money'] * $rows['value']['odds']);?></label> 
				  </li> 
				  <li class="ui-li-static ui-body-inherit"> 
	...
	<?php $i++; }?> 	
				  <li class="ui-li-static ui-body-inherit ui-last-child"> 
					  <div style="clear:both;text-align:left;"> 
						  ...
						  <?=$page->get_htmlPage("records_ss.php?cn_begin=".$cn_begin."&s_begin_h=".$s_begin_h."&s_begin_i=".$s_begin_i."&cn_end=".$cn_end."&s_end_h=".$s_end_h."&s_end_i=".$s_end_i."&atype=".$atype);?>					  
					  </div> 
				  </li>
<?php  }else{ ?> 				  
	<li class="ui-li-static ui-body-inherit ui-last-child"> 
...
<?php }
				...
				$params = array(':type' => $atypename, ':username' => $username, ':begin_time' => $begin_time, ':end_time' => $end_time);
				...
				if ($id){
				  ...
				  <li data-role="list-divider" role="heading" class="ui-li-divider ui-bar-inherit ui-first-child"> 
					  <label> 
						  <div style="position:relative;padding-bottom:5px;">投注时间： 
							  <div style="position:absolute;top:0px;left:80px;"> 
								 <?=date('Y-m-d H:i:s',strtotime($rows['addtime']));?>(美东)<br> 
								 <?=date('Y-m-d H:i:s',strtotime($rows['addtime']) + (12 * 3600));?>(北京) 
							  </div> 
							  <div></div> 
						  </div> 
					  </label> 
				  ...
				  <li class="ui-li-static ui-body-inherit"> 
					  <label>下注期号：第<?=$rows['qishu'];?> 期</label> 
				  </li> 
				  <li class="ui-li-static ui-body-inherit"> 
					  <label> 
						  <div style="position:relative;padding-bottom:5px;">投注信息： 
							  <div style="position:absolute;top:0px;left:80px;"><?=$rows['mingxi_1'];?>【<font color="#FF0000"><?=$rows['mingxi_2'];?></font>】 
							  </div> 
							  <div></div> 
						  </div> 
					  </label> 
