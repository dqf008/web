...
$stmt = $mydata1_db->prepare('select * from mydata2_db.ka_bl where  class1=:class1 and class2=:class2 order by id');
$stmt->execute($params);
$plArray = array();
$betnumber = 0;
...
<head> 
  ...
  <body class="mWrap ui-page ui-page-theme-a ui-page-active" data-role="page" tabindex="0" style="min-height: 659px;"> 
	  ...
	  <form name="form" id="form" action="_katan.php" method="POST"> 
		  ...
		  <div data-role="tabs" id="tabs" class="ui-tabs ui-widget ui-widget-content ui-corner-all"> 
			  <div id="three" class="ui-body-d ui-content" style="padding:0px;"> 
				  ...
				  <div class="xiazhu" style="clear:both;" data-role="none"> 
					  ...
					  <?php 
						...
						$money = $_POST['num_' . $btn];
						if (0 < $money){
					  ?> 							  
						<li style="border-top:1px solid #DDD;font-weight: bold;background-color:#EDEDED;width:30%;" 
						  data-role="none"> 
							  <p><?=$plArray[$btn]['class3'];?><input type="hidden" name="class3_<?=$btn;?>" value="<?=$plArray[$btn]['class3'];?>" /> 
							  </p> 
						  </li> 
						  <li style="border-top:1px solid #DDD;font-weight: bold;background-color:#EDEDED;width:20%;" 
						  data-role="none"> 
							  <p id="odd_<?=$btn;?>" style="color:#FF0000;"><?=$plArray[$btn]['rate'];?></p> 
						  </li> 
						  <li style="border-top:1px solid #DDD;font-weight: bold;background-color:#EDEDED;width:25%;text-align:center;" data-role="none">
						  <?=$money;?><input type="hidden" name="money_<?=$btn;?>" value="<?=$money;?>" /> 
						  </li> 
						  <li style="border-top:1px solid #DDD;font-weight: bold;background-color:#EDEDED;width:25%;" 
						  data-role="none"> 
							  <p id="win_<?=$btn;?>"><?=$money * $plArray[$btn]['rate'];?></p> 
						  </li>
						  <?php }
						}?> 						  
					</ul> 
			  ...
			  <div style="clear:both;padding-top:10px;padding-left:10px;"> 
				  <div style="clear:both;text-align:center;"> 
					  <a href="javascript:goBack();" class="ui-btn ui-btn-inline">取消下注</a> 
					  <a href="javascript:formSubmit();" class="ui-btn ui-btn-inline">确认下注</a> 
				  </div> 
			  </div> 
