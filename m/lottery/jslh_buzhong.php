  ...
  <script type="text/javascript">
    $(document).ready(function(){
        ...
        $("[data-id]").on("click", function(){
            var t = $(this).data("id");
            pankouflag && (!$(this).hasClass("caipiao-selected")&&$(".caipiao-selected[data-id]").size()>=max*2 ? alert("只能选择"+(limit>=max?max:limit+"-"+max)+"个号码") : $("[data-id="+t+"]").toggleClass("caipiao-selected"))
        }),
  ...
  <section class="mContent clearfix" style="padding: 0 1px 0 2px;">
      <form name="lt_form" id="lt_form" method="post"
          ...
          <div class="ui-grid-c">
              ...
              <div class="ui-block-d caipiao-bgc-abc">
                  <span>赔率</span>
              </div>
              <?php 
                foreach($odds[$list[$type]][1] as $id=>$val){
                    $ab = fmod($id, 2)==1?['a', 'b']:['c', 'd'];
                    $id = show_id($list[$type], $id);
              ?>                 
              <div class="ui-block-<?php echo $ab[0]; ?> caipiao-bgc-a" data-id="<?php echo $id; ?>">
                  <div><?php echo substr('00'.$val, -2); ?></div>
              </div>
              <div class="ui-block-<?php echo $ab[1]; ?> caipiao-bgc-b" data-id="<?php echo $id; ?>">
