...
if ($stype == 1){
	...
	if (($content < 0) && (9 < $content)){
		echo '<script type="text/javascript">alert("非法投注！");window.location.href="/m/lottery/3d.php";</script>';
		exit();
	}
	$ctype = $content;
}else if ($stype == 7){
...
if ($stype == 3){
	...
	$psql = 'select * from lottery_odds where class1=\'3d\' and class2=:class2 and class3=:class3';
	$stmt = $mydata1_db->prepare($psql);
	$stmt->execute($params);
	$prow = $stmt->fetch();
	$oddsshow[0] = $prow['odds'];
...
<html class="ui-mobile ui-mobile-viewport ui-overlay-a"> 
  ...
  <body class="mWrap ui-page ui-page-theme-a ui-page-active" data-role="page" tabindex="0" style="min-height: 659px;"> 
	  ...
	  <form name="form" id="form" action="3dOrder.php" method="POST"> 
