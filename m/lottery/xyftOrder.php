...
if(empty($_SESSION['subtime'])){
	...
	if($s<$sub){
		die('<script type="text/javascript">alert("'.$sub.'秒内禁止重复提交，返回下注页面！");window.location.href="/m/lottery/xyft.php";</script>');
	}else{
		$_SESSION['subtime'] = time();
	}
}
...
if (count($datas) <= 0){
	echo '<script type="text/javascript">alert("该页面为提交注单页面无法刷新，返回下注页面！");window.location.href="/m/lottery/xyft.php";</script>';
	exit();
}

if ($_REQUEST['type'] == 8){
	$qishu = lottery_qishu_8();
	if ($qishu == -1){
	...
	for ($i = 0;$i < count($datas);$i++){
		...
		if ($datas['' . $names[$i] . ''] == 0){
			echo '<script type="text/javascript">alert("投注金额非法！");window.location.href="/m/lottery/xyft.php";</script>';
			exit();
		}else if ($datas['' . $names[$i] . ''] < $cp_zd){
			echo '<script type="text/javascript">alert("最低下注金额：'.$cp_zd.'￥！");window.location.href="/m/lottery/xyft.php";</script>';
			exit();
		}else if ($cp_zg < $datas['' . $names[$i] . '']){
			echo '<script type="text/javascript">alert("最高下注金额：'.$cp_zg.'￥！");window.location.href="/m/lottery/xyft.php";</script>';
			exit();
		}
	...
	for ($i = 0;$i < count($datas);$i++){
		...
		$params = array(':did' => date('YmdHis', time()) . rand(1000, 9999), ':uid' => $uid, ':username' => $username, ':addtime' => date('Y-m-d H:i:s', time()), ':type' => '幸运飞艇', ':qishu' => $qishu, ':mingxi_1' => $qiuhao, ':mingxi_2' => $wanfa, ':odds' => $odds, ':money' => $money, ':win' => $money * $odds, ':bet_date' => date('Y-m-d', time()));
		...
		$stmt = $mydata1_db->prepare('INSERT INTO k_money_log(uid,userName,gameType,transferType,transferOrder,transferAmount,previousAmount,currentAmount,creationTime)SELECT uid,userName,\'XYFT\',\'BET\',:transferOrder,-:transferAmount,money+:tmpMoney,money+:tmpMoney2-:transferAmount2,:creationTime FROM k_user WHERE uid=:uid');
		$stmt->execute($params);
		$tmpMoney -= $money;
	  ...
	  <div data-role="tabs" id="tabs" class="ui-tabs ui-widget ui-widget-content ui-corner-all"> 
		  ...
		  <div style="clear:both;padding-top:10px;padding-left:10px;"> 
			  <div style="clear:both;text-align:center;"> 
				  <a href="javascript:window.location.href='xyft.php';" class="ui-btn ui-btn-inline"> 
					  继续下注 
				  </a> 
