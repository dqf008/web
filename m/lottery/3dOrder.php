...
$stmt = $mydata1_db->prepare('select * from lottery_k_3d where kaipan<:kaipan and fengpan>:fengpan');
$stmt->execute($params);
$trow = $stmt->fetch();
$tcou = $stmt->rowCount();
if ($tcou <= 0){
	echo '<script type="text/javascript">alert("已经封盘，禁止下注！");window.location.href="/m/lottery/3d.php";</script>';
	exit();
}
$stype = $_POST['stype'];
$btype = $_POST['btype'];
...
$orderinfo = $trow['qihao'].$zhnum.'3d'.$btype.$ctype.$dtype.$stype.'y';
$content = $_POST['content_0'];
if($_POST['key_0']!=StrToHex($orderinfo.$content, $postkey)) {
	echo "<script language=javascript>alert('投注信息被篡改，投注失败！');window.location.href='/m/lottery/3d.php';</script>";
	exit;
}
...
if (($btype == '跨度') || ($btype == '和值') || ($btype == '单双大小')){
	if (0 < $cp_zd){
		if (abs($_POST['money_0']) < $cp_zd){
			echo "<script language=javascript>alert('最低下注金额为".$cp_zd."RMB！');window.location.href='/m/lottery/3d.php';</script>";
			exit();
		}
	}else if (abs($_POST['money_0']) < $lowbet){
		echo "<script language=javascript>alert('最低下注金额为".$lowbet."RMB！');window.location.href='/m/lottery/3d.php';</script>";
		exit();
	}
	
	if (0 < $cp_zg){
		if ($cp_zg < abs($_POST['money_0'])){
			echo "<script language=javascript>alert('最高下注金额为".$cp_zg."RMB！');window.location.href='/m/lottery/3d.php';</script>";
			exit();
		}
	}
	$params = array(':class2' => $btype, ':class3' => $ctype);
	$sql = 'select id,class1,class2,class3,odds,modds,locked from lottery_odds where class1=\'3d\' and class2=:class2 and class3=:class3 order by ID asc';
	$stmt = $mydata1_db->prepare($sql);
	$stmt->execute($params);
	if ($row = $stmt->fetch()){
		$odds = $row['odds'];
	}else{
		echo "<script language=javascript>alert('非法投注！');window.location.href='/m/lottery/3d.php';</script>";
		exit();
	}
	$allmoney = abs($_POST['money_0']);
	if ($allmoney == 0){
		echo "<script language=javascript>alert('非法投注金额！');window.location.href='/m/lottery/3d.php';</script>";
		exit();
	}
	...
	if ($btype == '组二'){
		...
		$psql = 'select id,class1,class2,class3,odds,modds,locked from lottery_odds where class1=\'3d\' and class2=:class2 order by ID asc';
...
if ($allmoney <= $userinfo['money']){
	...
	echo "<script language=javascript>alert('余额不足，请先充值，后下注');window.location.href='/m/lottery/3d.php';</script>";
	exit();
}
...
for ($i = 0;$i < count($orderList);$i++){
	...
	$stmt = $mydata1_db->prepare('INSERT INTO k_money_log (uid,userName,gameType,transferType,transferOrder,transferAmount,previousAmount,currentAmount,creationTime) VALUE (:uid,:userName,\'FC3D\',\'BET\',:transferOrder,-:transferAmount,:previousAmount,:currentAmount,:creationTime)');
	$stmt->execute($params);
	$preMoney = $preMoney - $orderList[$i]['money'];
}
echo '<script type="text/javascript">alert("下注成功，请到下注记录中查询本注单！");window.location.href="/m/lottery/3d.php";</script>';
exit();
?>