...
if(empty($_SESSION['subtime'])){
	...
	if($s<$sub){
		die('<script type="text/javascript">alert("'.$sub.'秒内禁止重复提交，返回下注页面！");window.location.href="/m/lottery/cqssc.php";</script>');
	}else{
		$_SESSION['subtime'] = time();
	}
}
...
if (count($datas) <= 0){
	echo '<script type="text/javascript">alert("该页面为提交注单页面无法刷新，返回下注页面！");window.location.href="/m/lottery/cqssc.php";</script>';
	exit();
}
...
if ($_REQUEST['type'] == 2){
	...
	for ($i = 0;$i < count($datas);$i++){
		...
		$params = array(':did' => date('YmdHis', time()) . rand(1000, 9999), ':uid' => $uid, ':username' => $username, ':addtime' => date('Y-m-d H:i:s', time()), ':type' => '重庆时时彩', ':qishu' => $qishu, ':mingxi_1' => $qiuhao, ':mingxi_2' => $wanfa, ':odds' => $odds, ':money' => $money, ':win' => $money * $odds, ':bet_date' => date('Y-m-d', time()));
		...
		$stmt = $mydata1_db->prepare('INSERT INTO k_money_log(uid,userName,gameType,transferType,transferOrder,transferAmount,previousAmount,currentAmount,creationTime)SELECT uid,userName,\'CQSSC\',\'BET\',:transferOrder,-:transferAmount,money+:tmpMoney,money+:tmpMoney2-:transferAmount2,:creationTime FROM k_user WHERE uid=:uid');
		$stmt->execute($params);
		$tmpMoney -= $money;
	  ...
	  <div data-role="tabs" id="tabs" class="ui-tabs ui-widget ui-widget-content ui-corner-all"> 
		  <div id="three" class="ui-body-d ui-content" style="padding:0px;"> 
			  ...
			  <div class="xiazhu" style="line-height: 28px;clear:both;" data-role="none"> 
				  ...
				  <?php  for ($i = 0;$i < count($orderList);$i++){?> 						  
					  ...
					  <li style="border-top:1px solid #DDD;font-weight: bold;background-color:#EDEDED;width:25%;"  data-role="none"><?=$orderList[$i]['money'];?></li>
				  <?php }?> 					 
				  </ul> 
			  </div> 
		  </div> 
		  <div style="clear:both;padding-top:10px;padding-left:10px;"> 
			  <div style="clear:both;text-align:center;"> 
				  <a href="javascript:window.location.href='cqssc.php';" class="ui-btn ui-btn-inline"> 
					  继续下注 
				  </a> 
