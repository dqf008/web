...
$type = isset($_GET['i'])&&in_array($_GET['i'], array_keys($qxc_type))?$_GET['i']:'2';
?>
<!DOCTYPE html> 
  ...
  <body class="mWrap ui-page ui-page-theme-a ui-page-active" data-role="page" tabindex="0" style="min-height: 659px;"> 
      ...
      <section class="sliderWrap clearfix"> 
          ...
          <?php if (0 < $tcou){?>                 
              ...
              <div style="color:#f60;font-weight:bold;font-size:12pt;">玩法赔率：<?php echo $odds[$bet_type['zx'.$type]]; ?></div>
          <?php }else{?>                  
          ...
          <div data-role="tabs" id="tabs" class="ui-tabs ui-widget ui-widget-content ui-corner-all"> 
              <!-- <div id="three" class="ui-body-d ui-content ui-tabs-panel ui-widget-content ui-corner-bottom" style="padding: 0px;" aria-labelledby="ui-id-3" role="tabpanel" aria-expanded="true" aria-hidden="false">  -->
                  <div class="qxc" style="clear:both;" data-role="none"> 
                      <ul data-selected="0" class="zixian1" data-role="none"> 
                          <li style="border-top:1px solid #DDD;font-weight: bold;background-color:#EDEDED;text-align:right" data-role="none">字</li> 
                          <?php for($i=0;$i<=4;$i++){ ?><li onclick="qxc_zx(this)"><?php echo $i; ?></li><?php } ?>
                      </ul> 
                      <ul data-selected="0" class="zixian2" data-role="none"> 
                          <li style="border-top:1px solid #DDD;font-weight: bold;background-color:#EDEDED;text-align:left" data-role="none">现</li> 
                          <?php for($i=5;$i<=9;$i++){ ?><li onclick="qxc_zx(this)"><?php echo $i; ?></li><?php } ?>
                      </ul> 
                  </div> 
      ...
      <section class="mContent clearfix qxc_post" style="padding:0px;display:none"> 
        ...
        <input type="hidden" name="type" value="zx<?php echo $type; ?>" />
      </form>
      ...
      <script type="text/javascript"> 
          ...
          function RemainTime(){ 
            ...
            function qxc_zx(e){
                var p;
                return e = $(e), p = e.parents("div").find("li"),
                !e.hasClass("disabled") && (e.toggleClass("selected"),
                (p.filter(".selected").size()>=qxc_type ? p.not(".selected").addClass("disabled") : p.not(".selected").removeClass("disabled")))
            }
            function qxc_add(){
                var s, r;
                return r = $(".qxc li.selected").map(function(){
                    return $(this).text()
                }).get(),
                s = qxc_check(r),
                (s[0]>0) ? (
                    $(".qxc_post").show(),
                    $(".qxc li").removeClass("disabled selected"),
                    $(".qxc_post ul.ui-listview").append("<li class=\"ui-li-static ui-body-inherit ui-first-child\" data-count=\""+s[0]+"\" ><label><span>"+r.join("")+"</span>，<a href=\"javascript:;\">删除</a></label></li>"),
            ...
            function qxc_check(r){
                ...
                switch($("#qxc_post input[name=type]").val()){
                    ...
                    break;
                    case "zx2":
                    return r.length>2 ? [0, "最多选择两个号码"] : [(r.length<2?0:1), "至少选择两个号码"];
                    break;
                    case "zx3":
                    return r.length>3 ? [0, "最多选择三个号码"] : [(r.length<3?0:1), "至少选择三个号码"];
                    break;
                }
            }
            (function(){
