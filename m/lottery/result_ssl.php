...
<body class="mWrap ui-page ui-page-theme-a ui-page-active" data-role="page" data-theme="a"  tabindex="0" style="min-height: 892px;"> 
  ...
  <section class="sliderWrap clearfix"> 
	  <div data-role="header" style="overflow:hidden;" role="banner" class="ui-header ui-bar-inherit"> 
		  <h1 class="ui-title" role="heading" aria-level="1">上海时时乐</h1> 
	  </div> 
  </section> 
  ...
  <section class="mContent clearfix" style="padding:0px;"> 
	  <div data-role="cotent"> 
		  ...
		  <?php 
			...
			$sql = 'select * from lottery_k_ssl where qihao like :datetime order by qihao desc';
			$stmt = $mydata1_db->prepare($sql);
			$stmt->execute($params);
			while ($rows = $stmt->fetch()){
				$hmhz = $rows['hm1'] + $rows['hm2'] + $rows['hm3'];
				if (13 < $hmhz){
					$hzdx = '大';
				}else{
					$hzdx = '小';
		  ...
		  ?> 					 
				  <li data-role="list-divider" role="heading" class="ui-li-divider ui-bar-inherit ui-first-child"> 
					  <label>开奖期数：<?=$rows['qihao'];?></label> 
				  </li> 
				  <li class="ui-li-static ui-body-inherit"> 
					  <label>开奖时间：<?=$rows['addtime'];?></label> 
				  </li> 
				  <li class="ui-li-static ui-body-inherit"> 
					  <label id="haoma">号码： 
						  <span class="ball_b"><?=$rows['hm1'];?></span> 
						  <span class="ball_b"><?=$rows['hm2'];?></span> 
						  <span class="ball_b"><?=$rows['hm3'];?></span> 
					  </label> 
				  </li> 
				  <li class="ui-li-static ui-body-inherit"> 
