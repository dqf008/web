...
<body class="mWrap ui-page ui-page-theme-a ui-page-active" data-role="page" data-theme="a"  tabindex="0" style="min-height: 892px;"> 
  ...
  <section class="sliderWrap clearfix"> 
	  <div data-role="header" style="overflow:hidden;" role="banner" class="ui-header ui-bar-inherit"> 
		  <h1 class="ui-title" role="heading" aria-level="1">极速赛车</h1> 
	  </div> 
  </section> 
  ...
  <section class="mContent clearfix" style="padding:0px;"> 
	  <div data-role="cotent"> 
		  <form action="result_jssc.php" method="get" name="lt_form" data-role="none" data-ajax="false"> 
		  ...
		  <?php 
			...
			$params = array(':type' => 'JSSC', ':qtime' => strtotime($qtime.' 00:00:00'), ':etime' => strtotime($qtime.' 23:59:59'));
		...
		?>						  
					  ...
					  <li class="ui-li-static ui-body-inherit"> 
						  <label>冠军总和：<?=$rows['value']['info'][0];?> / <?=$rows['value']['info'][1];?> / <?=$rows['value']['info'][2];?></label> 
					  </li> 
					  <li class="ui-li-static ui-body-inherit"> 
						  <label>1V10：<?=$rows['value']['info'][3];?></label> 
					  </li> 
					  <li class="ui-li-static ui-body-inherit"> 
						  <label>2V9：<?=$rows['value']['info'][4];?></label> 
					  </li> 
					  <li class="ui-li-static ui-body-inherit"> 
						  <label>3V8：<?=$rows['value']['info'][5];?></label> 
					  </li> 
					  <li class="ui-li-static ui-body-inherit"> 
						  <label>4V7：<?=$rows['value']['info'][6];?></label> 
					  </li> 
					  <li class="ui-li-static ui-body-inherit"> 
						  <label>5V6：<?=$rows['value']['info'][7];?></label> 
					  </li>
				<?php $i++;
				  }
				...
				<li class="ui-li-static ui-body-inherit ui-last-child"> 
				  <div style="clear:both;text-align:left;margin:10px auto;line-height:28px;"> 
					  ...
					  <a class="ui-link" href="result_jssc.php?t=<?=$t;?>&page=1">首页</a> 
					  <a class="ui-link" href="result_jssc.php?t=<?=$t;?>&page=<?=($thisPage - 1) < 1 ? 1 : $thisPage - 1;?>">上一页</a> 
					  <a class="ui-link" href="result_jssc.php?t=<?=$t;?>&page=<?=$total_page < ( $thisPage + 1) ? $total_page : $thisPage + 1;?>">下一页</a> 
					  <a class="ui-link" href="result_jssc.php?t=<?=$t;?>&page=<?=$total_page;?>">末页</a> 
				  </div>     
			  </li>
			  <?php }?> 			  
			</ul> 
