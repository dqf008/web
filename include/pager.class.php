...
class pager{
	...
	public function GetPagerContent(){
		...
		if ($this->totalPagesCount <= 10)
		{
		...
		}else if ((5 < $this->pageIndex) && ($this->pageIndex <= $this->totalPagesCount - 5)){
			$i = 1;
			for (;$i <= 3;$i++)
			{
				$str .= '<a href=\'' . $this->pageUrl . '=' . $i . '\' >' . $i . '</a>' . "\n";
			...
			$str .= '<span class="dot">……</span>';
			$i = $this->pageIndex - 1;
			for (;$i <= ($this->totalPagesCount - 5) + 1;
			$i++)
			{
				if ($i == $this->pageIndex)
				{
