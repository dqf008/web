...
switch ($_POST['action']) {
    ...
    case 'register': //用户注册
        ...
        switch (false) {
            ...
            case $web_site['show_qq']!=1||(preg_match('/^(([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?))|[1-9]\d{4,9}$/', $_POST['email'])):
                unset($_SESSION['randcode']);
    ...
    case 'login': //用户登录
        ...
        switch (false) {
            ...
            case $stmt->execute([':username' => htmlEncode($_POST['username']), ':password' => md5($_POST['password'])]):
            case $stmt->rowCount()>0:
    ...
    case 'config': //配置信息
        ...
        if(empty($slides)){
            $MOBILE['output']['slides'] = ['../newindex/mobile/b_1.jpg', '../newindex/mobile/b_2.jpg', '../newindex/mobile/b_3.jpg'];
        }else{
                ...
                switch ($_POST['type']) {
                    case 'web': //系统通知
                        ...
                        if($MOBILE['output']['totalCount']>0){
                            ...
                            $stmt->execute([':index' => ($_POST['page']-1)*$_POST['rows'], ':limit' => $_POST['rows']]);
                    ...
                    case 'sms': //站内短信
                        ...
                        if($MOBILE['output']['totalCount']>0){
                            ...
                            $stmt->execute([':uid' => $_SESSION['uid'], ':index' => ($_POST['page']-1)*$_POST['rows'], ':limit' => $_POST['rows']]);
                            while ($rows = $stmt->fetch()) {
                                $MOBILE['output']['msg'][] = [
                                    'id' => $rows['msg_id'],
        ...
        switch (false) {
            ...
            case $_POST['type']!='trans':
                ...
                if(isset($_POST['transType'])&&in_array($_POST['transType'], $types)){
                    ...
                    (isset($_POST['start'])&&preg_match('/^[1-9]\d{3}\-\d{2}\-\d{2}$/', $_POST['start']))||$_POST['start'] = date('Y-m-d', time()-604800);
                    (isset($_POST['end'])&&preg_match('/^[1-9]\d{3}\-\d{2}\-\d{2}$/', $_POST['end']))||$_POST['end'] = date('Y-m-d');
            ...
            case $_POST['type']!='online':
                ...
                break;
            default:
                $stmt = $mydata1_db->prepare('SELECT `money`, `pay_card`, `pay_num`, `pay_address`, `pay_name`, `qk_pwd` FROM `k_user` WHERE `uid`=:uid AND `is_delete`=0 AND `is_stop`=0 LIMIT 1');
                $stmt->execute([':uid' => $_SESSION['uid']]);
                if($rows = $stmt->fetch()){
                    include IN_MOBILE.'../member/function.php';
                    include IN_MOBILE.'../common/function.php';
                    ...
                    switch ($_POST['type']) {
                        ...
                        case 'save': //保存银行卡
                            switch (true) {
                                ...
                                default:
                                    include IN_MOBILE.'../class/user.php';
                                    if(user::update_paycard($_SESSION['uid'], $_POST['bankName'], $_POST['cardNo'], $_POST['address'], $rows['pay_name'], $_SESSION['username'])){
                                        $MOBILE['output']['status'] = 1;
                                        $MOBILE['output']['msg'] = [[
                        ...
                        case 'withdraw':
                            switch (false) {
                                ...
                                case $check['num']<=0:
                                    ...
                                    break;
                                case $stmt = $mydata1_db->prepare('UPDATE `k_user` SET `money`=`money`-:money1 WHERE `uid`=:uid AND `money`>=:money2'):
                                case $stmt->execute([':uid' => $_SESSION['uid'], ':money1' => $_POST['amount'], ':money2' => $_POST['amount']]):
                                ...
                                default:
                                    ...
                                    $stmt->execute([
                                        ...
                                        ':assets' => $rows['money']-$_POST['amount'],
                                        ':balance' => $rows['money'],
                                    ])&&$MOBILE['output'] = ['status' => 1, 'msg' => 'success'];
                                    break;
                            }
    ...
    case 'setFullName': //设置真实名字
        ...
        switch (false) {
            ...
            case preg_match('/^[a-zA-Z ]{1,20}$/', $_POST['fullName']) || preg_match('/^[\x{4e00}-\x{9fa5}]{1,10}$/u', $_POST['fullName']):
                $MOBILE['output']['msg'] = '真实姓名格式不正确';
                break;
        ...
        switch (false) {
            ...
            default:
                $types = [
                    ...
                    'BGLIVE' => ['bgmoney', ['BG视讯（含BG电子、BG捕鱼）', 2], 'giro_BG', '../cj/live/live_money_BG.php'],
                    'CQ9' => ['cq9money', 'CQ9电子', 'giro_CQ9', '../cj/live/live_money_CQ9.php'],
                    'SB' => ['sbmoney', ['申博视讯（含RT电子、LAX电子）', 2], 'giro_SB', '../cj/live/live_money_SB.php'],
                    'MG' => ['mgmoney', 'MG电子'],
                    'AGIN' => ['agmoney', ['AG国际厅（含XIN电子、AG街机）', 2]],
                ];
                $names = [];
                foreach($types as $key=>$val){
                    $names[$key] = $val[0];
                }
                $stmt = $mydata1_db->prepare('SELECT `iszhuan`, `money`, `'.implode('`, `', $names).'` FROM `k_user` WHERE `uid`=:uid AND `is_delete`=0 AND `is_stop`=0 LIMIT 1');
                $stmt->execute([':uid' => $_SESSION['uid']]);
                if($rows = $stmt->fetch()){
                    switch ($_POST['type']) {
                        case 'check': //查询缓存额度
                        ...
                        case 'start': //开始转账
                            ...
                            switch (false) {
                                ...
                                case isset($_POST['money'])&&($_POST['money']=='all'||preg_match('/^[1-9]\d*$/', $_POST['money'])):
                                ...
                                default:
                                    ...
                                    if($_POST['out']!='SYSTEM'&&!money_change_game_is_close($_POST['out'])){
                                    ...
                                    }else if($_POST['in']!='SYSTEM'&&!money_change_game_is_close($_POST['in'])){
                                        $MOBILE['output']['msg'] = '转入钱包正在维护，请等待维护完毕';
                                    }else if($_POST['out']=='SYSTEM'&&($web_site['zh_low']>$_POST['money']||$_POST['money']>$web_site['zh_high'])){
                                        $MOBILE['output']['msg'] = '允许转账范围：<br />'.sprintf('%.2f', $web_site['zh_low']).' ~ '.sprintf('%.2f', $web_site['zh_high']);
                                    }else{
                                        ...
                                        switch (true) {
                                            case $_POST['out']=='SYSTEM'&&$_POST['in']!='SYSTEM':
                                                ...
                                                $MOBILE['output']['msg'] = $fun($_SESSION['uid'], $_POST['in'], 'IN', $_POST['money'], $allMoney);
                                                break;

                                            case $_POST['out']!='SYSTEM'&&$_POST['in']=='SYSTEM':
                                                $fun = isset($types[$_POST['out']][2])?$types[$_POST['out']][2]:'giro';
                                                $MOBILE['output']['msg'] = $fun($_SESSION['uid'], $_POST['out'], 'OUT', $_POST['money'], $allMoney);
                                                break;
                                        ...
                                        $MOBILE['output']['msg']=='ok'&&$MOBILE['output'] = ['status' => 1, 'msg' => 'success'];
                                    }
                                    break;
                            }
                            break;
                    ...
                    switch ($_POST['type']) {
                        ...
                        case 'Lottery': //系统彩票
                            ...
                            switch ($_POST['gameType']) {
                                ...
                                case 'PK10':
                                case 'XYFT':
                                    $tables = $_POST['gameType']=='CQSSC' ? 'c_bet' : 'c_bet_3';
                                    $MOBILE['output']['status'] = 1;
                                    $MOBILE['output']['msg'] = [];
                                    $MOBILE['output']['totalCount'] = 0;
                                    $MOBILE['output']['totalMoney'] = 0;
                                        ...
                                        while ($rows = $stmt->fetch()) {
                                            ...
                                            if($rows['atype']=='qxc'){
                                                if($rows['btype']=='定位'){
                                                    ...
                                                    $rows['dtype'].= ' - '.$rows['ctype'][0].'注 - '.sprintf('%.2f', $rows['ctype'][1]).'/注';
                                                }
                                ...
                                case 'MARKSIX':
                                    ...
                                    if($status){
                                        ...
                                        $params[':etime'] = date('Y-m-d H:i:s', strtotime($_POST['end'].' 23:59:59')+43200);
                                    ...
                                    if($sum['count']>0){
                                        ...
                                        $stmt->execute($params);
                                        while ($rows = $stmt->fetch()) {
                                            $rows['adddate'] = date('Y-m-d H:i:s', strtotime($rows['adddate'])-43200);
                                            ...
                                            if($rows['class1']=='过关'){
                                                ...
                                                $rows['content'] = [$rows['class1'].' - '.$rows['class2'], $rows['class3']];
                                            }
                                            if($rows['checked']==1){
                                                if($rows['bm']==2){
                                                    $rows['win'] = 0;
    ...
    case 'lottery': //彩票
        ...
        switch (true) {
            ...
            case $_POST['type']=='history': //开奖结果
                ...
                (isset($_POST['lastId'])&&preg_match('/^\d+$/', $_POST['lastId']))||$_POST['lastId'] = 0;
                (isset($_POST['rows'])&&preg_match('/^[1-9]\d*$/', $_POST['rows']))||$_POST['rows'] = 10;
                (isset($_POST['date'])&&preg_match('/^[1-9]\d{3}\-\d{2}\-\d{2}$/', $_POST['date']))||$_POST['date'] = date('Y-m-d');
                $_POST['rows']>100&&$_POST['rows'] = 100;
                switch ($_POST['gameType']) {
                    case 'jssc':
                    case 'jsssc':
                                ...
                                $getInfo = function($opencode){
                                    $r = function($n){
                                        ...
                                        if($n[0]==$n[1]&&$n[0]==$n[2]&&$n[1]==$n[2]){
                                            ...
                                            return '对子';
                                        }else if($a == '019'||$a == '089'||$s($n, 3)){
                                            return '顺子';
                                        }else if(preg_match('/0\d9/', $a)||$s($n, 2)){
                                            return '半顺';
                                        }else{
                                            return '杂六';
                                        }
                    ...
                    case 'kl8':
                        ...
                        if($rows['count']>0){
                            ...
                            while ($rows = $stmt->fetch()) {
                                $rows['opencode'] = $rows['openinfo'] = [];
                                foreach($rows as $key=>$val){
                                    substr($key, 0, 2)=='hm'&&$rows[substr($key, 2)>10?'openinfo':'opencode'][] = substr('00'.$val, -2);
                                }
                                $MOBILE['output']['msg'][] = [
                                    'issue' => $rows['qihao'],
                                    'opentime' => $rows['fengpan'],
