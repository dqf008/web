...
.run(function($ionicPlatform, $rootScope, $log, $location, PATH, ROUTE_ACCESS, Tools) {
...
.controller('AppCtrl', function($scope, $rootScope, $location, $ionicHistory, $log, Tools, PATH, My, md5, DOMAIN_CONFIG) {
	$scope.My = My;
	$rootScope.appConfig = {};
	$rootScope.appData = {};
	...
	$scope.logout = function() {
		Tools.confirm('确认要退出该帐号？', function() {
			...
			Tools.ajax({
				method: 'POST',
				url: 'ajax.php',
				params: {action: "logout", token: token},
				backdrop: true,
				success: function(data) {
					$location.path(PATH.loginPath);
...
.controller('HomeCtrl', function($scope, $rootScope, $log, $ionicSlideBoxDelegate, $document, Tools) {
		...
		switch(!0){
			...
			Tools.ajax({
				...
				params: {
					...
					jsr: !$scope.config.agent?$scope.config.agent:void 0
				},
				backdrop: true,
				success: function(data) {
...
.directive('zlNotice', function(Tools) {
	return {
		restrict: 'EC',
		scope: true,
		template: '<marquee scrollamount="3" style="height:35px"><span ng-repeat="msg in appConfig.notices" style="padding-right:20px" ng-click="showNoticeInfo($index);">{{msg}}</span></marquee>',
		replace: true,
		link: function(scope, element, attrs) {
			scope.showNoticeInfo = function (index) {
				Tools.modal({
					title: '网站公告',
					template: scope.appConfig.notices[index]
				});
			}
		}
	}
...
.directive('clipBoard', function(Tools) {
	return {
		restrict: 'C',
		scope: true,
		link: function(scope, element, attrs) {
...
.provider('Tools', function(DOMAIN_CONFIG) {
	...
	return {
		...
		$get: function($timeout, ENV, $rootScope, $http, $state, $injector, $location, $log, $ionicPopup, $ocLazyLoad, $ionicLoading, $cookies, $ionicHistory, PATH) {
			var self = this;
			...
			var ajax = function(opts) {
				...
				var req = {method: method, url: apiPath() + opts.url, headers: {}};
			...
			var alert = function(msg, callback, duration) {
				...
				var alertPopup = $ionicPopup.alert({title: msg[0], template: msg[1], okText: '确认'});
			...
			var modal = function(opts, duration) {
				...
				var popup = $ionicPopup.show({
					...
					buttons: angular.isFunction(opts.callback) ? [
						...
						{text: opts.okText || '确定', type: 'button-positive', onTap: function(e) {
							// 防止重复提交
							if(submitFlag) {
								return;
							}
					...
					] : [{text: '<b>'+(opts.cancelText || '确定')+'</b>', type: 'button-positive'}]
				});
				!duration || $timeout(function(){
					popup.close();
				}, duration);
...
.config(function($stateProvider, $urlRouterProvider, ToolsProvider, ROUTE_ACCESS, ionicDatePickerProvider) {
	$stateProvider
	...
	.state('ucenter.mypwd', {
		...
		views: {
			'ucenter-view': {
				templateUrl: 'views/ucenter/mypwd.html',
	...
	.state('ucenter.transfer', {
		...
		views: {
			'ucenter-view': {
				templateUrl: 'views/ucenter/transfer.html',
	...
	.state('ucenter.quickTransfer', {
		...
		views: {
			'ucenter-view': {
				templateUrl: 'views/ucenter/quickTransfer.html',
				controller: 'TransferCtroller'
			}
		}
	})
  ...
  //   })
	...
	.state('bank.money', {
		...
		views: {
			'bank-view': {
				templateUrl: 'views/bank/money.html',
	...
	.state('bank.qrcode', {
		...
		views: {
			'bank-view': {
				templateUrl: 'views/bank/qrcode.html',
	...
	.state('bank.transfer', {
		...
		views: {
			'bank-view': {
				templateUrl: 'views/bank/transfer.html',
			...
			Tools.ajax({
				...
				success: function(data) {
					if(data.status&&data.status==1){
						if(self.userMsg>=0&&data.msg>self.userMsg){
							Tools.tip('收到'+(data.msg-self.userMsg)+'条新消息');
							// Tools.modal({template:data.message.message});
						}
						self.userMsg = data.msg;
					}else{
						self.clear();
