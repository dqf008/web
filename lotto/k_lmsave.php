...
if ($_POST['pabc'] == 5){
	...
	switch ($_POST['pan3']){
		...
		break;
		case '1': $number2 = '1,11,21,31,41';
		$m1 = 5;
		break;
		case '0': $number2 = '10,20,30,40';
		$m1 = 4;
		break;
		default: break;
	}
...
if ($_POST['pabc'] == 4){
	switch ($_POST['pan3']){
		...
		break;
		case '1': $number1 = '1,11,21,31,41';
		$n1 = 5;
		break;
		case '0': $number1 = '10,20,30,40';
		$n1 = 4;
		break;
		default: break;
	}
...
switch ($_POST['rtype']){
	...
	if (($_POST['pabc'] == 3) || ($_POST['pabc'] == 4) || ($_POST['pabc'] == 5)){
		$mu = explode(',', $number1);
		$mu1 = explode(',', $number2);
		$mama = '1,1';
	...
	case '四中一': if ($_POST['pabc'] == 2){
		...
		for ($d = 0;$d < (count($mu) - 1);$d = $d + 1){
			...
			for ($f = 0;$f < (count($mu) - 1);$f = $f + 1){
				...
				for ($t = 0;$t < (count($ma) - 1);$t = $t + 1){
					$ma11 = explode(',', $ma[$t]);
					if (($ma11[0] != $mu[$f]) && ($ma11[1] != $mu[$f]) && ($ma11[2] != $mu[$f]) && ($ma11[1] != $mu[$d]) && ($ma11[2] != $mu[$d]) && ($ma11[3] != $mu[$d])){
						if (($f < $t) && ($d < $f)){
...
$params = array(':kithe' => $Current_Kithe_Num, ':username' => $_SESSION['kauser'], ':class1' => '连码', ':class2' => $rtype);
...
$stmt->execute($params);
$sum_money = $stmt->fetchColumn();
if ($zs == 0){
	echo "<script>alert('请选择相应码数!');window.location.href='index.php?action=left';</script>";
	exit();
}
$XF = 21;
...
$orderinfo = $zzz . $number2 . $number3 . $rate_id;
	  ...
	  <td valign="top" class="Left_Place">
        ...
        <tr>
          <td width="35%" colspan="2" height="25" class=t_td_caption_1 align="center" style="LINE-HEIGHT: 22px"><div align="center"><FONT color=#cc0000>连码&nbsp;<?=ka_bl($rate_id,"class2")?>
                  <? if (ka_bl($rate_id,"class2")=="三中二" or ka_bl($rate_id,"class2")=="三中二") {echo "之".ka_bl($rate_id,"class3"); }?>
				  
          </FONT> @ <strong><FONT color=#ff0000><?=ka_bl($rate_id,"rate")?></FONT></strong> </div><?=$number1?>
        ...
        <tr>
          <td height="25" colspan="2" bgcolor="#ffffff" style="LINE-HEIGHT: 23px">&nbsp;下注金额:
            <INPUT name=gold id=gold onKeyPress="return CheckKey()" onKeyUp="return CountWinGold()" value="<?=$_POST['jq']?>" size=8 maxLength=8></td>
        </tr>
        <tr>
