...
{?> 	  <tr class="Ball_tr_H"> 
	...
	<?php }?> 	  </tr></table>  	  </td> 
      ...
      style="HEIGHT: 18px"  class="input1" size="4" name="Num_<?=$I+1+6?>" id="Num_<?=$I+1+6?>" />
        ...
        <td height="25" bgcolor="f1f1f1"  ><table align=center><tr><?php $params = array(':sx' => $drop_table[$I + 6][0]);
	$stmt = $mydata2_db->prepare('Select m_number from ka_sxnumber where sx=:sx order by id');
	$stmt->execute($params);
	$image = $stmt->fetch();
	$xxm = explode(',', $image['m_number']);
	...
	<?php }?> 	  </tr></table>  	  </td> 
      ...
      </tr><?php }?> 	  <INPUT type=hidden value=0 name=gold_all> 
  </table><br /> 
...
switch ($bb)
{
	...
	case 'B':?> if (num2!=num3){ 
	  document.all[bl].innerHTML= "<SPAN STYLE='background-color:ffff00;WIDTH: 100%;height:25px;vertical-align:middle;' ><span style='display:inline-block;height:100%;vertical-align:middle;'></span><font color=ff0000><b>"+eval(Math.round(num2*100)+"-<?=$bsx * 100;?>")/100+"</b></font></span>";
	  }else{ 
  document.all[bl].innerHTML= eval(Math.round(num2*100)+"-<?=$bsx * 100;?>")/100;}<?php break;
	case 'C':?> if (num2!=num3){ 
	  document.all[bl].innerHTML= "<SPAN STYLE='background-color:ffff00;WIDTH: 100%;height:25px;vertical-align:middle;' ><span style='display:inline-block;height:100%;vertical-align:middle;'></span><font color=ff0000><b>"+eval(Math.round(num2*100)+"-<?=$csx * 100;?>")/100+"</b></font></span>";
	  }else{ 
  document.all[bl].innerHTML= eval(Math.round(num2*100)+"-<?=$csx * 100;?>")/100;}<?php break;
	case 'D':?>

  if (num2!=num3){ 
	  document.all[bl].innerHTML= "<SPAN STYLE='background-color:ffff00;WIDTH: 100%;height:25px;vertical-align:middle;' ><span style='display:inline-block;height:100%;vertical-align:middle;'></span><font color=ff0000><b>"+eval(Math.round(num2*100)+"-<?=$dsx * 100;?>")/100+"</b></font></span>";
	  }else{ 
  document.all[bl].innerHTML= eval(Math.round(num2*100)+"-<?=$dsx * 100;?>")/100;}<?php break;
