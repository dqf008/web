...
switch(false){
    ...
    default:
        $bet_qishu = fmod($LOT['bjtime']-14400, 300);
        if(($bet_qishu >=295 && $bet_qishu <=300) || ($bet_qishu >=0 && $bet_qishu <=6)){
            result('已经封盘，禁止下注');
            break;
        }
        if($bet_qishu<=300){
            $bet_qishu = $LOT['bjtime']-$bet_qishu;
            $sql = 'SELECT `class`, `value` FROM `c_odds_data` WHERE `type`=\'WFK3\'';
                    ...
                    if(check_odds($id1, $id2)){
                        ...
                        $order[] = array(
                            ...
                            ':value' => serialize(array(
                                ...
                                'odds' => $odds[$val['Id']],
                                'qishu' => date('Ymd', $bet_qishu).substr('0000'.(floor(fmod($bet_qishu-14400, 86400)/300)+1), -3),
                                'class' => array($LOT['odds'][$id1][0], $val['BetContext']),
                            )),
                        );
                    }else{
                        $bet_money = 0;
