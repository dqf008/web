...
switch(false){
    ...
    default:
        ...
        $sql = 'SELECT `qishu` FROM `c_opentime_4` WHERE `kaipan`<=:kaipan AND `fengpan`>=:fengpan ORDER BY `fengpan` ASC';
        $stmt = $mydata1_db->prepare($sql);
        $stmt->execute($params);
        if($stmt->rowCount()>0){
            $rows = $stmt->fetch();
            $daynum = floor(($LOT['bjtime']-strtotime($web_site['pk10_ktime'].' 00:00:00'))/3600/24)-1;
            $bet_qishu = $web_site['pk10_knum'];
            $bet_qishu+= $rows['qishu'];
            $bet_qishu+= $daynum*179;
