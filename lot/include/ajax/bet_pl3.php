    ...
    default:
        ...
        if($stmt->rowCount()>0){
            ...
            foreach($LOT['odds'] as $id1=>$val){
                $k1 = $val[0][0];
                $k2 = $val[0][1];
                !isset($class1[$k1])&&$class1[$k1] = array();
            ...
            foreach($LOT['input']['betParameters'] as $val){
                if(isset($odds[$val['Id']])&&$odds[$val['Id']]==floatval($val['Lines'])){
                    ...
                    if(check_odds($id1, $id2)){
                        ...
                        $order[] = array(
                            ':mid' => $bet_qishu,
                            ':uid' => date('YmdHis', $LOT['mdtime']).rand(1000, 9999),
                            ':atype' => 'pl3',
                            ':btype' => $LOT['odds'][$id1][0][0],
                            ':ctype' => isset($LOT['odds'][$id1][0][2])?$LOT['odds'][$id1][0][1]:$LOT['odds'][$id1][1][$id2],
                            ':dtype' => isset($LOT['odds'][$id1][0][2])?$LOT['odds'][$id1][0][2]:$LOT['odds'][$id1][0][1],
