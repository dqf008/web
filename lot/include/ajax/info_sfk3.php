...
$LOT['output']['Obj']['CurrentPeriod'].= substr('000'.(floor($LOT['output']['Obj']['OpenCount']/180)+1), -3);
$LOT['output']['Obj']['OpenCount'] = 180-fmod($LOT['output']['Obj']['OpenCount'], 180);
$LOT['output']['Obj']['CloseCount'] = $LOT['output']['Obj']['OpenCount']-5;
//$LOT['output']['Obj']['CloseCount'] = 0;
/* 加载赔率信息 */
$sql = 'SELECT `class`, `value` FROM `c_odds_data` WHERE `type`=\'SFK3\' ORDER BY `class` ASC';
...
/* 查询用户数据 */
if($LOT['user']['login']){
    $params = array(':time' => $LOT['mdtime']-fmod($LOT['mdtime']-14400, 86400)-43200, ':type' => 'SFK3', ':uid' => $LOT['user']['uid']);
