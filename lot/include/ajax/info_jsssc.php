...
/* 加载赔率信息 */
$sql = 'SELECT `class`, `value` FROM `c_odds_data` WHERE `type`=\'JSSSC\' ORDER BY `class` ASC';
...
/* 加载长龙信息 */
$cache_jsc = IN_LOT.'../cache/lot_jsssc.php';
if($LOT['more']&&file_exists($cache_jsc)){
    ...
    if(isset($cache_jsc['output'])){
        ...
        $LOT['output']['Obj']['ZongchuYilou'] = $cache_jsc['output']['ZongchuYilou'];
...
/* 查询用户数据 */
if($LOT['user']['login']){
    $params = array(':time' => $LOT['mdtime']-fmod($LOT['mdtime']-14400, 86400)-43200, ':type' => 'JSSSC', ':uid' => $LOT['user']['uid']);
