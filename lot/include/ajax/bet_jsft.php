...
switch(false){
    ...
    default:
        $lotteryType = isset($LOT['lottery_type'])?$LOT['lottery_type']:'jsft';
        $params = array(':kaipan' => date('H:i:s', $LOT['bjtime']), ':fengpan' => date('H:i:s', $LOT['bjtime']));
        $sql = 'SELECT `qishu` FROM `c_opentime_jsft` WHERE `kaipan`<=:kaipan AND `fengpan`>=:fengpan ORDER BY `fengpan` ASC';
        $stmt = $mydata1_db->prepare($sql);
        $stmt->execute($params);
        if($stmt->rowCount()>0){
            $rows = $stmt->fetch();
            $bet_qishu = date('Ymd', $LOT['bjtime']).substr('00'.$rows['qishu'], -4);
