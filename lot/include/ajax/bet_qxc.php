...
switch(false){
    ...
    default:
        ...
        $params = array(':kaipan' => $LOT['bjtime'], ':fengpan' => $LOT['bjtime']);
        $sql = 'SELECT `qishu` FROM `lottery_k_qxc` WHERE `kaipan`<=:kaipan AND `fengpan`>=:fengpan AND `status`=0 ORDER BY `kaijiang` ASC LIMIT 1';
        $stmt = $mydata1_db->prepare($sql);
        $stmt->execute($params);
        if($stmt->rowCount()>0){
            $rows = $stmt->fetch();
            $bet_qishu = $rows['qishu'];
            /* 加载赔率信息 */
