...
$sql = 'SELECT `qishu`, `kaipan`, `fengpan`, `kaijiang` FROM `c_opentime_tjssc` WHERE `kaipan`<=:kaipan AND `kaijiang`>=:kaijiang ORDER BY `kaijiang` ASC';
...
if($stmt->rowCount()>0){
    ...
    /* 加载赔率信息 */
    $sql = 'SELECT * FROM `c_odds_tjssc` ORDER BY `id` ASC';
    ...
    /* 加载长龙信息 */
    $lot_cache = IN_LOT.'../cache/lot_tjssc.php';
    ...
    /* 查询开奖信息 */
    $query = $mydata1_db->query('SELECT `qishu`, `ball_1`, `ball_2`, `ball_3`, `ball_4`, `ball_5` FROM `c_auto_tjssc` ORDER BY `qishu` DESC LIMIT 1');
    if($query->rowCount()>0){
        $rows = $query->fetch();
