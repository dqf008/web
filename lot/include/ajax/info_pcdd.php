...
/* 查询盘口信息 */
$lotteryType = isset($LOT['lottery_type']) ?$LOT['lottery_type']:'pcdd';
$params = array(':kaipan' => date('Y-m-d H:i:s', $LOT['mdtime']), ':fengpan' => date('Y-m-d H:i:s', $LOT['bjtime']));
$sql = 'SELECT `qihao`, `kaipan`, `fengpan` FROM `lottery_k_pcdd` WHERE `kaipan`<=:kaipan AND `fengpan`>=:fengpan AND `ok`=0  ORDER BY `fengpan` ASC LIMIT 1';
...
if($stmt->rowCount()>0){
    ...
    $sql = "SELECT `qihao` FROM `lottery_k_pcdd` ORDER BY `addtime` DESC LIMIT 1";
    $query = $mydata1_db->query($sql);
    if($query->rowCount()>0){
        $rows = $query->fetch();
        $LOT['output']['Obj']['CurrentPeriod'] = $rows['qihao'];
...
/* 加载长龙信息 */
$cache_pcdd = IN_LOT.'../cache/lot_pcdd.php';
if($LOT['more']&&file_exists($cache_pcdd)){
    $cache_pcdd = include($cache_pcdd);
    if(isset($cache_pcdd['output'])){
        if(isset($LOT['input']['numberPostion'])&&intval($LOT['input']['numberPostion'])==0){
            $LOT['output']['Obj']['LuZhu'] = $cache_pcdd['output']['LuZhu'];
        }
        $LOT['output']['Obj']['ChangLong'] = $cache_pcdd['output']['ChangLong'];
    }
}
/* 查询开奖信息 */
$sql = "SELECT * FROM `lottery_k_pcdd` WHERE `ok`=1 ORDER BY `addtime` DESC LIMIT 1";
$query = $mydata1_db->query($sql);
...
if($query->rowCount()>0){
    ...
    $rows['data'][] = $rows['total']>14?'大':'小';
    $rows['data'][] = fmod($rows['total'], 2)==0?'双':'单';
