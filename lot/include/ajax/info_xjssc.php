...
$sql = 'SELECT `qishu`, `kaipan`, `fengpan`, `kaijiang` FROM `c_opentime_xjssc` WHERE `kaipan`<=:kaipan AND `kaijiang`>=:kaijiang ORDER BY `kaijiang` ASC';
...
if($stmt->rowCount()>0){
    ...
    /* 加载赔率信息 */
    $sql = 'SELECT * FROM `c_odds_xjssc` ORDER BY `id` ASC';
    ...
    /* 加载长龙信息 */
    $lot_cache = IN_LOT.'../cache/lot_xjssc.php';
    ...
    /* 查询开奖信息 */
    $query = $mydata1_db->query('SELECT `qishu`, `ball_1`, `ball_2`, `ball_3`, `ball_4`, `ball_5` FROM `c_auto_xjssc` ORDER BY `qishu` DESC LIMIT 1');
    if($query->rowCount()>0){
        $rows = $query->fetch();
    ...
    /* 查询用户数据 */
    if($LOT['user']['login']){
        $params = array(':time' => date('Y-m-d 12:00:00', $LOT['mdtime']-43200), ':type' => '新疆时时彩', ':uid' => $LOT['user']['uid']);
