...
$sql = 'SELECT `qishu`, `kaipan`, `fengpan`, `kaijiang` FROM `c_opentime_3` WHERE `kaipan`<=:kaipan AND `kaijiang`>=:kaijiang ORDER BY `kaijiang` ASC';
$stmt = $mydata1_db->prepare($sql);
$stmt->execute($params);
if($stmt->rowCount()>0){
    $rows = $stmt->fetch();
    $LOT['output']['Obj']['CurrentPeriod'] = date('Ymd', $LOT['bjtime']).substr('00'.$rows['qishu'], -2);
    ...
    $sql = 'SELECT `qishu`, `kaijiang` FROM `c_opentime_3` ORDER BY `kaijiang` ASC LIMIT 1';
    $query = $mydata1_db->query($sql);
    if($query->rowCount()>0){
        $rows = $query->fetch();
...
/* 加载长龙信息 */
$lot_cache = IN_LOT.'../cache/lot_gdkl10.php';
...
/* 查询开奖信息 */
$query = $mydata1_db->query('SELECT `qishu`, `ball_1`, `ball_2`, `ball_3`, `ball_4`, `ball_5`, `ball_6`, `ball_7`, `ball_8` FROM `c_auto_3` ORDER BY `qishu` DESC LIMIT 1');
if($query->rowCount()>0){
    $rows = $query->fetch();
...
/* 查询用户数据 */
if($LOT['user']['login']){
    $params = array(':time' => date('Y-m-d 12:00:00', $LOT['mdtime']-43200), ':type' => '广东快乐10分', ':uid' => $LOT['user']['uid']);
