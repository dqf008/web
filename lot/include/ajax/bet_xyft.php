...
switch(false){
    ...
    default:
        ...
        $sql = 'SELECT `qishu` FROM `c_opentime_8` WHERE `kaipan`<=:kaipan AND `fengpan`>=:fengpan ORDER BY `fengpan` ASC';
        $stmt = $mydata1_db->prepare($sql);
        $stmt->execute($params);
        if($stmt->rowCount()>0){
            $rows = $stmt->fetch();
            $bet_qishu = date('Ymd', $LOT['bjtime']).substr('00'.$rows['qishu'], -3);
            if(strtotime(date('Y-m-d 04:04:00',$LOT['bjtime']))>$LOT['bjtime']){
                $bet_qishu = date('Ymd', $LOT['bjtime']-24*60*60).substr('00'.$rows['qishu'], -3);
            }
