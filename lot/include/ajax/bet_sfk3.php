...
switch(false){
    ...
    default:
        $bet_qishu = fmod($LOT['bjtime']-14400, 180);
        if(($bet_qishu >=175 && $bet_qishu <=180) || ($bet_qishu >=0 && $bet_qishu <=6)){
            result('已经封盘，禁止下注');
            break;
        }
        if($bet_qishu<=180){
            $bet_qishu = $LOT['bjtime']-$bet_qishu;
            $sql = 'SELECT `class`, `value` FROM `c_odds_data` WHERE `type`=\'SFK3\'';
                    ...
                    if(check_odds($id1, $id2)){
                        ...
                        $order[] = array(
                            ...
                            ':value' => serialize(array(
                                ...
                                'odds' => $odds[$val['Id']],
//                                'qishu' => date('Ymd', $bet_qishu).substr('000'.(floor(fmod($bet_qishu, 86400)/180)+1), -3),
                                'qishu' => date('Ymd', $bet_qishu).substr('0000'.(floor(fmod($bet_qishu-14400, 86400)/180)+1), -3),
