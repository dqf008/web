...
$LOT['output']['Obj']['PrePeriodNumber'] = ''; //最近开奖期号
...
$lotteryType = 'jsft';
$params = array(':kaipan' => date('H:i:s', $LOT['bjtime']), ':kaijiang' => date('H:i:s', $LOT['bjtime']),':name'=>$lotteryType);
$sql = 'SELECT `qishu`, `kaipan`, `fengpan`, `kaijiang` FROM `c_opentime_jsft` WHERE `kaipan`<=:kaipan AND `kaijiang`>=:kaijiang and `name`=:name ORDER BY `kaijiang` ASC';
$stmt = $mydata1_db->prepare($sql);
$stmt->execute($params);
if($stmt->rowCount()>0){
    $rows = $stmt->fetch();
    $qishu = date('Ymd', $LOT['bjtime']).substr('00'.$rows['qishu'], -4);
    ...
    /* 加载赔率信息 */
    $sql = "SELECT * FROM `c_odds_jsft` where `name`='$lotteryType' ORDER BY `id` ASC";
    ...
    $sql = "SELECT `qishu`, `kaijiang` FROM `c_opentime_jsft` where `name`='$lotteryType' ORDER BY `kaijiang` ASC LIMIT 1";
    $query = $mydata1_db->query($sql);
    if($query->rowCount()>0){
        $rows = $query->fetch();
...
/* 加载长龙信息 */
$cache_jsft = IN_LOT.'../cache/lot_jsft.php';
if($LOT['more']&&file_exists($cache_jsft)){
    $cache_jsft = include($cache_jsft);
    if(isset($cache_jsft['output'])){
        if(isset($LOT['input']['numberPostion'])&&intval($LOT['input']['numberPostion'])==0){
            $LOT['output']['Obj']['LuZhu'] = $cache_jsft['output']['LuZhu'];
        }
        $LOT['output']['Obj']['ChangLong'] = $cache_jsft['output']['ChangLong'];
    }
}
/* 查询开奖信息 */
$query = $mydata1_db->query("SELECT `qishu`, `ball_1`, `ball_2`, `ball_3`, `ball_4`, `ball_5`, `ball_6`, `ball_7`, `ball_8`, `ball_9`, `ball_10` FROM `c_auto_jsft` where `name`='$lotteryType' ORDER BY `qishu` DESC LIMIT 1");
if($query->rowCount()>0){
    $rows = $query->fetch();
...
/* 查询用户数据 */
if($LOT['user']['login']){
    $params = array(':time' => date('Y-m-d 12:00:00', $LOT['mdtime']-43200), ':type' => '极速飞艇', ':uid' => $LOT['user']['uid']);
