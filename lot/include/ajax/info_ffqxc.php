...
$lotteryType = isset($LOT['lottery_type'])?$LOT['lottery_type']:'ffxqc';
/* 查询盘口信息 */
...
if($stmt->rowCount()>0){
    ...
    $sql = "SELECT `qishu` FROM `lottery_k_new_qxc` where `lottery_name`='$lotteryType' ORDER BY `kaijiang` DESC LIMIT 1";
    $query = $mydata1_db->query($sql);
    if($query->rowCount()>0){
        $rows = $query->fetch();
...
/* 查询用户数据 */
if($LOT['user']['login']){
    $params = array(':mid1' => $LOT['output']['Obj']['PrePeriodNumber'], ':mid2' => $LOT['output']['Obj']['CurrentPeriod'], ':username' => $LOT['user']['username'], ':atype' => 'ffqxc');
