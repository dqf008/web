...
/* 查询盘口信息 */
$params = array(':kaipan' => $LOT['bjtime'], ':kaijiang' => $LOT['bjtime']);
$sql = 'SELECT `qishu`, `kaipan`, `fengpan`, `kaijiang` FROM `lottery_k_qxc` WHERE `kaipan`<=:kaipan AND `kaijiang`>=:kaijiang AND `status`=0 ORDER BY `kaijiang` ASC LIMIT 1';
...
if($stmt->rowCount()>0){
    ...
    $LOT['output']['Obj']['OpenCount'] = $rows['kaijiang']-$LOT['bjtime'];
    $web_site['qxc']!=0&&$LOT['output']['Obj']['CloseCount'] = 0;
    /* 加载赔率信息 */
    $sql = 'SELECT * FROM `lottery_odds` WHERE `class1`=\'qxc\' ORDER BY `id` ASC';
    ...
    $sql = 'SELECT `qishu` FROM `lottery_k_qxc` ORDER BY `kaijiang` DESC LIMIT 1';
    $query = $mydata1_db->query($sql);
    if($query->rowCount()>0){
        $rows = $query->fetch();
        $LOT['output']['Obj']['CurrentPeriod'] = $rows['qishu'];
...
/* 查询开奖信息 */
$sql = 'SELECT * FROM `lottery_k_qxc` WHERE `status`=1 ORDER BY `kaijiang` DESC LIMIT 1';
$query = $mydata1_db->query($sql);
...
/* 查询用户数据 */
if($LOT['user']['login']){
    $params = array(':mid1' => $LOT['output']['Obj']['PrePeriodNumber'], ':mid2' => $LOT['output']['Obj']['CurrentPeriod'], ':username' => $LOT['user']['username'], ':atype' => 'qxc');
