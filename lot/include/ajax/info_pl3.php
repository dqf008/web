...
/* 查询盘口信息 */
$params = array(':kaipan' => date('Y-m-d H:i:s', $LOT['bjtime']), ':fengpan' => date('Y-m-d H:i:s', $LOT['bjtime']-300));
$sql = 'SELECT `qihao`, `kaipan`, `fengpan` FROM `lottery_k_pl3` WHERE `kaipan`<=:kaipan AND `fengpan`>=:fengpan AND `ok`=0 ORDER BY `fengpan` ASC LIMIT 1';
...
if($stmt->rowCount()>0){
    ...
    $LOT['output']['Obj']['OpenCount'] = $rows['fengpan']-$LOT['bjtime']+300;
    $web_site['pl3']!=0&&$LOT['output']['Obj']['CloseCount'] = 0;
    /* 加载赔率信息 */
    ...
    $sql = 'SELECT `qihao` FROM `lottery_k_pl3` ORDER BY `fengpan` DESC LIMIT 1';
    $query = $mydata1_db->query($sql);
    if($query->rowCount()>0){
        $rows = $query->fetch();
        $LOT['output']['Obj']['CurrentPeriod'] = $rows['qihao'];
...
/* 加载长龙信息 */
$cache_pl3 = IN_LOT.'../cache/lot_pl3.php';
if($LOT['more']&&file_exists($cache_pl3)){
    $cache_pl3 = include($cache_pl3);
    if(isset($cache_pl3['output'])){
        if(isset($LOT['input']['numberPostion'])&&intval($LOT['input']['numberPostion'])==0){
            $LOT['output']['Obj']['LuZhu'] = $cache_pl3['output']['LuZhu'];
        }
        $LOT['output']['Obj']['ChangLong'] = $cache_pl3['output']['ChangLong'];
    }
}
/* 查询开奖信息 */
$sql = 'SELECT * FROM `lottery_k_pl3` WHERE `ok`=1 ORDER BY `fengpan` DESC LIMIT 1';
$query = $mydata1_db->query($sql);
...
/* 查询用户数据 */
if($LOT['user']['login']){
    $params = array(':mid1' => $LOT['output']['Obj']['PrePeriodNumber'], ':mid2' => $LOT['output']['Obj']['CurrentPeriod'], ':username' => $LOT['user']['username'], ':atype' => 'pl3');
