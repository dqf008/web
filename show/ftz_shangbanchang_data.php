...
$sql = 'select id from mydata4_db.bet_match where Match_CoverDate>now() and match_date!=:Match_Date and (Match_BHo+Match_BAo<>0 or Match_Bdpl+Match_Bxpl<>0) and Match_IsShowb=1 ' . $sqlwhere . ' order by Match_CoverDate,iPage,match_name,Match_Master,Match_ID,iSn';
...
if ($i == 1){
	...
	$sql = 'select match_name from mydata4_db.bet_match where Match_CoverDate>now() and match_date!=:Match_Date and (Match_BHo+Match_BAo<>0 or Match_Bdpl+Match_Bxpl<>0) and Match_IsShowb=1 group by match_name';
	$stmt = $mydata1_db->prepare($sql);
	$stmt->execute($params);
	$i = 0;
	$lsm = '';
