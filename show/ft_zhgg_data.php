...
$sqlwhere .= ' and Match_Master not like \'%角球数%\' and Match_Master not like \'%先开球%\' and Match_Master not like \'%星期%\' ';
...
if (trim($_GET['ids']) != ''){
	...
	$sql = 'select match_name from mydata4_db.bet_match WHERE id in(' . $id . ')  and   Match_CoverDate>now()  '.$sqlwhere.' group by match_name';
	$stmt = $mydata1_db->query($sql);
	$i = 0;
	$lsm = '';
	while ($row = $stmt->fetch()){
	...
	$ids = "";
	$sql = 'select id from mydata4_db.bet_match WHERE  Match_HalfId is not null AND Match_CoverDate>now()  ' . $sqlwhere . ' order by '.$esc;
	$stmt = $mydata1_db->query($sql);
	while ($row = $stmt->fetch()){
		$ids.=$row['id'].',';
		if (($start <= $i) && ($i <= $end)){
	...
	if ($i == 1){
		...
		$sql = 'select match_name from mydata4_db.bet_match WHERE  Match_CoverDate>now() '.$sqlwhere.'  group by match_name';
