...
if (trim($_GET['ids']) != ''){
	...
	$sql = 'select match_name from mydata4_db.bet_match WHERE id in(' . $id . ')  and Match_Date=\''.date('m-d').'\' and  Match_Type=1 AND Match_CoverDate>now() and Match_HalfId is not null '.$sqlwhere.' group by match_name';
	$stmt = $mydata1_db->query($sql);
	$i = 0;
	$lsm = '';
	while ($row = $stmt->fetch()){
	...
	$ids = "";
	$sql = 'select id from mydata4_db.bet_match WHERE Match_Type=1 AND Match_CoverDate>now() AND Match_Date=\''.date('m-d').'\' and Match_HalfId is not null ' . $sqlwhere . ' order by '.$esc;
	$stmt = $mydata1_db->query($sql);
	while ($row = $stmt->fetch()){
		$ids.=$row['id'].',';
		if (($start <= $i) && ($i <= $end)){
	...
	if ($i == 1){
		...
		$sql = 'select match_name from mydata4_db.bet_match WHERE Match_Type=1 AND Match_CoverDate>now() AND Match_Date=:Match_Date and Match_HalfId is not null group by match_name';
