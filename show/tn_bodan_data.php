...
$leaguenames = "";
if ( $leaguename != ''){
	$sql = 'select Match_Name from mydata4_db.tennis_Match WHERE Match_ID in ('.$leaguename.')';
...
$sql = 'select id from mydata4_db.tennis_Match where match_date=\''.date('m-d').'\' and Match_Type=1 and Match_IsShowbd=1 and Match_CoverDate>now() and Match_Bd21>0 ' . $sqlwhere . ' order by '.$esc;
...
if ($i == 1){
	...
	$sql = 'select match_name from mydata4_db.tennis_Match where match_date=\''.date('m-d').'\' and Match_Type=1 and Match_IsShowbd=1 and Match_CoverDate>now() and Match_Bd21>0 group by match_name';
