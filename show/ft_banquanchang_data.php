...
$sql = 'select id from mydata4_db.bet_match WHERE Match_Date=\''.date('m-d').'\' and Match_CoverDate>now() and Match_BqMM>0 ' . $sqlwhere . ' order by '.$esc;
...
if ($i == 1){
	...
	$sql = 'select match_name from mydata4_db.bet_match WHERE Match_Date=:Match_Date and Match_CoverDate>now() and Match_BqMM>0 group by match_name';
	$stmt = $mydata1_db->prepare($sql);
	$stmt->execute($params);
	$i = 0;
	$lsm = '';
