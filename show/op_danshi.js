...
function loaded(league,thispage,p){
	...
	$.getJSON("op_danshi_data.php?leaguename="+league+"&CurrPage="+thispage+"&ids="+ids+"&sort="+sort+"&callback=?",function(json){
		dbs  = null;
			...
			for(var i=0; i<dbs.length; i++){
				...
				if(dbs[i]["Match_BzM"]!="0" || dbs[i]["Match_Ho"]!=0 || dbs[i]["Match_DxXpl"]!="0" || dbs[i]["Match_DsDpl"]!="0"){
					...
					if(dbs[i]["Match_BzM"]>0 && dbs[i]["Match_BzG"]>0){
						...
						var BzM= '<span id="MH'+dbs[i]["Match_ID"]+'" class="'+color_H+'" onclick="Bright(this.id);"><span onclick="javascript:setbet(\'其他单式\',\'独赢-'+dbs[i]["Match_Master"]+'\',\''+dbs[i]["Match_ID"]+'\',\'Match_BzM\',0,0,\''+dbs[i]["Match_Master"]+'\');" title="'+TeamName(dbs[i]["Match_Master"])+'">'+formatNumber(dbs[i]["Match_BzM"],2)+'</span></span>';
						var BzG= '<span id="MC'+dbs[i]["Match_ID"]+'" class="'+color_C+'" onclick="Bright(this.id);"><span onclick="javascript:setbet(\'其他单式\',\'独赢-'+dbs[i]["Match_Guest"]+'\',\''+dbs[i]["Match_ID"]+'\',\'Match_BzG\',0,0,\''+dbs[i]["Match_Guest"]+'\');" title="'+TeamName(dbs[i]["Match_Guest"])+'">'+formatNumber(dbs[i]["Match_BzG"],2)+'</span></span>';
						...
						if (dbs[i]["Match_BzH"]==null || dbs[i]["Match_BzH"]-0.05<=0){
							...
							var BzH= '<span id="MN'+dbs[i]["Match_ID"]+'" class="'+color_N+'"><span onclick="javascript:setbet(\'其他单式\',\'独赢-和局\',\''+dbs[i]["Match_ID"]+'\',\'Match_BzH\',0,0,\'和局\');" title="和局">'+formatNumber(dbs[i]["Match_BzH"],2)+'</span></span>';
					...
					if(dbs[i]["Match_Ho"]>0 && dbs[i]["Match_Ao"]>0){
						...
						var Ho = '<span id="RH'+dbs[i]["Match_ID"]+'" class="'+color_H+'" onclick="Bright(this.id);"><span title="'+dbs[i]["Match_Master"]+'" onclick="javascript:setbet(\'其他单式\',\'让球-'+(dbs[i]["Match_ShowType"]=="H" ? "主让" :"客让")+dbs[i]["Match_RGG"]+"-"+dbs[i]["Match_Master"]+'\',\''+dbs[i]["Match_ID"]+'\',\'Match_Ho\',1,0,\''+dbs[i]["Match_Master"]+'\');" >'+formatNumber(dbs[i]["Match_Ho"],2)+'</span></span>';
						var Ao = '<span id="RC'+dbs[i]["Match_ID"]+'" class="'+color_C+'" onclick="Bright(this.id);"><span title="'+dbs[i]["Match_Guest"]+'" onclick="javascript:setbet(\'其他单式\',\'让球-'+(dbs[i]["Match_ShowType"]=="H" ? "主让" :"客让")+dbs[i]["Match_RGG"]+"-"+dbs[i]["Match_Guest"]+'\',\''+dbs[i]["Match_ID"]+'\',\'Match_Ao\',1,0,\''+dbs[i]["Match_Guest"]+'\');" >'+formatNumber(dbs[i]["Match_Ao"],2)+'</span></span>';
					...
					if(dbs[i]["Match_DxDpl"]>0 && dbs[i]["Match_DxXpl"]>0){
						...
						var DxDpl = '<span id="OUH'+dbs[i]["Match_ID"]+'" class="'+color_H+'" onclick="Bright(this.id);"><span title="大" onclick="javascript:setbet(\'其他单式\',\'大小-'+dbs[i]["Match_DxGG1"]+'\',\''+dbs[i]["Match_ID"]+'\',\'Match_DxDpl\',1,0,\''+dbs[i]["Match_DxGG1"]+'\');" >'+formatNumber(dbs[i]["Match_DxDpl"],2)+'</span></span>';
						var DxXpl = '<span id="OUC'+dbs[i]["Match_ID"]+'" class="'+color_C+'" onclick="Bright(this.id);"><span title="小" onclick="javascript:setbet(\'其他单式\',\'大小-'+dbs[i]["Match_DxGG2"]+'\',\''+dbs[i]["Match_ID"]+'\',\'Match_DxXpl\',1,0,\''+dbs[i]["Match_DxGG2"]+'\');">'+formatNumber(dbs[i]["Match_DxXpl"],2)+'</span></span>';
					...
					if(dbs[i]["Match_DsDpl"]>0 && dbs[i]["Match_DsSpl"]>0){
						...
						var EOO = '<span id="EOO'+dbs[i]["Match_ID"]+'" class="'+color_H+'" onclick="Bright(this.id);"><span title="单" onclick="javascript:setbet(\'其他单式\',\'单双-单\',\''+dbs[i]["Match_ID"]+'\',\'Match_DsDpl\',0,0,\'单\');">'+formatNumber(dbs[i]["Match_DsDpl"],2)+'</span></span>';
						var EOE = '<span id="EOE'+dbs[i]["Match_ID"]+'" class="'+color_C+'" onclick="Bright(this.id);"><span title="双" onclick="javascript:setbet(\'其他单式\',\'单双-双\',\''+dbs[i]["Match_ID"]+'\',\'Match_DsSpl\',0,0,\'双\');">'+formatNumber(dbs[i]["Match_DsSpl"],2)+'</span></span>';
