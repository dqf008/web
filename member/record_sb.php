			  ...
			  <table width="98%" border="0" cellspacing="0" cellpadding="5"> 
                  ...
                  <tr> 
                      <td height="25" colspan="9" align="center" bgcolor="#3C3C3C" style="color:#FFF">
                        <span style="color:#FF0">【温馨提醒】</span>沙巴体育注单需要同步后才能显示，请您耐心等待。
                      </td> 
                  </tr>
				  <tr> 
				  ...
				  <tr> 
					  <th align="center">投注时间(美东)</th> 
					  <th align="center">投注时间(北京)</th> 
					  <th align="center">订单号</th> 
					  <th align="center">局号</th> 
					  <th align="center">下注金额</th> 
					  <th align="center">派彩金额</th> 
                      <th align="center">有效投注额</th> 
				  ...
				  <?php
					...
					$sql = 'select id from `shababetdetail` where uid=:uid and betTime>=:begin_time and betTime<=:end_time order by id desc';
					...
					{
					$id = rtrim($id, ',');
					$sql	=	"select * from `shababetdetail` where id in($id) order by betTime desc";
					...
					while ($rows = $query->fetch())
					{
                        ...
                        $status = $rows['flag'];
                        $result = $rows['result'];
                        empty($status)&&$status='未知';
                        empty($result)&&$result='未知';
						if ($rows['netAmount'] == 0)
						{
						...
						else if ($rows['netAmount'] < 0)
						{
							...
							$result = '<span style=\'color:#00CC00;\'>'.$result.'</span>';
						}
						else
						{
						...
						<td align="center" style="line-height:30px"><?=date("Y-m-d H:i:s",strtotime($rows["betTime"]))?></td>
						<td align="center"><?=date("Y-m-d H:i:s",strtotime($rows["betTime"])+1*12*3600)?></td>
						<td align="center"><?=$rows["billNo"]?></td>
						<td align="center"><?=empty($rows["gameCode"])?'未知局号':$rows["gameCode"]?></td>
					...
					<tr>
						<th colspan="9" align="center">
							<div class="Pagination">
								...
								<?=$page->get_htmlPage("record_sb.php?cn_begin=".$cn_begin."&s_begin_h=".$s_begin_h."&s_begin_i=".$s_begin_i."&cn_end=".$cn_end."&s_end_h=".$s_end_h."&s_end_i=".$s_end_i);?>
							</div>
						</th>
					</tr>
			  </table> 
