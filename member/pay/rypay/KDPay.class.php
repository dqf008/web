...
class KDPay {
    ...
    private $P_OrderId;
    //卡类充值网关
    public $CardUrl = "https://gateway.ruyipay.com/pay/KDCard.aspx";
    //非卡类充值网关
    public $noCardUrl = "https://gateway.ruyipay.com/pay/KDBank.aspx";
    //订单查询接口
    public $queryUrl = "http://gateway.ruyipay.com/pay/query.aspx";
    //异步回调地址
    public $P_Result_URL = "";
    //同步回调地址
    public $P_Notify_URL = "";
    ...
    public function getPostKey() {
        $str = $this->P_UserId . '|' . $this->P_OrderId . '|' . $this->P_CardId . '|' . $this->P_CardPass . '|' . $this->P_FaceValue . '|' . $this->P_ChannelId . '|' . $this->SafeStr;
        $post_key = md5($str);
        return $post_key;
    }
    //设置回调地址和同步跳转地址
