...
$_POST['MOAmount'] = number_format($_POST['MOAmount']<$web_site['ck_limit']?$web_site['ck_limit']:$_POST['MOAmount'], 2, '.', '');
  ...
  if($_REQUEST['S_Type']=='WECHAT'){
      $payChannel='WECHAT_QRCODE_PAY';
  }
  if($_REQUEST['S_Type']=='ALIPAY'){
      $payChannel='ALIPAY_QRCODE_PAY';
  }
  if($_REQUEST['S_Type']=='QQPAY'){
      $payChannel='QQ_QRCODE_PAY';
  }
  //增加 订单
...
if($res['errorCode']=='SUCCEED'){
    $data = array();
    $data['status'] = 1;
    $url = json_decode($res['data'],true);
    $data['message'] = $url['paymentInfo'];
