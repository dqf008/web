...
class Pay {
    ...
    public function prepareSign($data) {
        ...
        if ($data['service'] == 'TRADE.B2C') {
            ...
            $result = sprintf("service=%s&version=%s&merId=%s&tradeNo=%s&tradeDate=%s&amount=%s&notifyUrl=%s&extra=%s&summary=%s&bankCardNo=%s&bankCardName=%s&bankId=%s&bankName=%s&purpose=%s", $data['service'], $data['version'], $data['merId'], $data['tradeNo'], $data['tradeDate'], $data['amount'], $data['notifyUrl'], $data['extra'], $data['summary'], $data['bankCardNo'], $data['bankCardName'], $data['bankId'], $data['bankName'], $data['purpose']);
            
            return $result;
            
            //6单笔委托结算查询
        } else if ($data['service'] == 'TRADE.SETTLE.QUERY') {
            $result = sprintf("service=%s&version=%s&merId=%s&tradeNo=%s&tradeDate=%s", $data['service'], $data['version'], $data['merId'], $data['tradeNo'], $data['tradeDate']);
            return $result;
            //7回调
