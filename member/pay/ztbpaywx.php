...
$payType = array(1 => '支付宝', 2 => '微信', 3 => 'QQ钱包');
  ...
  <head> 
	  ...
	  <script language="JAVAScript"> 
		...
		function VerifyData(payType) {
			...
			tb_show("请使用"+payType1+"客户端扫码","#TB_inline?width=320&height=320&inlineId=info",false);
			$("#qrimg").html("<img src=\"ztbpaywx/loading.gif\" style=\"padding-top:130px\" /><br />二维码加载中，请稍后...");
			$.ajax({
  				url: "<?php echo $advice_url; ?>",
   				type: "get",
  				dataType: "jsonp",
  				jsonp: "callback",
   				data: {"uid":"<?=$rows['uid'];?>", "amount":paymoney, "type":payType, "pay_online":"<?=$pay_online;?>"},
   				success: function(data){
   					if(data['status']=="error"){
   						alert(data['message']);
   						tb_remove();
   					}else{
   						$("#qrimg").html("<img src=\"qrcode.php?s="+encodeURIComponent(data['message'])+"\" height=\"300\" />");
   					}
   				}
			});
			return false;
						  ...
						  <td align="left" bgcolor="#FFFFFF" style="line-height:22px;"> 
							  <table width="100%" border="0" cellspacing="0" cellpadding="5"> 
								  ...
								  <tr> 
									  ...
									  <td align="left"><input name="SubTran" type="button" class="submit_108" id="SubTran" value="<?php echo $payType[$type]; ?>扫码" onclick="VerifyData(<?php echo $type; ?>);" /></td> 
								  </tr> 
								  </form> 
							  </table> 
						  </td> 
