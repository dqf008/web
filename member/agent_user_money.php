		  ...
		  <?php include_once 'agentmenu.php';?>
            <div class="content">
                  <table width="98%" border="0" cellspacing="0" cellpadding="5"> 
                      ...
                      <tr> 
                          <td height="25" colspan="7" align="center" bgcolor="#3C3C3C" style="color:#FFF"><?php $subsub=1;include_once 'agent_user_moneysubmenu.php';?>                        </td> 
                      </tr> 
                      <tr> 
                      ...
                      <tr> 
                          ...
                          <th align="center">存款流水号</th> 
                          <th align="center">存款金额</th> 
                          <th align="center">手续费</th> 
                          <th align="center">存款状态</th> 
                      </tr>
                      <?php 
                        $params = array(':uid' => $uid, ':begin_time' => $begin_time, ':end_time' => $end_time);
                        $sql = 'select `m`.`m_id` from `k_money` as `m` left join `k_user` as `u` on `u`.`uid`=`m`.`uid` where `u`.`top_uid`=:uid and `m`.`type`=1 and `m`.`m_make_time`>=:begin_time and `m`.`m_make_time`<=:end_time';
                        if(!empty($selectuser)){
                            $params[':username'] = $selectuser;
                            $sql.= ' and u.username=:username';
                        }
                        $sql.= ' order by `m`.`m_id` desc';
                        ...
                        if ($id){
                            $id = rtrim($id, ',');
                            $sql    =   "select m.*,u.username from k_money m left join k_user u on u.uid=m.uid where m_id in($id) order by m_id desc";
                    ...
                    <tr>
                        <th colspan="7" align="center">
                            <div class="Pagination">
                                本页存款成功总金额：<font color="#ff0000"><?=sprintf("%.2f",$sum_money)?></font> RMB&nbsp;&nbsp;手续费(公司支付)：<font color="#ff0000"><?=sprintf("%.2f",$sum_sxf)?></font> RMB&nbsp;&nbsp;
                                <?=$page->get_htmlPage("agent_user_money.php?cn_begin=".$cn_begin."&s_begin_h=".$s_begin_h."&s_begin_i=".$s_begin_i."&cn_end=".$cn_end."&s_end_h=".$s_end_h."&s_end_i=".$s_end_i."&selectuser=".$selectuser);?>
                            </div>
                        </th>
                    </tr> 
                  </table> 
