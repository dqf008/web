			...
			<div class="content">
				...
				<?php 
					$params = array(':uid' => $uid, ':begin_time' => $begin_time, ':end_time' => $end_time);
                        $sql = 'select `m`.`m_id` from `k_money` as `m` left join `k_user` as `u` on `u`.`uid`=`m`.`uid` where `u`.`top_uid`=:uid and `m`.`type`=2 and `m`.`m_make_time`>=:begin_time and `m`.`m_make_time`<=:end_time';
                        if(!empty($selectuser)){
					...
					<tr>
						<th colspan="8" align="center">
							<div class="Pagination">
								...
								<?=$page->get_htmlPage("agent_user_t_money.php?cn_begin=".$cn_begin."&s_begin_h=".$s_begin_h."&s_begin_i=".$s_begin_i."&cn_end=".$cn_end."&s_end_h=".$s_end_h."&s_end_i=".$s_end_i."&selectuser=".$selectuser);?>
							</div>
						</th>
					</tr>
				</table>
