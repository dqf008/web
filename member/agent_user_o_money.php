		...
		<td colspan="2" align="center" valign="middle">
			...
			<div class="content">
				<table width="98%" border="0" cellspacing="0" cellpadding="5">
					...
					<tr>
						<td height="25" colspan="8" align="center" bgcolor="#3C3C3C" style="color:#FFF">
							<?php 
					...
					$sql = 'select `m`.`m_id` from `k_money` as `m` left join `k_user` as `u` on `u`.`uid`=`m`.`uid` where `u`.`top_uid`=:uid and `m`.type in(' . $m_type . ') and `m`.`m_make_time`>=:begin_time and `m`.`m_make_time`<=:end_time';
                        if(!empty($selectuser)){
					...
					<tr>
						<th colspan="8" align="center">
							<div class="Pagination">
								...
								<?=$page->get_htmlPage("agent_user_o_money.php?cn_begin=".$cn_begin."&s_begin_h=".$s_begin_h."&s_begin_i=".$s_begin_i."&cn_end=".$cn_end."&s_end_h=".$s_end_h."&s_end_i=".$s_end_i."&m_type=".$m_type."&selectuser=".$selectuser);?>
							</div>
						</th>
					</tr>
				</table>
