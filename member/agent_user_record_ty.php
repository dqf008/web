	...
	<tr>
		<td colspan="2" align="center" valign="middle">
			...
			<table width="98%" border="0" cellspacing="0" cellpadding="5"> 
				...
				$sql = 'select k.bid from k_bet k left join k_user u on u.uid=k.uid where u.top_uid=:uid and k.status in (0,1,2,3,4,5,6,7,8) and k.bet_time>=:begin_time and k.bet_time<=:end_time';
                if(!empty($selectuser)){
				...
				{
					$id = rtrim($id, ',');
					$sql	=	"select k.*,u.username from k_bet k left join k_user u on u.uid=k.uid where k.bid in($id) order by k.bid desc";
			   ...
			   <tr bgcolor="<?=$i%2==0?'#FFFFFF':'#F5F5F5'?>" align="center" onMouseOver="this.style.backgroundColor='#FFFFCC'" onMouseOut="this.style.backgroundColor='<?=$i%2==0?'#FFFFFF':'#F5F5F5'?>'" >
					...
					<?php
							...
							$params = array(':uid' => $uid, ':begin_time' => $begin_time, ':end_time' => $end_time);
							$sql = 'select k.gid from k_bet_cg_group as k left join k_user as u on u.uid=k.uid where k.status in (0,1,2,3,4) and u.top_uid=:uid and k.bet_time>=:begin_time and k.bet_time<=:end_time';
			                if(!empty($selectuser)){
			                    $params[':username'] = $selectuser;
			                    $sql.= ' and u.username=:username';
			                }
							...
							{
								$id = rtrim($id, ',');
								$sql	=	"select u.username, k.* from k_bet_cg_group as k left join k_user as u on u.uid=k.uid where k.gid in($id) order by k.gid desc";
								$query = $mydata1_db->query($sql);
								$arr_b_cg = array();
								while ($rows = $query->fetch())
								{
									$arr_b_cg[$rows['gid']]['username'] = $rows['username'];
					...
					<tr>
						<th colspan="9" align="center">
							<div class="Pagination">
								...
								<?=$page->get_htmlPage("agent_user_record_ty.php?cn_begin=".$cn_begin."&s_begin_h=".$s_begin_h."&s_begin_i=".$s_begin_i."&cn_end=".$cn_end."&s_end_h=".$s_end_h."&s_end_i=".$s_end_i."&atype=".$atype."&selectuser=".$selectuser);?>
							</div>
					  </th> 
				  </tr> 
			  </table> 
