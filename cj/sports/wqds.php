...
$arr = $client->wqds($site_id,$reTime);
...
if(is_array($arr) and $arr){
	$count = sizeof($arr);
	foreach ($arr as $row) {
		$exist_row = $db->row('select id from tennis_match where Match_ID=:Match_ID',['Match_ID' => $row['Match_ID']]);
		$params = array();
		$sql = '';
		$commonsql = '';
		$row_i = 0;
		...
		if ($exist_row){
			...
			$sql = 'update tennis_match set ' . $commonsql . ' where Match_ID=:Match_ID2 and Match_JS!=1';
		}else{
			$sql = 'insert into tennis_match set ' . $commonsql;
		}
		$db->query($sql,$params);
	}
	$db->CloseConnection();
