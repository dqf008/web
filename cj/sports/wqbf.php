...
$sql = 'select Match_ID from tennis_match where  Match_CoverDate<now() and (Match_Date=\''.$day.'\' or Match_Date=\''.$fast.'\') and  MB_Inball is null';
	...
	if(is_array($arr) and $arr){
		foreach ($arr as $row) {
			...
			if((string)$row['MB_Inball_HR'] != 'nnnn'){
				...
				$sql = 'update tennis_match set Match_LstTime = now() ' . $commonsql . ' where Match_ID=:Match_ID';
				$db->query($sql,$params);
				$count++;
			}
		}
