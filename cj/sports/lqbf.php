...
$sql = 'select Match_ID from lq_match where  Match_CoverDate<now() and (Match_Date=\''.$day.'\' or Match_Date=\''.$fast.'\') and  (MB_Inball< MB_Inball_HR or TG_Inball< TG_Inball_HR or MB_Inball is null or TG_Inball_HR is null) ';
$db = new DB(DB::DB4);
$dt = $db->column($sql);
if($dt){
	$client = new rpcclient($cj_url);
	$arr = $client->lqbf($site_id,$dt);
	...
	if(is_array($arr) and $arr){
		foreach ($arr as $row) {
			...
			$sql = 'update lq_Match set Match_LstTime = now() ' . $commonsql . ' where Match_ID=:Match_ID';
			$db->query($sql,$params);
			$count++;
		}
	}else if(!$arr){
