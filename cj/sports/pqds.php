...
$arr = $client->pqds($site_id,$reTime);
...
if(is_array($arr) and $arr){
	...
	foreach ($arr as $row) {
		...
		$exist_row = $db->row('select id from volleyball_match where Match_ID=:Match_ID',['Match_ID' => $row['Match_ID']]);
		$params = array();
		$sql = '';
		$commonsql = '';
		$row_i = 0;
		...
		if ($exist_row){
			...
			$sql = 'update volleyball_match set ' . $commonsql . ' where Match_ID=:Match_ID2 and Match_JS!=1';
		}else{
			$sql = 'insert into volleyball_match set ' . $commonsql;
		}
		$db->query($sql,$params);
	}
	$db->CloseConnection();
