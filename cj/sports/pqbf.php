...
$sql = 'select Match_ID from volleyball_match where  Match_CoverDate<now() and (Match_Date="'.$day.'" or Match_Date="'.$fast.'") and  MB_Inball is null';
$db = new DB(DB::DB4);
$dt = $db->column($sql);
if($dt){
	...
	if(is_array($arr) and $arr){
		foreach ($arr as $row) {
			...
			if((string)$row['MB_Inball_HR'] != 'nnnn'){
				...
				$sql = 'update volleyball_match set Match_LstTime = now() ' . $commonsql . ' where Match_ID=:Match_ID';
				$db->query($sql);
				$count++;
			}
		}
