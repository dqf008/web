...
while ($rows = $stmt->fetch()) {
    if(!isset($info['ok'][$rows['qishu']])&&!in_array($rows['qishu'], $info['no'])){
        ...
        $stmt1->execute([':qishu' => $rows['qishu'], ':type' => 'JSSC', ':time' => $now]);
    ...
    if(!in_array($rows['qishu'], $info['no'])){
        ...
        // var_dump($rows['value']);
        switch ($rows['value']['class'][1]) {
            case '冠亚大':
            case '冠亚小':
            case '冠亚单':
            case '冠亚双':
                if($temp['info'][0]==11){
                    $info['bet_ok'][$rows['id']]['money'] = $rows['money'];
                }else if(in_array(str_replace('冠亚', '', $rows['value']['class'][1]), [$temp['info'][1], $temp['info'][2]])){
            ...
            default:
                switch ($rows['value']['class'][0]) {
                    ...
                    case '第五名':
                        ...
                        break;
                    case '第六名':
                        $temp = $temp['opencode'][5];
                        break;
                    case '第七名':
                        $temp = $temp['opencode'][6];
                        break;
                    case '第八名':
                        $temp = $temp['opencode'][7];
                        break;
                    case '第九名':
                        $temp = $temp['opencode'][8];
                        break;
                    case '第十名':
                        $temp = $temp['opencode'][9];
                        break;
                }
...
$stmt->execute([':type' => 'JSSC', ':time' => $now]);
while ($rows = $stmt->fetch()) {
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
    <tr>
        <td align="left" style="padding-left:10px; padding-top:10px; line-height:22px;">
            ...
            <font color="#FF0000">极速赛车共结算<?=count($info['bet_ok'])?>个注单</font></td>
    </tr>
</table>
</body>
</html>
