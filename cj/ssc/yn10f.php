...
$cache_file    = '../../cache/lot_ynkl10.php';
$lotteryName ='ynkl10';
...
if (is_array($arr) and $arr) {
    foreach (array_reverse($arr, true) as $key => $v) {
        ...
        if (strlen($qishu) > 0 && is_numeric($num1) && is_numeric($num2) && is_numeric($num3) && is_numeric($num4) && is_numeric($num5) && is_numeric($num6) && is_numeric($num7) && is_numeric($num8)) {
            $params = [':qishu' => $qishu, ':name' => 'ynkl10', ':datetime' => $datetime, ':ball_1' => $num1, ':ball_2' => $num2, ':ball_3' => $num3, ':ball_4' => $num4, ':ball_5' => $num5, ':ball_6' => $num6, ':ball_7' => $num7, ':ball_8' => $num8, ':ok' => 1];
