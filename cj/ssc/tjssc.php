...
$cache_file    = '../../cache/lot_tjssc.php';
...
if (is_array($arr) and $arr) {
    foreach (array_reverse($arr, true) as $key => $v) {
        ...
        if (strlen($qishu) > 0 && is_numeric($num1) && is_numeric($num2) && is_numeric($num3) && is_numeric($num4) && is_numeric($num5)) {
            ...
            $sql  = 'insert into c_auto_tjssc(qishu,datetime,ball_1,ball_2,ball_3,ball_4,ball_5,ok) values (:qishu,:datetime,:ball_1,:ball_2,:ball_3,:ball_4,:ball_5,:ok) ON DUPLICATE KEY UPDATE ok=values(ok)';
