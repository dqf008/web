...
$cache_file = '../../cache/lot_shssl.php';
...
if(is_array($arr) and $arr){
	foreach(array_reverse($arr,true) as $key=>$v){
		...
		if(strlen($qishu)>0 && is_numeric($num1) &&  is_numeric($num2)){
			$params = array(':qishu' => $qishu, ':datetime' => $datetime, ':ball_1' => $num1, ':ball_2' => $num2, ':ball_3' => $num3,':ok'=>1);
			$sql = 'insert into `lottery_k_ssl`(qihao,addtime,hm1,hm2,hm3,ok) values (:qishu,:datetime,:ball_1,:ball_2,:ball_3,:ok) ON DUPLICATE KEY UPDATE addtime=values(addtime),hm1=values(hm1),hm2=values(hm2),hm3=values(hm3),ok=values(ok)';
            ...
            if($lot_changlong['lastest']<$qishu){
                ...
                foreach(array('和', '大小', '单双') as $i){
                    zoushi('和值', $i, $lot_t[$i], $lot_changlong['date']!=substr($qishu, 0, 8));
                }
