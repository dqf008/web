...
$cache_file = '../../cache/lot_cqssc.php';
...
if(is_array($arr) and $arr){
	foreach(array_reverse($arr,true) as $key=>$v){
		...
		if(strlen($qishu)>0 && is_numeric($num1) &&  is_numeric($num2) && is_numeric($num3) && is_numeric($num4) && is_numeric($num5)){
			...
			$params = array(':qishu' => $qishu, ':datetime' => $datetime, ':ball_1' => $num1, ':ball_2' => $num2, ':ball_3' => $num3, ':ball_4' => $num4, ':ball_5' => $num5,':ok'=>1);
			
				$sql = 'insert into c_auto_2(qishu,datetime,ball_1,ball_2,ball_3,ball_4,ball_5,ok) values (:qishu,:datetime,:ball_1,:ball_2,:ball_3,:ball_4,:ball_5,:ok) ON DUPLICATE KEY UPDATE datetime=values(datetime),ball_1=values(ball_1),ball_2=values(ball_2),ball_3=values(ball_3),ball_4=values(ball_4),ball_5=values(ball_5),ok=values(ok)';
