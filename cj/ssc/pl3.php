...
if(is_array($arr) and $arr){
	foreach(array_reverse($arr,true) as $key=>$v){
		...
		if(strlen($qihao)>0 && is_numeric($num1)  && is_numeric($num2) && is_numeric($num3)){
			...
			$sql = 'insert into lottery_k_pl3(qihao,kaipan,fengpan,hm1,hm2,hm3,ok)values(:qihao,:kaipan,:fengpan,:ball_1,:ball_2,:ball_3,:ok) ON DUPLICATE KEY UPDATE kaipan=values(kaipan),fengpan=values(fengpan),hm1=values(hm1),hm2=values(hm2),hm3=values(hm3),ok=values(ok)';
