...
$cache_file = '../../cache/lot_gdkl10.php';
...
if(is_array($arr) and $arr){
    foreach(array_reverse($arr,true) as $key=>$v){
        ...
        if(strlen($qishu)>0 && is_numeric($num1)  && is_numeric($num2) && is_numeric($num3) && is_numeric($num4) && is_numeric($num5) && is_numeric($num6) && is_numeric($num7) && is_numeric($num8)){
            $params = array(':qishu' => $qishu, ':datetime' => $datetime, ':ball_1' => $num1, ':ball_2' => $num2, ':ball_3' => $num3, ':ball_4' => $num4, ':ball_5' => $num5, ':ball_6' => $num6, ':ball_7' => $num7, ':ball_8' => $num8,':ok'=>1);

            $sql = 'insert into c_auto_3(qishu,datetime,ball_1,ball_2,ball_3,ball_4,ball_5,ball_6,ball_7,ball_8,ok) values (:qishu,:datetime,:ball_1,:ball_2,:ball_3,:ball_4,:ball_5,:ball_6,:ball_7,:ball_8,:ok) ON DUPLICATE KEY UPDATE datetime=values(datetime),ball_1=values(ball_1),ball_2=values(ball_2),ball_3=values(ball_3),ball_4=values(ball_4),ball_5=values(ball_5),ball_6=values(ball_6),ball_7=values(ball_7),ball_8=values(ball_8),ok=values(ok)';
            ...
            if($lot_changlong['lastest']<$qishu){
                ...
                /* 1~8球 */
                $keys = array('第一球', '第二球', '第三球', '第四球', '第五球', '第六球', '第七球', '第八球');
                $fangwei = array('东', '南', '西', '北');
                foreach($tempNum as $k=>$n){
                    ...
                    $k = $keys[$k];
                    foreach(array('尾大' => '尾数', '尾小' => '尾数', '大' => '大小', '小' => '大小', '单' => '单双', '双' => '单双', '合双' => '合数', '合单' => '合数') as $i=>$j){
                    ...
                    foreach(array('大小', '单双', '合数', '尾数', '方位', '中发白') as $i){
                ...
                foreach(array('总和大小' => array('大', '小', '和'), '总和单双' => array('单', '双'), '总和尾数' => array('大', '小')) as $i=>$j){
                ...
                foreach($keys as $j=>$key){
                    foreach(array('大小', '单双', '合数', '尾数', '方位', '中发白') as $i){
                        $lot_changlong['output']['LuZhu'][] = array(
                            'c' => str_replace(array('合', '尾'), array('', ''), implode(',', $lot_changlong[$key][$i])),
                            'n' => str_replace(array('合数', '尾数'), array('合数单双', '尾数大小'), $i),
                            'p' => $j+1,
                        );
                    }
                }
                foreach(array('总和大小', '总和单双', '总和尾数') as $key){
                    $lot_changlong['output']['LuZhu'][] = array(
