...
$mDate = date('m-d', strtotime("-1 day"));
$mDate1 = date('m-d');
...
try
{
	...
	$sql = 'select MB_Inball,TG_Inball,MB_Inball_HR,TG_Inball_HR,Match_Master,match_name,Match_Guest,Match_ID from mydata4_db.bet_match where (`Match_Date` =:Match_Date or `Match_Date` =:Match_Date1) and (MB_Inball>=0 or (MB_Inball=\'-1\' and TG_Inball=\'-1\')) and match_js!=\'1\' order by ID';
	...
	while ($rows = $stmt->fetch()){
		...
		$sqlSub = 'update k_bet set MB_Inball=:MB_Inball,TG_Inball=:TG_Inball where lose_ok=1 and status=0 and match_id=:match_id and not (ball_sort like(\'%上半场%\') or bet_info like(\'%上半场%\'))';
		$stmtSub = $mydata1_db->prepare($sqlSub);
		$stmtSub->execute($paramsSub);
		$paramsSub = array(':MB_Inball' => $MB_Inball, ':TG_Inball' => $TG_Inball, ':match_id' => $rows['Match_ID']);
		$sqlSub = 'update k_bet_cg set mb_inball=:MB_Inball,tg_inball=:TG_Inball where status=0 and match_id=:match_id and not (ball_sort like(\'%上半场%\') or bet_info like(\'%上半场%\'))';
		$stmtSub = $mydata1_db->prepare($sqlSub);
		$stmtSub->execute($paramsSub);
	}
	$mid = implode(',', $mids);
	...
	if (0 < count($mids)){
		...
		while ($rows = $result->fetch()){
			...
			$hr_params = array(':match_id' => $match_id);
			$hr_sql = 'select MB_Inball_HR,TG_Inball_HR from mydata4_db.bet_match where Match_ID=:match_id';
			$hr_stmt = $mydata1_db->prepare($hr_sql);
			$hr_stmt->execute($hr_params);
			$hr_rows = $hr_stmt->fetch();
			if ($hr_rows){
		...
		if ($bool){
			$mydata1_db->query('update mydata4_db.bet_match set match_js=\'1\',match_sbjs=\'1\' where match_id in(' . $mid . ')');
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr> 
    <td align="left" style="padding-left:10px; padding-top:10px; line-height:22px;">
	   ...
	   采集网址：<?=$url?><br />
       <font color="#FF0000">接收<?=$m;?>条足球结算！</font> <br />
      </td>
  </tr>
</table>
</body>
