...
try
{
	...
	$sql = 'select MB_Inball,TG_Inball,MB_Inball_HR,TG_Inball_HR,Match_Master,match_name,Match_Guest,Match_ID from mydata4_db.lq_match where (`Match_Date` =:Match_Date or `Match_Date` =:Match_Date1) and (MB_Inball>=0 or (MB_Inball=\'-1\' and TG_Inball=\'-1\')) and match_js!=\'1\' order by ID';
	...
	if (0 < count($mids)){
		...
		while ($rows = $result->fetch()){
			...
			$t = make_point($column, $rows['MB_Inball'], $rows['TG_Inball'], $MB_Inball_HR, $TG_Inball_HR, $rows['match_type'], $rows['match_showtype'], $rows['match_rgg'], $rows['match_dxgg'], '0:0');
			$bid = intval($rows['bid']);
		...
		while ($rows = $result_cg->fetch()){
			$all_bet_money += $rows['bet_money'];
			$column = $rows['point_column'];
			$t = make_point(strtolower($column), $rows['MB_Inball'], $rows['TG_Inball'], $MB_Inball_HR, $TG_Inball_HR, $rows['match_type'], $rows['match_showtype'], $rows['match_rgg'], $rows['match_dxgg'],'0:0');//$rows['match_nowscore']
			$bid = intval($rows['bid']);
			$status = intval($t['status']);
			$mb_inball = $rows['MB_Inball'];
			$tg_inball = $rows['TG_Inball'];
		...
		if ($bool){
			$mydata1_db->query('update mydata4_db.lq_match set match_js=\'1\' where match_id in(' . $mid . ')');
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr> 
    <td align="left" style="padding-left:10px; padding-top:10px; line-height:22px;">
	   ...
	   采集网址：<?=$url?><br />
       <font color="#FF0000">接收<?=$m;?>条篮球结算！</font> <br />
      </td>
  </tr>
</table>
</body>
