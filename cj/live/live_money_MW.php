...
if(!empty($uid)){
  ...
  if(is_array($arrUrl) and $arrUrl){
    ...
    if($arrUrl['info'] == 'ok'){
      ...
      $data['name'] = $userinfo['mwUserName'];
      $params = array();
      $params[':uid']=$userinfo['uid'];
      $params[':mwmoney'] = $data['msg'];
      $sql = 'update  mydata1_db.k_user set mwmoney=:mwmoney,mwAddtime=now() where ismw=1 and uid=:uid';
      $stmt = $mydata1_db->prepare($sql);
      $stmt -> execute($params);
      echo $callback.'('.json_encode($data).')';
      exit;
