...
if (is_array($arr) and $arr) {
    foreach (array_reverse($arr, true) as $key => $v) {
        ...
        $addtime    = $v['dateline'];
        $ok =1;
        if (strlen($qihao) > 0 && !empty($kaipan) && !empty($fengpan)) {
            $params = [':qihao' => $qihao, ':kaipan' => $kaipan, ':fengpan' => $fengpan, ':balls' => $balls, ':ok' => $ok, ':addtime' => $addtime, ':lottery_name' => $lotteryName];
            $sql  = 'insert into lottery_k3 (qihao,kaipan,fengpan,balls,ok,addtime,name) values (:qihao,:kaipan,:fengpan,:balls,:ok,:addtime,:lottery_name) ON DUPLICATE KEY UPDATE balls =values(balls),ok=values(ok)';
