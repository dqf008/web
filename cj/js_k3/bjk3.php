...
$params = array(':name'=>$atype);
$sql = 'select id,qihao,balls from lottery_k3 where name=:name and ok=0 order BY end_sale desc ';
$stmta = $mydata1_db->prepare($sql);
$stmta->execute($params);
while($rsq = $stmta->fetch()){
    $qi = $rsq['qihao'];
    $balls = explode(',',$rsq['balls']);
    asort($balls);
    $sum   = array_sum($balls);
...
function set_draw($id){
    global $mydata1_db;
    $params = array(':id' => $id);
    $msql = 'update `lottery_k3` set ok=1 where id=:id';
    $stmt = $mydata1_db->prepare($msql);
    $stmt->execute($params) || exit('标记已开奖失败' . $id);
}
