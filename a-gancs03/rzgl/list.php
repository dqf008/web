...
<?php 
$admin = array();
$sql = 'select uid,login_name from mydata3_db.sys_admin order by uid desc';
$query = $mydata1_db->query($sql);
while ($rows = $query->fetch()){
$admin[$rows['uid']] = $rows['login_name'];
?>          
		<option value="<?=$rows['uid'];?>"<?=$_GET['show']<$rows['uid'] ? 'selected="selected"' : '';?>><?=$rows['login_name'];?></option><?php }?>         </select></td> 
...
<?php 
$params = array();
$sql = 'select log_id from mydata3_db.sys_log where log_id>0';
if ($_GET['show'] == 'all'){

}else if (0 < $_GET['show']){
	$params[':uid'] = intval($_GET['show']);
	$sql .= ' and uid=:uid';
}else{
	$params[':uid'] = intval($_SESSION['adminid']);
	$sql .= ' and uid=:uid';
}

if ($_GET['ip']){
	$params[':log_ip'] = $_GET['ip'];
	$sql .= ' and log_ip=:log_ip';
}

if ($_GET['info']){
	$params[':log_info'] = '%' . $_GET['info'] . '%';
	$sql .= ' and log_info like(:log_info)';
}
...
if ($id){
	$id = rtrim($id, ',');
	$sql = 'select * from mydata3_db.sys_log where log_id in(' . $id . ') order by log_id desc';
	$query = $mydata1_db->query($sql);
	while ($rows = $query->fetch()){
?> 	          
