...
$sql = 'select * from c_auto_xjssc where qishu=:qishu order by id desc limit 1';
...
$sqlMain = 'select * from c_bet where type=\'新疆时时彩\' and js=0 and qishu=:qishu order by addtime asc';
...
$params = array(':qishu' => $qi);
$msql = 'update c_auto_xjssc set ok=1 where qishu=:qishu';
$stmt = $mydata1_db->prepare($msql);
$stmt->execute($params) || exit('期数修改失败!!!');
if ($_GET['t'] == 1)
{?> <script>window.location.href='../../Lottery/auto_xjssc.php';</script><?php }
