...
$mysql = 'select * from lottery_k_3d where qihao=:qihao';
...
$sqlMain = 'select * from lottery_data where atype=\'3d\' and mid=:qihao and bet_ok=0 order by ID asc';
...
while ($row = $stmtMain->fetch())
{
	...
	if ($row['btype'] == '组三')
	{
		...
		if ($z3z6 == '组三')
		{
			if (($row['content'] == $bw . $sw . $gw) || ($row['content'] == $bw . $gw . $sw) || ($row['content'] == $sw . $bw . $gw) || ($row['content'] == $sw . $gw . $bw) || ($row['content'] == $gw . $bw . $sw) || ($row['content'] == $gw . $sw . $bw))
			{
