...
try
{
	...
	$sql = 'select MB_Inball,TG_Inball,MB_Inball_HR,TG_Inball_HR,Match_Master,match_name,Match_Guest,Match_ID from mydata4_db.lq_match where `Match_Date` >=:Match_Date and `Match_Date` <=:Match_Date1 and (MB_Inball>=0 or (MB_Inball=\'-1\' and TG_Inball=\'-1\')) and match_js!=\'1\' order by ID';
	...
	if (0 < count($mids)){
		...
		while ($rows = $result->fetch()){
			...
			$t = make_point($column, $rows['MB_Inball'], $rows['TG_Inball'], $MB_Inball_HR, $TG_Inball_HR, $rows['match_type'], $rows['match_showtype'], $rows['match_rgg'], $rows['match_dxgg'], '');
			$bid = intval($rows['bid']);
