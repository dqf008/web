            ...
            <form name="form1" method="get" action="<?=$_SERVER['REQUEST_URI'];?>">
                <tr>
                    <td align="left">
                        ...
                        &nbsp;<a href="MemberReport2.php?type=11" class="sel_btn <?php if($type == 11){ echo 'ch_cls';}?>">新MG电子</a>
                        &nbsp;<a href="MemberReport2.php?type=14" class="sel_btn <?php if($type == 14){ echo 'ch_cls';}?>">新PT电子</a>
                        &nbsp;<a href="MemberReport2.php?type=7" class="sel_btn <?php if($type == 7){ echo 'ch_cls';}?>">MW电子</a>
                        &nbsp;<a href="MemberReport2.php?type=9" class="sel_btn <?php if($type == 9){ echo 'ch_cls';}?>">AV女优</a>
                        &nbsp;<a href="MemberReport2.php?type=10" class="sel_btn <?php if($type == 10){ echo 'ch_cls';}?>">CQ9电子</a>
                        &nbsp;<a href="MemberReport2.php?type=12" class="sel_btn <?php if($type == 12){ echo 'ch_cls';}?>">BG捕鱼</a>
                        &nbsp;<a href="MemberReport2.php?type=13" class="sel_btn <?php if($type == 13){ echo 'ch_cls';}?>">申博电子(RT,LAX)</a>
                        &nbsp;<a href="MemberReport2.php?type=15" class="sel_btn <?php if($type == 15){ echo 'ch_cls';}?>">开元棋牌</a>
                        &nbsp;<a href="MemberReportHunter.php?type=5" class="sel_btn">AG捕鱼</a>
                        &nbsp;<a href="MemberReport.php?type=2" class="sel_btn old">MG电子</a>
                        &nbsp;<a href="MemberReport.php?type=3" class="sel_btn old">PT电子</a>
...
if($type == 11){
    ...
    $sql = 'SELECT A.playerName,count(A.id) AS orderNum,sum(A.netAmount) AS netAmount,sum(A.betAmount) AS betAmount,sum(A.validBetAmount) AS validBetAmount,0 AS donatefee,0 AS red_pocket,0 AS comp_enroll,0 AS comp_prize,B.username FROM ' . $table . ' A left outer join k_user B on A.uid=B.uid WHERE A.betTime >= :betTime1 AND A.betTime <= :betTime2 and platformtype="GAME" ' . $where . ' GROUP BY A.uid';
}else if($type == 12){
    $sql = 'SELECT A.playerName,count(A.id) AS orderNum,sum(A.netAmount) AS netAmount,sum(A.betAmount) AS betAmount,sum(A.validBetAmount) AS validBetAmount,0 AS donatefee,0 AS red_pocket,0 AS comp_enroll,0 AS comp_prize,B.username FROM ' . $table . ' A left outer join k_user B on A.uid=B.uid WHERE A.betTime >= :betTime1 AND A.betTime <= :betTime2 and platformtype="BGFISH" ' . $where . ' GROUP BY A.uid';
}else if($type == 13){
    $sql = 'SELECT A.playerName,count(A.id) AS orderNum,sum(A.netAmount) AS netAmount,sum(A.betAmount) AS betAmount,sum(A.validBetAmount) AS validBetAmount,0 AS donatefee,0 AS red_pocket,0 AS comp_enroll,0 AS comp_prize,B.username FROM ' . $table . ' A left outer join k_user B on A.uid=B.uid WHERE A.betTime >= :betTime1 AND A.betTime <= :betTime2 and platformtype<>"SB" ' . $where . ' GROUP BY A.uid';
}else{
    $sql = 'SELECT A.playerName,count(A.id) AS orderNum,sum(A.netAmount) AS netAmount,sum(A.betAmount) AS betAmount,sum(A.validBetAmount) AS validBetAmount,0 AS donatefee,0 AS red_pocket,0 AS comp_enroll,0 AS comp_prize,B.username FROM ' . $table . ' A left outer join k_user B on A.uid=B.uid WHERE A.betTime >= :betTime1 AND A.betTime <= :betTime2 ' . $where . ' GROUP BY A.uid';
}
