...
if (@($_GET['action']) == 'save'){
	$uid = intval($_GET['uid']);
	$params = array(':password' => md5($_POST['password']), ':sex' => $_POST['password'], ':uid' => $uid);
	$sql = 'update k_user set password=:password,sex=:sex where uid=:uid';
...
if (@($_GET['action']) == 'saveqk'){
	$uid = intval($_GET['uid']);
	$params = array(':qk_pwd' => md5($_POST['qk_pwd']), ':birthday' => $_POST['qk_pwd'], ':uid' => $uid);
	$sql = 'update k_user set qk_pwd=:qk_pwd,birthday=:birthday where uid=:uid';
	$stmt = $mydata1_db->prepare($sql);
	$stmt->execute($params);
	$q1 = $stmt->rowCount();
	if ($q1 == 1){
...
if (@($_GET['action']) == 'savedl'){
	$uid = intval($_GET['uid']);
	$params = array(':dl_pwd' => md5($_POST['dl_pwd']), ':uid' => $uid);
	$sql = 'update k_user set dl_pwd=:dl_pwd where uid=:uid';
	$stmt = $mydata1_db->prepare($sql);
	$stmt->execute($params);
	$q1 = $stmt->rowCount();
	if ($q1 == 1){
...
<?php 
$params = array(':uid' => intval($_POST['uid'][0]));
$sql = 'select uid,username,is_daili from k_user where uid=:uid limit 1';
$stmt = $mydata1_db->prepare($sql);
$stmt->execute($params);
$rows = $stmt->fetch();
?> 
...
<tr> 
  ...
  <td width="473"><input name="qk_pwd" type="password" id="qk_pwd" value="" maxlength="6"/> 
	  <font color="#FF0000">提款密码为6位数字</font></td> 
</tr> 
<tr> 
  <td bgcolor="#F0FFFF">确认密码</td> 
  <td><input  name="qk_pwd1" type="password" id="qk_pwd1" value="" maxlength="6"/></td> 
</tr> 
  <tr> 
  <td bgcolor="#F0FFFF">操作</td> 
  <td><input type="submit" value="提交"/></td> 
