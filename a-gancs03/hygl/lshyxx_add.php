...
if ($_GET['action'] == 'add'){
	...
	$stmt->execute($params);
	$row_uid = $stmt->fetchColumn();
	if (0 < $row_uid){
		$params = array(':uid' => $row_uid, ':username' => $username, ':pay_card' => $pay_card, ':pay_num' => $pay_num, ':pay_address' => $pay_address, ':pay_name' => $pay_name);
		...
		if ($stmt->rowCount() == 1){
			...
			admin::insert_log($_SESSION['adminid'], '新增了会员 ' . $row['uid'] . ' 历史银行卡信息 ' . $mydata1_db->lastInsertId());
