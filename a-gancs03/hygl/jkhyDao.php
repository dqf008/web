...
$zxhy = array();
$zxuid = '';
$arr_uid = explode(',', $jk_uid);

for ($i = 0;$i < count($arr_uid);$i++){
	$arr_uid[$i] = intval($arr_uid[$i]);
}

$jk_uid = implode(',', $arr_uid);
...
if ($zxuid){
	...
	if ($ds){
		$arr_ds = explode(',', $ds);
		for ($i = 0;$i < count($arr_ds);$i++)
		{
			$arr_ds[$i] = intval($arr_ds[$i]);
		}
		$ds = implode(',', $arr_ds);
		$ds = ' and bid not in(' . $ds . ')';
	}
	$ds_arr = array();
	...
	while ($rows = $query->fetch()){
		...
		if (strpos($rows['master_guest'], 'VS.')){
			$ds_arr[$i]['master_guest'] = str_replace('VS.', '<br/>', $rows['master_guest']);
		}else{
			$ds_arr[$i]['master_guest'] = str_replace('VS', '<br/>', $rows['master_guest']);
		}
		...
		{
			$ds_arr[$i]['bet_info'] = $rows['bet_info'];
		}else{
			$ds_arr[$i]['bet_info'] = str_replace('-', '<br>', $rows['bet_info']);
		}
		$i++;
	}
	...
	if($cg){
		...
		for ($i = 0;$i < count($arr_cg);$i++){
			$arr_cg[$i] = intval($arr_cg[$i]);
		}
		$cg = implode(',', $arr_cg);
		$cg = ' and gid not in(' . $cg . ')';
	}
	$cg_arr = array();
	...
	if ($gid){
		...
		foreach ($cg_arr as $k => $arr){
			...
			foreach ($arr_z as $bid => $rows){
				...
				if (0 < count($matches)){
					$str .= @($rows['match_time']) . $matches[0] . '<br/>';
				}
