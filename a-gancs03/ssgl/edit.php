...
if ($_GET['action'] == 'save'){
	...
	$params = array(':match_name' => $match_name, ':x_title' => $x_title, ':match_date' => $match_date, ':match_time' => $match_time, ':match_coverdate' => $match_coverdate, ':match_id' => $match_id, ':x_id' => $_GET['id']);
	$sql = 'update mydata4_db.t_guanjun set match_name=:match_name,x_title=:x_title,match_date=:match_date,match_time=:match_time,match_coverdate=:match_coverdate,match_id=:match_id where x_id=:x_id';
...
$params = array(':x_id' => $_GET['id']);
$sql = 'select * from mydata4_db.t_guanjun where x_id=:x_id';
$stmt = $mydata1_db->prepare($sql);
$stmt->execute($params);
$rows = $stmt->fetch();
?> 
