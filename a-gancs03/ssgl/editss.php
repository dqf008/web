...
if ($_GET['action'] == 'add'){
	...
	if (!in_array($table, $arr_db_table)){
		go('数据[表]查询失败，操作失败');
	}
	
	$params = array(':match_name' => $match_name, ':match_master' => $match_master, ':match_guest' => $match_guest, ':match_id' => $match_id, ':match_date' => $match_date, ':match_time' => $match_time, ':match_coverdate' => $match_coverdate, ':id' => $id);
	$sql = 'update mydata4_db.' . $table . ' set match_name=:match_name,match_master=:match_master,match_guest=:match_guest,match_id=:match_id,match_date=:match_date,match_time=:match_time,match_coverdate=:match_coverdate where id=:id';
...
$params = array(':id' => $id);
$sql = 'SELECT Match_ID, Match_Date, Match_Time, Match_Master, Match_Guest, Match_Name, match_coverdate FROM mydata4_db.' . $table . ' where ID=:id';
$stmt = $mydata1_db->prepare($sql);
$stmt->execute($params);
$rows = $stmt->fetch();
?> 
...
<tr> 
  <td height="24" nowrap background="../images/06.gif"><font >&nbsp;<span class="STYLE2">赛事管理：编辑赛事</span></font></td> 
</tr> 
<tr> 
