...
<?php 
	$params = array(':page_date' => $page_date, ':page_date2' => $page_date2);
	$sql = 'SELECT ID, Match_ID, Match_Date, Match_Time, Match_Master, Match_Guest,Match_Name FROM mydata4_db.' . $table . ' where (match_date=:page_date or match_date=:page_date2) order by Match_CoverDate,match_name,Match_Master desc';
...
function getName($n){
	switch ($n){
		case 'bet_match': return '足球';
		break;
		case 'lq_match': return '篮球';
		break;
		case 'tennis_match': return '网球';
		break;
		case 'volleyball_match': return '排球';
		break;
		case 'baseball_match': return '棒球';
	}
}
?>