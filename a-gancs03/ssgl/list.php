...
<?php 
$sql = 'select x_id from mydata4_db.t_guanjun where x_id>0';
$params = array();
$sqlwhere = '';
if (isset($_GET['type'])){
	if ($_GET['type'] < 3){
		$params[':match_type'] = $_GET['type'];
		$sqlwhere .= ' and match_type=:match_type';
	}
}

if (@($_GET['x_title'])){
	$params[':x_title'] = $_GET['x_title'];
	$sqlwhere .= ' and x_title=:x_title';
}

if (@($_GET['date'])){
	$params[':match_coverdate'] = $_GET['date'] . '%';
	$sqlwhere .= ' and match_coverdate like(:match_coverdate)';
}
$sql .= $sqlwhere;
...
if ($xid){
$xid = rtrim($xid, ',');
$sql = 'select * from mydata4_db.t_guanjun left join (select count(*) as num,xid from mydata4_db.t_guanjun_team group by xid) as t on t_guanjun.x_id=t.xid where x_id in(' . $xid . ') order by match_coverdate desc,x_id desc';
$stmt = $mydata1_db->prepare($sql);
$stmt->execute($params);
...
?> 	          
		   <tr align="center" onMouseOver="this.style.backgroundColor='#EBEBEB'" onMouseOut="this.style.backgroundColor='#ffffff'" style="background-color:#FFFFFF;"> 
			...
			<td><?php if ($rows['x_result'] == ''){ echo "暂无结果"; }else{?><font style="color:#FF0000"><?=$rows['x_result'];?></font><?php }?></td> 
			<td><?=$rows['add_time'];?></td> 
