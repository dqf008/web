...
if ($_GET['action'] == 'save'){
	...
	$id = rtrim($id, ',');
	$sql = 'update mydata4_db.bet_match set match_type=1 where id in (' . $id . ')';
...
<?php 
$params = array(':match_date' => date('m-d'), ':match_day' => date('Y-m-d'));
$sql = 'SELECT ID, Match_ID, Match_Date, Match_Time, Match_Master, Match_Guest, Match_Name, match_js FROM mydata4_db.Bet_Match where match_js=0 and (match_date=:match_date or match_date=:match_day) and Match_Type=0 order by  Match_CoverDate,iPage,match_name,Match_Master,iSn desc';
$stmt = $mydata1_db->prepare($sql);
$stmt->execute($params);
...
?>     
	...
	<td width="250"><?=$rows['Match_ID'];?><br/><?=$rows['Match_Name'];?></td> 
	<td width="80"><?=$rows['Match_Date'];?><br/><?=$rows['Match_Time'];?></td> 
