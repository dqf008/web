...
check_quanxian('ssgl');
if ($_GET['action'] == 'add'){
	$params = array(':team_name' => $_POST['team_name'], ':point' => floatval($_POST['point']), ':xid' => intval($_GET['id']));
	$sql = 'insert into mydata4_db.t_guanjun_team(team_name,point,xid) values(:team_name,:point,:xid)';
	$stmt = $mydata1_db->prepare($sql);
	$stmt->execute($params);
}
$params = array(':x_id' => intval($_GET['id']));
$sql = 'select * from mydata4_db.t_guanjun where x_id=:x_id limit 1';
$stmt = $mydata1_db->prepare($sql);
$stmt->execute($params);
$rows = $stmt->fetch();
if (!$rows){
...
<?php 
$params = array(':xid' => intval($_GET['id']));
$sql = 'select * from mydata4_db.t_guanjun_team where xid=:xid';
$stmt = $mydata1_db->prepare($sql);
$stmt->execute($params);
...
<tr align="center" onMouseOver="this.style.backgroundColor='#EBEBEB'" onMouseOut="this.style.backgroundColor='#ffffff'" style="background-color:#FFFFFF;"> 
  ...
  <td width="394" align="center"><a onClick="return confirm('确定删除上子项<?=$rows['team_name'];?>');" href="team_del.php?tid=<?=$rows['tid'];?>&type=2&xid=<?=$rows['xid'];?>">删除子项</a></td> 
</tr>
<?php }?>   
