...
$i = 0;
$temp1 = array();
$sql = 'select www from k_bet where bet_time>="'.$sdate.'" and bet_time<="'.$edate.'"';
$res = $mydata1_db->query($sql);
...
$sql = 'select www from k_bet_cg_group where bet_time>="'.$sdate.'" and bet_time<="'.$edate.'"';
$res = $mydata1_db->query($sql);
$list = $res->fetchAll(PDO::FETCH_ASSOC);
foreach ($list as $rows){
	$temp1[$rows['www']]++;
...
$sql = 'select "'.$conf_www.'" as www from lottery_data where bet_time>="'.$sdate.'" and bet_time<="'.$edate.'"';
$res = $mydata1_db->query($sql);
$list = $res->fetchAll(PDO::FETCH_ASSOC);
foreach ($list as $rows){
	$temp1[$rows['www']]++;
...
$params = array(':conf_www' => $conf_www, ':bet_time' => $ymd . '%');
$sql = 'select "'.$conf_www.'" as www from c_bet where addtime>="'.$sdate.'" and addtime<="'.$edate.'"';
$res = $mydata1_db->query($sql);
$list = $res->fetchAll(PDO::FETCH_ASSOC);
foreach ($list as $rows){
	$temp1[$rows['www']]++;
...
$params = array(':conf_www' => $conf_www, ':bet_time' => $ymd . '%');
$sql = 'select "'.$conf_www.'" as www from c_bet_3 where addtime>="'.$sdate.'" and addtime<="'.$edate.'"';
$res = $mydata1_db->query($sql);
$list = $res->fetchAll(PDO::FETCH_ASSOC);
foreach ($list as $rows){
	$temp1[$rows['www']]++;
...
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"> 
  ...
  <div style="text-align:center;height:250px;"><object width="550" height="250" align="middle" id="OrderGeneral" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,0,0" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"> 
            <param value="&amp;dataXML=&lt;graph caption='<?=$sum_today;?>条交易注单<?=substr($ymd, 5);?>' decimalPrecision='2' showPercentageValues='0' showNames='1' showValues='1' showPercentageInLabel='0' pieYScale='45' pieBorderAlpha='40' pieFillAlpha='70' pieSliceDepth='15' pieRadius='100' outCnvBaseFontSize='13' baseFontSize='12'&gt;
<?php 
$sum_zd	=	0;
foreach($temp as $i=>$k){
	if($i < 1){
