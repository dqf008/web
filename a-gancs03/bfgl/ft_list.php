...
$sql = 'select Match_ID,MB_Inball_HR,TG_Inball_HR from mydata4_db.bet_match where Match_ID in('.$mid.')';
$re = $mydata1_db->query($sql);
$m = array();
while ($rows = $re->fetch()){
...
<table   border="0" cellspacing="1" cellpadding="0"  bgcolor="006255" width="900" height="41"> 
  ...
  <?php 
	...
	while ($rows = $result->fetch()){
		...
		$color = '';
		if (($rows['MB_Inball'] == '') || ($rows['TG_Inball'] == '')){
		...
		<tr class="m_cen"<?=$color?>  align="center" onMouseOver="this.style.backgroundColor='#EBEBEB'" onMouseOut="this.style.backgroundColor='#ffffff'"> 
				...
				<td>
					<?=$rows["ball_sort"]?> - 单式<br/><?=$rows["match_time"]?>&nbsp;<font style="color:#FF0033"><?=str_replace("-","<br/>",$rows["bet_info"])?></font>[<?=$rows["MB_Inball"]?>:<?=$rows["TG_Inball"]?>]			
			  	</td>
			  ...
			  <?php 
				...
				{
					   case 1:  
						   $win=($t["ben_add"]+$rows["bet_point"])*$rows["bet_money"];
						   break;
					   case 2:  
						   $win=0;
						   break;
					   case 3:  
						   ...
						   break;
					   case 4:  
						   $win=(1+$rows["bet_point"]/2)*$rows["bet_money"]; 
						   break;
					   case 5:  
						   $win=$rows["bet_money"]/2;
						   break;
					   case 8:  
						   $win=$rows["bet_money"];
						   break;
			  ...
			  ?>               
				...
				<input type="hidden" name="tg_inball[]" value="<?=$t['tg_inball']?> " />      
			</td> 
	   ...
	   <?php 
		...
		?>     
		<tr class="m_cen"<?=$color?>  align="center" onMouseOver="this.style.backgroundColor='#EBEBEB'" onMouseOut="this.style.backgroundColor='#ffffff'"> 
			<td  align="center" ><a href="../zdgl/<?=$cg_php?>?gid=<?=$rows['gid']?> " target="_blank"><?=$rows['gid']?></a><br/><?=$rows['match_name'];?></td> 
			<td>
			<?php 
				if(strpos($rows["master_guest"],'VS.')){
					echo str_replace("VS.","<br/>",$rows["master_guest"]);
			...
			<td><?=$rows["ball_sort"]?> - 串关<br/><font style="color:#FF0033"><?=str_replace("-","<br/>",$rows["bet_info"])?></font>[<?=$rows["MB_Inball"]?>:<?=$rows["TG_Inball"]?>]</td> 
			<td><?=$rows['bet_money']?></td> 
			<td>
			<?php 
				$all_bet_money += $rows['bet_money'];
			...
			<input type="hidden" name="tg_inball_cg[]" value="<?=$t['tg_inball']?> " />      
		</td> 
		<td><?=substr($rows['bet_time'], 5)?> <br/><?=substr($rows['match_endtime'], 5)?> </td> 
