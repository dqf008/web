...
<table   border="0" cellspacing="1" cellpadding="0"  bgcolor="006255" width="900" height="41"> 
  ...
  <?php 
	...
	$sql = 'SELECT Match_ID,Match_Time,Match_Master,Match_Guest,Match_Name,MB_Inball,TG_Inball FROM mydata4_db.volleyball_match where match_js=1 and match_Date=:page_date order by  Match_CoverDate,match_name,Match_Master desc';
  ...
  <tr class="m_cen" align="center" onMouseOver="this.style.backgroundColor='#EBEBEB'" onMouseOut="this.style.backgroundColor='#FFFFFF'">  
   ...
   <td width="100"><? if($rows["MB_Inball"]<0){ echo "比赛无效"; }else {?> <?=$rows["MB_Inball"]?>：<?=$rows["TG_Inball"]?> <? }?></td> 
