...
if ($_GET['action'] == 're'){
	...
	$sql = 'update mydata4_db.' . $table . ' set match_js=0' . $set . ' where match_id in(' . $mid . ')';
...
$sql = 'select k_bet.*,k_user.username from k_bet left join k_user on k_bet.uid=k_user.uid where k_bet.lose_ok=1 and k_bet.status not in(0,6,7) and match_id in('.$mid.') order by  k_bet.bid desc';
