...
<table   border="0" cellspacing="1" cellpadding="0"  bgcolor="006255" width="900" height="41"> 
  ...
  <?php 
	...
	$sql = 'SELECT ID,Match_ID, Match_Date, Match_Time, Match_Master, Match_Guest,Match_Name,MB_Inball,TG_Inball,MB_Inball_HR,TG_Inball_HR,match_sbjs FROM mydata4_db.Bet_Match where match_js=0 and (match_date=:page_date or match_date=:page_date2) ' . $sqlwhere . ' order by Match_CoverDate,Match_Name,Match_Master,iPage,iSn desc';
	$stmt = $mydata1_db->prepare($sql);
	$stmt->execute($params);
	$arr_bet = array();
	while ($rows = $stmt->fetch()){
		if (0 < $rows['match_sbjs']){
			$bgcolor = '#FF9999';
		}else{
			$bgcolor = '#ffffff';
		}
	...
	<td width="190"><?=$rows['Match_ID']?> <br/><?=$rows['Match_Name']?></td> 
