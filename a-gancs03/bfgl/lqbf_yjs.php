...
<table width="1000"   height="41" border="0" cellpadding="0" cellspacing="1"  bgcolor="006255" id="glist_table"> 
  ...
  <?php 
	...
	if ($_GET['type'] != '0'){
		...
		$sqlwhere .= ' OR (Match_ID in (select match_id from mydata1_db.k_bet_cg where match_endtime>=:begin_time2)))';
	}
	
	$params[':page_date'] = $page_date;
	$params[':page_date2'] = $page_date2;
	$sql = 'select Match_ID,Match_Time,Match_Master, Match_Guest,Match_Name,MB_Inball_1st,TG_Inball_1st,MB_Inball_2st,TG_Inball_2st,MB_Inball_3st,TG_Inball_3st,MB_Inball_4st,TG_Inball_4st,MB_Inball_HR,' . "\t" . 'TG_Inball_HR,MB_Inball_ER,TG_Inball_ER,MB_Inball,TG_Inball,MB_Inball_Add,TG_Inball_Add ,MB_Inball_OK,TG_Inball_OK from mydata4_db.lq_match where match_js=1 and (match_Date=:page_date or match_date=:page_date2) ' . $sqlwhere . ' order by  Match_CoverDate,iPage,match_name,Match_Master,iSn desc';
  ...
  <tr class="m_cen" align="center" onMouseOver="this.style.backgroundColor='#EBEBEB'" onMouseOut="this.style.backgroundColor='#FFFFFF'">  
	<td width="160" height="18" align="middle"><?=$rows["Match_ID"]?> <br/> <?=$rows["Match_Name"]?></td>
    <td align="middle" width="50"><?=$rows["Match_Time"]?></td>
    <td align="left" width="160"><?=$rows["Match_Master"]?></td>
    <td align="middle" width="45"><?=$rows["MB_Inball"]>=0 ? $rows["MB_Inball"] : '<span style="color:#FF0000;">无效</span>'?><br /><?=$rows["TG_Inball"]>=0 ? $rows["TG_Inball"] : '<span style="color:#FF0000;">无效</span>'?></td>
    <td align="left" width="160"><?=$rows["Match_Guest"]?></td>
    <td align="middle" width="40"><?=$rows["MB_Inball_1st"]?><br/><?=$rows["TG_Inball_1st"]?></td>
    <td align="middle" width="40"><?=$rows["MB_Inball_2st"]?><br/><?=$rows["TG_Inball_2st"]?></td>
    <td align="middle" width="40"><?=$rows["MB_Inball_3st"]?><br/><?=$rows["TG_Inball_3st"]?></td>
    <td align="middle" width="40"><?=$rows["MB_Inball_4st"]?><br/><?=$rows["TG_Inball_4st"]?></td>
    <td align="middle" width="40"><?=$rows["MB_Inball_HR"]?><br/><?=$rows["TG_Inball_HR"]?></td>
    <td align="middle" width="40"><?=$rows["MB_Inball_ER"]?><br/><?=$rows["TG_Inball_ER"]?></td>
    <td align="middle" width="40"><? if ($rows["MB_Inball_Add"]>0) echo $rows["MB_Inball_Add"]; ?><br /><? if ($rows["TG_Inball_Add"]>0) echo $rows["TG_Inball_Add"];?></td>
