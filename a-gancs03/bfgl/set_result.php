...
check_quanxian('bfgl');
if ($_GET['action'] == 'add'){
	$params = array(':team_name' => $_POST['team_name'], ':point' => floatval($_POST['point']), ':xid' => $_GET['id']);
...
<tr> 
 ...
 <tr> 
  ...
  <?php 
  	$params = array(':xid' => $_GET['id']);
	$sql = 'select team_name,point,tid from mydata4_db.t_guanjun_team where xid=:xid';
	$stmt = $mydata1_db->prepare($sql);
	$stmt->execute($params);
