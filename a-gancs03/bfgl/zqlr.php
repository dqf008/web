...
if ($mid){
	...
	$sql = 'select Match_Master,match_name,Match_Guest from mydata4_db.bet_match where Match_ID=:Match_ID limit 1';
	...
	else
	{
		...
		$sql = 'update mydata4_db.bet_match set mb_inball=:MB_Inball,tg_inball=:TG_Inball where match_id in(' . $mid . ')';
		$stmt = $mydata1_db->prepare($sql);
		$stmt->execute($params);
