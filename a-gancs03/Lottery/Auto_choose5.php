...
$lotteryType = isset($_REQUEST['lottery_type'])?$_REQUEST['lottery_type']:'gdchoose5';
if (($_GET['action'] == 'add') && ($id == 0)){
    ...
    $params = array(':qishu' => $qishu, ':datetime' => $datetime, ':ball_1' => $ball_1, ':ball_2' => $ball_2, ':ball_3' => $ball_3, ':ball_4' => $ball_4, ':ball_5' => $ball_5,':name'=>$lotteryType);
    $sql = 'insert into c_auto_choose5(qishu,datetime,ball_1,ball_2,ball_3,ball_4,ball_5,name) values (:qishu,:datetime,:ball_1,:ball_2,:ball_3,:ball_4,:ball_5,:name)';
    ...
    $sql = 'update c_auto_choose5 set qishu=:qishu,datetime=:datetime,ball_1=:ball_1,ball_2=:ball_2,ball_3=:ball_3,ball_4=:ball_4,ball_5=:ball_5 where id=:id';
    $stmt = $mydata1_db->prepare($sql);
    $stmt->execute($params);
    echo "<script>alert('更新成功');</script>";
}else if (($_GET['action'] == 'delete') && (0 < $id)){
    $params = array(':id' => $id);
    $sql = 'delete from c_auto_choose5 where id=:id';
    $stmt = $mydata1_db->prepare($sql);
    $stmt->execute($params);
            ...
            <td valign="top">
                ...
                <form name="form1" onSubmit="return check_submit();" method="post" action="?lottery_type=<?php echo $lotteryType ?>&id=<?=$id;?>&action=<?=$id>0 ? 'edit' : 'add'?>&page=<?=$_REQUEST['page'];?>&orderno=<?=$orderno;?>">
                    ...
                    if ((0 < $id) && !isset($_GET['action'])){
                        $sql = 'select * from c_auto_choose5 where id=\'' . $id . '\' limit 1';
                    ...
                    <table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:5px;" bgcolor="#798EB9">
                        <tr>
                            ...
                            <td  align="left" bgcolor="#FFFFFF"><?php echo $lotteryNames[$lotteryType]?>【<a href="Uptime_choose5.php?lottery_type=<?php echo $lotteryType ?>" style="color:#ff0000;">点击进入盘口管理</a>】</td>
                        </tr>
                        <tr>
                ...
                <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1" class="font12" bgcolor="#798EB9" style="margin-top:5px;">
                    <form name="form1" method="get" action="?lottery_type=<?php echo $lotteryType ?>">
                        <tr>
                ...
                <table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:5px;" bgcolor="#798EB9">
                    ...
                    $sql = 'select id from c_auto_choose5 ' . $sqlwhere . ' order by qishu desc';
                    $stmt = $mydata1_db->prepare($sql);
                    $stmt->execute($params);
                    $sum = $stmt->rowCount();
                    $thisPage = 1;
                    ...
                    if ($id){
                        $id = rtrim($id, ',');
                        $sql = 'select * from c_auto_choose5 where id in(' . $id . ') order by qishu desc';
                        $query = $mydata1_db->query($sql);
                        $time = time();
                        while ($rows = $query->fetch()){
                            ...
                            if ($rows['ok'] == 1)
                            {
                                $ok = '<a href="../cj/lottery/js_choose5.php?lottery_type='.$lotteryType.'&qi=' . $rows['qishu'] . '&t=1"><font color="#FF0000">已结算</font></a>';
                            }
                            else
                            {
                                $ok = '<a href="../cj/lottery/js_choose5.php?lottery_type='.$lotteryType.'qi=' . $rows['qishu'] . '&t=1" onClick="return confirm(\'结算后将无法更改结算结果。\\n\\n请核对开奖结果，以免结算出错！\\n\\n如已经核对正确请点击确定进行结算。\');"><font color="#0000FF">未结算</font></a>';
                            }
                            ?>
                            <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over;?>'" onMouseOut="this.style.backgroundColor='<?=$out;?>'" style="background-color:<?=$color;?>;line-height:20px;">
                                ...
                                <td>
                                    ...
                                    <a href="javascript:void();" onClick="if(confirm('是否确定删除该记录？')){window.location.href='Auto_choose5.php?lottery_type=<?php echo $lotteryType?>action=delete&id=<?=$rows['id'];?>'}">删除</a>&nbsp;
