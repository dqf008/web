...
if (($_GET['action'] == 'cancel_qishu') && (0 < $qishu)){
	...
	$sql = 'select * from c_bet_3 where type=\'北京赛车PK拾\' and qishu=:qishu';
	...
	<tr> 
	  ...
	  <script> 
		  ...
		  if(confirm('您确定要撤销【'+qishu+'】期注单？撤销后金额将重算并退回！')) 
				  location.href='Order4.php?action=cancel_qishu&qishu='+qishu+'&page=<?=$_REQUEST['page'];?>';
		  } 
	  </script> 
  </table> 
...
$params = array();
$sql = 'select id from c_bet_3 where money>0 and type=\'北京赛车PK拾\' ';
if ($type){
	$params[':type'] = $type;
	$sql .= ' and type=:type';
}
...
function cancel_order($id){
	global $mydata1_db;
	$params = array(':id' => $id);
	$sql = 'select * from c_bet_3 where type=\'北京赛车PK拾\' and id=:id';
	$stmt = $mydata1_db->prepare($sql);
	$stmt->execute($params);
	$rs = $stmt->fetch();
	if ($rs){
		...
		$params = array(':id' => $id);
		$sql = 'delete from c_bet_3 where type=\'北京赛车PK拾\' and id=:id';
		$stmt = $mydata1_db->prepare($sql);
		$stmt->execute($params);
		...
		SELECT :uid,:userName,\'BJPK10\',\'CANCEL_BET\',:transferOrder,:transferAmount,money-:transferAmount2,money,:creationTime FROM k_user WHERE uid=:quid';
		$stmt = $mydata1_db->prepare($sql);
		$stmt->execute($params);
		include_once '../../class/admin.php';
		$message = '撤销[' . $kusername . ']北京赛车PK拾期号[' . $qishu . ']注单[' . $id . '],[注单金额:' . $money . ',可赢金额:' . $win . ',结算状态:' . $js . '],退回金额:' . $remoney;
