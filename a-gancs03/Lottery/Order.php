...
check_quanxian('cpgl');
$type = $_GET['type']?$_GET['type']:'重庆时时彩';
...
if (($_GET['action'] == 'cancel_qishu') && (0 < $qishu)){
	...
	$sql = 'select * from c_bet where type=\'重庆时时彩\' and qishu=:qishu';
	...
	<tr> 
	  ...
	  <script> 
		  ...
		  if(confirm('您确定要撤销【'+qishu+'】期注单？撤销后金额将重算并退回！')) 
				  location.href='Order.php?action=cancel_qishu&qishu='+qishu+'&page=<?=$_REQUEST['page'];?>';
		  } 
	  </script> 
  </table> 
...
function cancel_order($id){
	global $mydata1_db;
	$params = array(':id' => $id);
	$sql = 'select * from c_bet where type=\'重庆时时彩\' and id=:id';
	$stmt = $mydata1_db->prepare($sql);
	$stmt->execute($params);
	$rs = $stmt->fetch();
	if ($rs){
		...
		$params = array(':id' => $id);
		$sql = 'delete from c_bet where type=\'重庆时时彩\' and id=:id';
		$stmt = $mydata1_db->prepare($sql);
		$stmt->execute($params);
		...
		$message = '撤销[' . $kusername . ']重庆时时彩期号[' . $qishu . ']注单[' . $id . '],[注单金额:' . $money . ',可赢金额:' . $win . ',结算状态:' . $js . '],退回金额:' . $remoney;
