...
if ($save == 'ok'){
	if ($type < 6){
		...
		$sql = 'update c_odds_xjssc set h1=:h1,h2=:h2,h3=:h3,h4=:h4,h5=:h5,h6=:h6,h7=:h7,h8=:h8,h9=:h9,h10=:h10,h11=:h11,h12=:h12,h13=:h13,h14=:h14 where type=:type';
		$stmt = $mydata1_db->prepare($sql);
		$stmt->execute($params);
		echo "<script>alert(\"赔率修改完毕！\");window.open('odds_xjssc.php?type=".$type."','mainFrame');</script>";
		exit();
	}
	...
	if ($type == 6){
		...
		$sql = 'update c_odds_xjssc set h1=:h1,h2=:h2,h3=:h3,h4=:h4,h5=:h5,h6=:h6,h7=:h7 where type=:type';
		$stmt = $mydata1_db->prepare($sql);
		$stmt->execute($params);
		echo "<script>alert(\"赔率修改完毕！\");window.open('odds_xjssc.php?type=".$type."','mainFrame');</script>";
		exit();
	...
	if (6 < $type){
		...
		$sql = 'update c_odds_xjssc set h1=:h1,h2=:h2,h3=:h3,h4=:h4,h5=:h5 where type=:type';
		$stmt = $mydata1_db->prepare($sql);
		$stmt->execute($params);
		echo "<script>alert(\"赔率修改完毕！\");window.open('odds_xjssc.php?type=".$type."','mainFrame');</script>";
		exit();
...
<html xmlns="http://www.w3.org/1999/xhtml"> 
  ...
  //读取当前期数盘口赔率与投注总额 
  function loadinfo(){ 
	  $.post("get_odds_xjssc.php", {type :<?=$type?>}, function(data)
		  { 
  ...
  function UpdateRate(num ,i){ 
	  $.post("updaterate_xjssc.php", {type :<?=$type?> ,num : num ,i : i}, function(data)
		  { 
