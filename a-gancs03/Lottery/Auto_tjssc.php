...
if (($_GET['action'] == 'add') && ($id == 0)){
	...
	$sql = 'insert into c_auto_tjssc(qishu,datetime,ball_1,ball_2,ball_3,ball_4,ball_5' . "\r\n\t" . ') values (:qishu,:datetime,:ball_1,:ball_2,:ball_3,:ball_4,:ball_5)';
	...
	$sql = 'update c_auto_tjssc set qishu=:qishu,datetime=:datetime,ball_1=:ball_1,ball_2=:ball_2,ball_3=:ball_3,ball_4=:ball_4,ball_5=:ball_5 where id=:id';
	$stmt = $mydata1_db->prepare($sql);
	$stmt->execute($params);
    echo "<script>alert('更新成功');</script>";
}else if (($_GET['action'] == 'delete') && (0 < $id)){
	$params = array(':id' => $id);
	$sql = 'delete from c_auto_tjssc where id=:id';
	$stmt = $mydata1_db->prepare($sql);
	$stmt->execute($params);
...
<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="5"> 
  <tr> 
	...
	if ((0 < $id) && !isset($_GET['action'])){
		$sql = 'select * from c_auto_tjssc where id=\'' . $id . '\' limit 1';
	...
	?> 
		...
		<tr> 
		  ...
		  <td  align="left" bgcolor="#FFFFFF">天津时时彩【<a href="Uptime_tjssc.php" style="color:#ff0000;">点击进入盘口管理</a>】</td>
		</tr> 
		<tr> 
      ...
      <select name="ball_5" id="ball_5">
		...
		$sql = 'select id from c_auto_tjssc ' . $sqlwhere . ' order by qishu desc';
		$stmt = $mydata1_db->prepare($sql);
		$stmt->execute($params);
		$sum = $stmt->rowCount();
		$thisPage = 1;
		...
		if ($id){
		$id = rtrim($id, ',');
		$sql = 'select * from c_auto_tjssc where id in(' . $id . ') order by qishu desc';
		$query = $mydata1_db->query($sql);
		$time = time();
		while ($rows = $query->fetch()){
			...
			$hm[] = $rows['ball_5'];
			if ($rows['ok'] == 1){
				$ok = '<a href="../cj/lottery/js_tjssc.php?qi=' . $rows['qishu'] . '&t=1"><font color="#FF0000">已结算</font></a>';
			}else{
				$ok = '<a href="../cj/lottery/js_tjssc.php?qi=' . $rows['qishu'] . '&t=1" onClick="return confirm(\'结算后将无法更改结算结果。\\n\\n请核对开奖结果，以免结算出错！\\n\\n如已经核对正确请点击确定进行结算。\');"><font color="#0000FF">未结算</font></a>';
			}?>       
			<tr align="center" onMouseOver="this.style.backgroundColor='<?=$over;?>'" onMouseOut="this.style.backgroundColor='<?=$out;?>'" style="background-color:<?=$color;?>;line-height:20px;"> 
			  <td height="25" align="center" valign="middle">天津时时彩</td>
			  ...
			  <td> 
				  ...
				  <a href="javascript:void();" onClick="if(confirm('是否确定删除该记录？')){window.location.href='Auto_tjssc.php?action=delete&id=<?=$rows['id'];?>'}">删除</a>&nbsp;
				  <a href="javascript:void();" onClick="if(confirm('是否重算，期数为：<?=$rows['qishu'];?>？')){window.location.href='js_reset.php?action=js_reset&t=1&atype=tjssc&qi=<?=$rows['qishu'];?>'}">重算</a>&nbsp;
			  </td> 
			  </tr>
			  <?php }
