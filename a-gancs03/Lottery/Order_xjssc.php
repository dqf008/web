...
check_quanxian('cpgl');
$type= $_GET['type']?$_GET['type']:'新疆时时彩';
$type == '' ? $se1 = '#FF0' : $se1 = '#FFF';
$type == '新疆时时彩' ? $se4 = '#FF0' : $se4 = '#FFF';
...
if (($_GET['action'] == 'cancel_qishu') && (0 < $qishu)){
	...
	$sql = 'select * from c_bet where type=\'新疆时时彩\' and qishu=:qishu';
	...
	<tr> 
	  ...
	  <script> 
		  ...
		  if(confirm('您确定要撤销【'+qishu+'】期注单？撤销后金额将重算并退回！')) 
				  location.href='Order_xjssc.php?action=cancel_qishu&qishu='+qishu+'&page=<?=$_REQUEST['page'];?>';
		  } 
	  </script> 
  </table> 
...
?>
     <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over;?>'" onMouseOut="this.style.backgroundColor='<?=$out;?>'" style="background-color:<?=$color;?>;line-height:20px;"> 
	  ...
	  if ($rows['js'] == 1){?> <font color="#FF0000">已结算</font><?php }?> 
	  </td> 
...
function cancel_order($id){
	global $mydata1_db;
	$params = array(':id' => $id);
	$sql = 'select * from c_bet where type=\'新疆时时彩\' and id=:id';
	$stmt = $mydata1_db->prepare($sql);
	$stmt->execute($params);
	$rs = $stmt->fetch();
	if ($rs){
		...
		$params = array(':id' => $id);
		$sql = 'delete from c_bet where type=\'新疆时时彩\' and id=:id';
		$stmt = $mydata1_db->prepare($sql);
		$stmt->execute($params);
		...
		$sql = 'INSERT INTO k_money_log (uid,userName,gameType,transferType,transferOrder,transferAmount,previousAmount,currentAmount,creationTime) SELECT :uid,:userName,\'CQSSC\',\'CANCEL_BET\',:transferOrder,:transferAmount,money-:transferAmount2,money,:creationTime FROM k_user WHERE uid=:quid';
		$stmt = $mydata1_db->prepare($sql);
		$stmt->execute($params);
		include_once '../../class/admin.php';
		$message = '撤销[' . $kusername . ']新疆时时彩期号[' . $qishu . ']注单[' . $id . '],[注单金额:' . $money . ',可赢金额:' . $win . ',结算状态:' . $js . '],退回金额:' . $remoney;
