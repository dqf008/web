...
if (($_GET['action'] == 'add') && ($id == 0)){
	...
	$sql = 'update c_auto_4 set qishu=:qishu,datetime=:datetime,ball_1=:ball_1,ball_2=:ball_2,ball_3=:ball_3,ball_4=:ball_4,ball_5=:ball_5,ball_6=:ball_6,ball_7=:ball_7,ball_8=:ball_8,ball_9=:ball_9,ball_10=:ball_10 where id=:id';
	$stmt = $mydata1_db->prepare($sql);
	$stmt->execute($params);
	message('更新成功');
}else if (($_GET['action'] == 'delete') && (0 < $id)){
	$params = array(':id' => $id);
	$sql = 'delete from c_auto_4 where id=:id';
	$stmt = $mydata1_db->prepare($sql);
	$stmt->execute($params);
	message('删除成功');
}
...
<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="5"> 
  <tr> 
	...
	<tr> 
	  ...
	  <td  align="left" bgcolor="#FFFFFF">北京赛车PK拾【<a href="Uptime_4.php" style="color:#ff0000;">点击进入盘口管理</a>】</td> 
	</tr> 
	<tr> 
...
$sql = 'select id from c_auto_4 ' . $sqlwhere . ' order by qishu desc';
$stmt = $mydata1_db->prepare($sql);
$stmt->execute($params);
$sum = $stmt->rowCount();
$thisPage = 1;
...
while ($rows = $query->fetch()){
	...
	$hm[] = $rows['ball_10'];
	if ($rows['ok'] == 1){
		$ok = '<a href="../cj/lottery/js_4.php?qi=' . $rows['qishu'] . '&t=1"><font color="#FF0000">已结算</font></a>';
	}else{
		$ok = '<a href="../cj/lottery/js_4.php?qi=' . $rows['qishu'] . '&t=1" onClick="return confirm(\'结算后将无法更改结算结果。\\n\\n请核对开奖结果，以免结算出错！\\n\\n如已经核对正确请点击确定进行结算。\');"><font color="#0000FF">未结算</font></a>';
	}
?>       
	<tr align="center" onMouseOver="this.style.backgroundColor='<?=$over;?>'" onMouseOut="this.style.backgroundColor='<?=$out;?>'" style="background-color:<?=$color;?>;line-height:20px;"> 
	  ...
	  <td> 
		  ...
		  <a href="javascript:void();" onClick="if(confirm('是否确定删除该记录？')){window.location.href='Auto_4.php?action=delete&id=<?=$rows['id'];?>'}">删除</a>&nbsp;
		  <a href="javascript:void();" onClick="if(confirm('是否重算，期数为：<?=$rows['qishu'];?>？')){window.location.href='js_reset.php?action=js_reset&t=1&atype=bjpk10&qi=<?=$rows['qishu'];?>'}">重算</a>&nbsp;
	  </td> 
	  </tr>
<?php }
