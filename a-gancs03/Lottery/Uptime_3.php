...
if ($action == 'ok'){
	for ($i = 1;$i <= $nums;$i++){
		...
		$params = array(':kaipan' => $kaipan, ':fengpan' => $fengpan, ':kaijiang' => $kaijiang, ':qishu' => $i,':name'=>$lotteryType);
		if($lotteryType=='gdkl10'){
            $sql = "update c_opentime_3 set kaipan=:kaipan,fengpan=:fengpan,kaijiang=:kaijiang where qishu=:qishu";
        }else {
		    $params[':name'] = $lotteryType;
            $sql = "update c_opentime_klsf set kaipan=:kaipan,fengpan=:fengpan,kaijiang=:kaijiang where qishu=:qishu and `name`=:name";
        }
		  ...
		  <td valign="top"> 
			  ...
			  <table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:5px;" bgcolor="#798EB9"> 
				  <tr> 
					  ...
					  <td align="left" bgcolor="#FFFFFF"><?php echo $lotteryNames[$lotteryType]?>【<a href="Auto_3.php?lottery_type=<?php echo $lotteryType?>" style="color:#ff0000;">点击进入开奖设置</a>】</td>
				  </tr> 
				  <tr> 
...
if($lotteryType=='gdkl10'){
    ...
    $params[':name'] = $lotteryType;
    $sql =  "select * from c_opentime_klsf where `name`='$lotteryType' order by qishu asc";
}
