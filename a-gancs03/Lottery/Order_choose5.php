...
$id           = 0;
$qishu        = 0;
$lotteryType  = isset($_GET['lottery_type']) ? $_GET['lottery_type'] : 'gdchoose5';
$lotteryNames = ['gdchoose5' => 'GDSYXW', 'sdchoose5' => 'SDXYXW', 'fjchoose5' => 'FJSYXW', 'bjchoose5' => 'BJSYXW', 'ahchoose5' => 'AHSYXW','yfchoose5'=>'YFSYXW','sfchoose5'=>'SFSYXW'];
...
if (($_GET['action'] == 'cancel') && (0 < $id)) {
    check_quanxian('cpcd');
    cancel_order($id, $lotteryNames[$lotteryType], $lotteryType);
    message('操作成功');
}
...
if (($_GET['action'] == 'cancel_qishu') && (0 < $qishu)) {
    check_quanxian('cpcd');
    $params = [':qishu' => $qishu];
    $sql    = "select * from c_bet_choose5 where type='$lotteryNames[$lotteryType]' and qishu=:qishu";
            ...
            <tr>
                <td valign="top">
                            ...
                            <tr>
                                <td align="center" bgcolor="#FFFFFF">
                                    ...
                                    <select name="js" id="js">
                                        <option value="0"
                                                style="color:#FF9900;" <?= $_GET['js'] == '0' ? 'selected' : '' ?>>未结算注单
                                        </option>
                                        <option value="1"
                                                style="color:#FF0000;" <?= $_GET['js'] == '1' ? 'selected' : '' ?>>已结算注单
                                        </option>
                                        <option value="0,1" <?= $_GET['js'] == '0,1' ? 'selected' : '' ?>>全部注单</option>
                                    </select>

                                    期号：<input name="qishu" type="text" id="qishu" value="<?= $_GET['qishu'] ?>"
                                              style="width:100px">&nbsp;&nbsp;
                                    会员：<input name="username" type="text" id="username" value="<?= $_GET['username'] ?>"
                                              size="15">
                                    &nbsp;&nbsp;日期：
                                    <input name="s_time" type="text" id="s_time" value="<?= $_GET['s_time'] ?>"
                                           onClick="new Calendar(2008,2020).show(this);" size="10" maxlength="10"
                                           readonly="readonly"/>
                                    ~
                                    <input name="e_time" type="text" id="e_time" value="<?= $_GET['e_time'] ?>"
                                           onClick="new Calendar(2008,2020).show(this);" size="10" maxlength="10"
                                           readonly="readonly"/>&nbsp;&nbsp;注单号：
                    ...
                    <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1" class="font12"
                        ...
                        <tr>
                            <td align="center" bgcolor="#FFFFFF">
                                ...
                                <input type="button" name="Submit" value="撤销当期注单"
                                       onClick="cancle_qishu_check(<?php echo '"' . $_GET['lottery_type'] . '"' ?>);">
                            </td>
                        </tr>
                        <script>
                            function cancle_qishu_check(lotteryType) {
                                ...
                                if (confirm('您确定要撤销【' + qishu + '】期注单？撤销后金额将重算并退回！'))
                                    location.href = 'Order_choose5.php?lottery_type=' + lotteryType + '&action=cancel_qishu&qishu=' + qishu + '&page=<?=$_REQUEST['page'];?>';
                            }
                        </script>
                    </table>
                    <table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12"
                        ...
                        $params = [];
                        $sql    = "select id from c_bet_choose5 where money>0 and type='$lotteryNames[$lotteryType]' ";
                        if ($type) {
                            $params[':type'] = $type;
                            $sql             .= ' and type=:type';
                        }
                        ...
                        if ($bid) {
                            $bid    = rtrim($bid, ',');
                            $sql    = 'select * from c_bet_choose5 where id in(' . $bid . ') order by id desc';
                            ...
                            <tr align="center" onMouseOver="this.style.backgroundColor='<?= $over; ?>'"
                                ...
                                style="background-color:<?= $color; ?>;line-height:20px;">
                                ...
                                <td><a href="javascript:void(0);"
                                       onClick="if(confirm('您确定要撤销该注单？撤销后金额将重算并退回！'))location.href='?lottery_type=<?php echo $lotteryType?>&action=cancel&id=<?= $rows['id']; ?>&page=<?= $_REQUEST['page']; ?>';">撤销</a>
                                </td>
                                </tr><?php }
                        } ?>
                        <tr style="background-color:#FFFFFF;">
                            <td colspan="12" align="center" valign="middle">本页投注总金额：<?= sprintf('%.2f', $sum_tz); ?>元&nbsp;&nbsp;派彩总金额：<?= sprintf('%.2f', $sum_pc); ?>
                                元&nbsp;&nbsp;赢利总金额：<?= sprintf('%.2f', $sum_tz - $sum_pc); ?>元
                            </td>
                        </tr>
                        <tr style="background-color:#FFFFFF;">
...
function cancel_order($id, $lotteryName, $lotteryType)
{
    global $mydata1_db;
    $params = [':id' => $id];
    $sql    = "select * from c_bet_choose5 where type='$lotteryName' and id=:id";
    $stmt   = $mydata1_db->prepare($sql);
    $stmt->execute($params);
    $rs      = $stmt->fetch();
    $lottery = [
    ...
    if ($rs) {
        ...
        $params = [':id' => $id];
        $sql    = "delete from c_bet_choose5 where type='$lotteryName' and id=:id";
        $stmt   = $mydata1_db->prepare($sql);
        $stmt->execute($params);
        ...
        $params       = [':uid' => $kuid, ':userName' => $kusername,':gameType'=>$lottery[$lotteryType], ':transferOrder' => $id, ':transferAmount' => $remoney, ':transferAmount2' => $remoney, ':creationTime' => $creationTime, ':quid' => $kuid];
        $sql
                      = "INSERT INTO k_money_log (uid,userName,gameType,transferType,transferOrder,transferAmount,previousAmount,currentAmount,creationTime) 
		SELECT :uid,:userName,:gameType,'CANCEL_BET',:transferOrder,:transferAmount,money-:transferAmount2,money,:creationTime FROM k_user WHERE uid=:quid";
        $stmt         = $mydata1_db->prepare($sql);
        $stmt->execute($params);
        include_once '../../class/admin.php';
        $message = '撤销[' . $kusername . ']' . $lotteryName . '期号[' . $qishu . ']注单[' . $id . '],[注单金额:' . $money . ',可赢金额:' . $win . ',结算状态:' . $js . '],退回金额:' . $remoney;
