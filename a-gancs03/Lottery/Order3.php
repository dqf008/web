...
$lotteryNames = array('gdkl10'=>'广东快乐10分','cqkl10'=>'重庆快乐10分','tjkl10'=>'天津快乐10分','hnkl10'=>'湖南快乐10分','sxkl10'=>'山西快乐10分','ynkl10'=>'云南快乐10分','ffkl10'=>'一分快乐10分','sfkl10'=>'三分快乐10分');
if (0 < $_GET['id']){
	$id = intval($_GET['id']);
}
...
if (($_GET['action'] == 'cancel_qishu') && (0 < $qishu)){
	...
	$sql = "select * from c_bet_3 where type='$lotteryNames[$lotteryType]' and qishu=:qishu";
	...
	<tr> 
	  <td align="center" bgcolor="#FFFFFF">
          <select name="lottery_type" id="lottery_type" style="display: none">
              <option value="<?php echo $lotteryType?>" style="color:#FF0000;" <?=$_GET['lottery_type']? 'selected' : ''?>>彩票类型</option>
          </select>
	...
	<tr> 
	  ...
	  <script> 
		  ...
		  if(confirm('您确定要撤销【'+qishu+'】期注单？撤销后金额将重算并退回！')) 
				  location.href='Order3.php?lottery_type='+lotteryType+'&action=cancel_qishu&qishu='+qishu+'&page=<?=$_REQUEST['page'];?>';
		  } 
	  </script> 
  </table> 
...
?>
     <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over;?>'" onMouseOut="this.style.backgroundColor='<?=$out;?>'" style="background-color:<?=$color;?>;line-height:20px;"> 
	  ...
	  <td><a href="javascript:void(0);" onClick="if(confirm('您确定要撤销该注单？撤销后金额将重算并退回！'))location.href='?lottery_type=<?php echo $lotteryType?>&action=cancel&id=<?=$rows['id'];?>&page=<?=$_REQUEST['page'];?>';">撤销</a></td>
	  </tr><?php }
...
function cancel_order($id,$lotteryName,$lotteryType){
	global $mydata1_db;
	$params = array(':id' => $id);
	$sql = "select * from c_bet_3 where type='$lotteryName' and id=:id";
	$stmt = $mydata1_db->prepare($sql);
	$stmt->execute($params);
	$rs = $stmt->fetch();
	$lottery = array(
	...
	if ($rs){
		...
		$params = array(':id' => $id);
		$sql = "delete from c_bet_3 where type='$lotteryName' and id=:id";
		$stmt = $mydata1_db->prepare($sql);
		$stmt->execute($params);
		...
		SELECT :uid,:userName,'$lottery[$lotteryType]','CANCEL_BET',:transferOrder,:transferAmount,money-:transferAmount2,money,:creationTime FROM k_user WHERE uid=:quid";
		$stmt = $mydata1_db->prepare($sql);
		$stmt->execute($params);
