...
if (count($_POST) > 0) {
    ...
    $conf['pay_mkey'] = preg_replace('/(-----.*?-----)|\s*/', '', $_POST['pay_mkey']);
    $conf['pay_pubKey'] = preg_replace('/(-----.*?-----)|\s*/', '', $_POST['pay_pubKey']);
...
function save_to_data()
{
    ...
    $userConf['pay_mkey'] = "-----BEGIN PRIVATE KEY-----\n" . chunk_split($userConf['pay_mkey'], 64, "\n") . "-----END PRIVATE KEY-----";
    $userConf['pay_pubKey'] = "-----BEGIN PUBLIC KEY-----\n" . chunk_split($userConf['pay_pubKey'], 64, "\n") . "-----END PUBLIC KEY-----";
    $channelList = array();
...
<body>
<div class="container">
    <form style="max-width: 700px" method="post" action="">
        ...
        <div class="form-group">
            <label for="pay_pubKey" class="control-label">智慧付公钥：</label>
            <textarea class="form-control" rows="3" name="pay_pubKey"
                      placeholder="智慧付公钥"><?= $conf['pay_pubKey'] ?></textarea>
        </div>
        <div class="form-group">
            <label for="pay_mkey" class="control-label">商户秘钥：</label>
            <textarea class="form-control" rows="12" name="pay_mkey"
                      placeholder="商户秘钥"><?= $conf['pay_mkey'] ?></textarea>
        </div>
        <div class="form-group">
            <label for="pay_url" class="control-label">支付域名：</label>
            <input class="form-control" name="pay_url" placeholder="支付域名" value="<?= $conf['pay_url'] ?>">
            <p class="help-block">只需写域名即可，例如：http://pay.xxxx.com/</p>
        </div>
        <div class="form-group">
            <label for="inputEmail3" class="control-label">支付开关：</label>
            <label class="checkbox-inline">
                <input type="checkbox" name="list[]" <?= in_array('bank', $conf['open']) ? 'checked' : '' ?>
                       value="bank"> 网银支付
            </label>
            <label class="checkbox-inline">
