...
$now_time = time();
/* 调出下注时间为180秒内的下注记录 */
$stmt = $mydata1_db->query('SELECT `match_nowscore`, `match_time`, `match_id`, `bid`, `uid`, `master_guest`, `bet_time`, `bet_info`, `Match_GRedCard`, `Match_HRedCard`, `bet_money` FROM `mydata1_db`.`k_bet` WHERE `lose_ok`=0 AND `bet_time`>=DATE_SUB(now(), INTERVAL 180 SECOND)');
$match = array();
				...
				foreach($match[$val['Match_ID']] as $key=>$rows){
					...
					if($num-1<=$rows['num']){
						...
						if($bool){
							...
							$somecontent = '['.date('Y-m-d H:i:s', $now_time).']	管理员'.$_SESSION['login_name'].'审核了编号为'.$rows['bid'].'的'.$msg_t.'	投注金额['.$rows['bet_money'].']'.PHP_EOL;
...
<div align="left" style="padding:5px;background-color:#CCC">滚球自动审核 v2</div> 
