		...
		<tr align="center"> 
		  ...
		  <td width="100" align="center"> 
			  <select name="status" id="status" onChange="self.form1.submit()"> 
				  <option value="0" style="color:#FF9900;"<?=$_GET['status']=='0' ? 'selected="selected"' : '' ;?>>未结算注单</option> 
				  <option value="1,4,5" style="color:#FF0000;"<?=$_GET['status']=='1,4,5' ? 'selected="selected"' : '';?>>已赢注单</option> 
				  <option value="2" style="color:#009900;"<?=$_GET['status']=='2' ? 'selected="selected"' : '';?>>已输注单</option> 
				  <option value="3,6,7,8" style="color:#0000FF;"<?=$_GET['status']=='3,6,7,8' ? 'selected="selected"' : '';?>>和局或取消</option> 
				  <option value="8,0,1,2,3,4,5,6,7" <?=$_GET['status']=='8,0,1,2,3,4,5,6,7' ? 'selected="selected"' : '' ;?>>全部注单</option> 
			  </select> 
		  </td> 
		  <td width="90" align="center"> 
		  <label> 
			  <select name="order" id="order" onChange="self.form1.submit()"> 
				  <option value="bid"<?=$_GET['order']=='bid' ? 'selected="selected"' : '';?>>默认排序</option> 
				  <option value="bet_money"<?=$_GET['order']=='bet_money' ? 'selected="selected"' : '';?>>交易金额</option> 
				  <option value="win"<?=$_GET['order']=='win' ? 'selected="selected"' : '';?>>已赢金额</option> 
			  </select> 
		  </label> 
		  </td> 
...
<tr> 
  <td height="24" nowrap bgcolor="#FFFFFF"><table width="100%" border="1" bgcolor="#FFFFFF" bordercolor="#96B697" cellspacing="0" cellpadding="0" style="border-collapse: collapse;color: #225d9c;" > 
	...
	while ($rows = $query->fetch()){
		...
		}else if (($rows['match_type'] == 1) && (strtotime($rows['match_endtime']) <= strtotime($rows['bet_time']))){
			$over = $out = $color = '#FBA09B';
		}else if (double_format($rows['bet_money'] * ($rows['ben_add'] + $rows['bet_point'])) !== double_format($rows['bet_win'])){
			$over = $out = $color = '#FBA09B';
		}
		?> 	          
		<tr align="center" onMouseOver="this.style.backgroundColor='<?=$over;?>'" onMouseOut="this.style.backgroundColor='<?=$out;?>'" style="background-color:<?=$color;?>;"> 
			...
			<td>
			<a href="list.php?match_name=<?=urlencode($rows['match_name'])?>"><?=$rows['match_name']?> </a><br /><a href="list.php?www=<?=$rows['www']?>" style="color:#999999;"><?=$rows['www']?></a>
					<?php
					  if($rows["status"] == 3){
			...
			<td> 
			<a href="list.php?ball_sort=<?=urlencode($rows["ball_sort"])?>">
			...
			<font style="color:#FF0033">
			 <?php 
                if(strpos($rows["ball_sort"],'滚球') === false){
                    ...
                    if(strpos($rows["ball_sort"],'篮球') === false){
                         $rows["bet_info"] = str_replace('@','('.$rows["match_nowscore"].')@',$rows["bet_info"]);
                    }
			...
			<td align="center"><?=date("m-d H:i:s",strtotime($rows["bet_time"]))?><br>
				<?//获取开赛时间
					$table = '';
