...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC"> 
	...
	if ($gid){
	$gid = rtrim($gid, ',');
	$sql = 'select gid,count(*) as num from k_bet_cg where status not in(0,3) and gid in ('.$gid.') group by gid';
	$query = $mydata1_db->query($sql);
	while ($rows = $query->fetch()){
		$yjs[$rows['gid']] = $rows['num'];
	}
              ...
              <td>
			  	<div>
					<div style="float:left;">
						已结算：<?=$yjs[$rows["gid"]]?>&nbsp;[<a href="sgjcg_zd.php?gid=<?=$rows["gid"]?>">详细</a>]
					</div>
					<div style="float:right;">
						<span style="color:#999999;"><?=$rows["assets"]?></span>&nbsp;
						<a href="sgjcg.php?uid=<?=$rows["uid"]?>"><?=$rows["username"]?></a>&nbsp;
						<span style="color:#999999;"><?=$rows["balance"]?></span>
					</div>
					<div style="float:left; padding-left:10px;">
						<a href="sgjcg.php?www=<?=$rows['www']?>" style="color:#999999;"><?=$rows['www']?></a>
					</div>
				</div>
			  </td>
