	...
	<tr> 
	  <td height="24" nowrap bgcolor="#FFFFFF"><table width="100%" border="1" bgcolor="#FFFFFF" bordercolor="#96B697" cellspacing="0" cellpadding="0" style="border-collapse: collapse;color: #225d9c;" > 
		...
		while ($rows = $stmt->fetch()){
			...
			$over = '#EBEBEB';
			$out = '#ffffff';
			if (strtotime($rows['match_endtime']) <= strtotime($rows['bet_time'])){
		...
		?>   
		 <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>;">
			...
			<td >
			<a href="<?=$_SERVER['PHP_SELF']?>?match_id=<?=$rows["match_id"]?>" ><?=$rows["match_id"]?></a>
			<br/>
			<?php
			...
			<td>
				<?=$rows["ball_sort"]?>
				<br/>
				<font style="color:#FF0033"><?=str_replace("-","<br/>",$rows["bet_info"])?></font>
				<? 
			...
			<td><a href="cg_result.php?uid=<?=$rows["uid"]?>"><?=$rows["username"]?></a></td> 
			<td align="center">
				<?php
					if($rows["status"]==0){
						echo '未结算';
