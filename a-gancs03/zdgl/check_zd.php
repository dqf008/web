...
<table width="100%" border="0"> 
	...
	<tr> 
	  ...
	  <td width="84%">
		<label><textarea name="id" cols="80" rows="3" id="id"><?=$_REQUEST['id']?></textarea> 多条注意请用 , 隔开</label>
	  </td> 
	</tr> 
	<tr> 
	  <td>&nbsp;</td> 
...
if ($_REQUEST['id'] && $_REQUEST['action']){
	...
	foreach ($arr_id as $k => $id){
		if (strlen($id) > 8){
			...
			$sql = 'select k_bet.*,k_user.username from k_bet left join k_user on k_bet.uid=k_user.uid where `number`=:number limit 1';
...
<table width="100%" border="1" bgcolor="#FFFFFF" bordercolor="#96B697" cellspacing="0" cellpadding="0" style="border-collapse: collapse;color: #225d9c;" > 
	...
	<tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>;"> 
	  ...
	  <td>
	  <span style="color:#999999;"><?=$rows["assets"]?></span><br /><a href="list.php?uid=<?=$rows["uid"]?>"><?=$rows["username"]?></a><br /><span style="color:#999999;"><?=$rows["balance"]?></span>
	  </td> 
	</tr>  	
	<tr> 
...
<?php
		...
		$arr_z = $arr_b = array();
		$params = array(':gid' => $id);
		$sql = 'SELECT k.cg_count,k.uid,k.bet_money,k.win,k.bet_win,k.bet_time,k.status,k.balance,k.assets,k.www,u.username FROM k_bet_cg_group k,k_user u where k.uid=u.uid and k.gid=:gid';
		$stmt = $mydata1_db->prepare($sql);
		$stmt->execute($params);
		$rows = $stmt->fetch();
		$params = array(':gid' => $id);
		$sql = 'select * from k_bet_cg where gid=:gid';
		$stmt = $mydata1_db->prepare($sql);
		$stmt->execute($params);
		$sum = 0;
		while ($myrows = $stmt->fetch()){
			$arr_z[$sum++] = $myrows;
		}
?> 
<table width="100%" border="1" bgcolor="#FFFFFF" bordercolor="#96B697" cellspacing="0" cellpadding="0" style="border-collapse: collapse;color: #225d9c;" > 
	  	...
	  	foreach($arr_z as $k => $myrows){
			...
			<? if(mb_strpos($m[1],"让")>0) { //让球?>
            	<? if(mb_strpos($m[1],"主")===false) { //客让?>
                	<?=$team[1]?> <?=str_replace(array("主让","客让"),array("",""),$m[1])?> <?=$team[0]?>(主)     
				<? }else{ //主让?>
			...
			echo str_replace($arrayold,$arraynew,preg_replace('[\((.*)\)]', '',$m[$m_count-1]))
		   ?>
		   <br/>
		   </font>
		   <? if($myrows["status"]==3){?>
