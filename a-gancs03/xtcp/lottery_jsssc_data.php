...
include_once '../../cj/include/jsssc.func.php';
check_quanxian('cpgl');
check_quanxian('cpkj');
...
file_exists($config_file)&&$jsssc_config = include($config_file);
if(isset($jsssc_config['level'])&&in_array($jsssc_config['level'], $level)){
    foreach($level as $key=>$val){
        if($val==$jsssc_config['level']){
            $jsssc_config['level'] = $key;
            break;
        }
    }
}else{
    $jsssc_config['level'] = 's1';
}
...
if(in_array($action, array('random', 'setting', 'check', 'delete', 'change', 'save'))){
    ...
    switch($action){
        case 'random':
            ...
            $check = isset($jsssc_config['check'])&&$jsssc_config['check'];
            $tempRanking = getRanking(isset($_POST['history'])?$qishu:0, $check);
            if($jsssc_config['mode']=='s'||(isset($jsssc_config['ratio'][2])&&$jsssc_config['ratio'][2]!=0)){
                $keys = $tempArray = [[], []];
                for($i=0;$i<$level[$jsssc_config['level']];$i++){
                    // 随机计算结果
                ...
                if(empty($tempArray[1])){
                    ...
                    if($jsssc_config['mode']=='s'){
                        $jsssc_config['ratio'] = rand($jsssc_config['ratio'][0], $jsssc_config['ratio'][1]);
                        foreach($tempArray[1] as $key=>$val){
                            if($val[2]>=$jsssc_config['ratio']){
                        ...
                        $jsssc_config['ratio'] = $jsssc_config['ratio'][2];
                        $jsssc_data = [0, 0, 0];
                        $data_file = '../../cache/jsssc.data.php';
                        file_exists($data_file)&&$jsssc_data = include($data_file);
                        foreach($tempArray[1] as $key=>$val){
                            if(round(-10000*($jsssc_data[1]+$val[1])/($jsssc_data[0]+$val[0]))>=$jsssc_config['ratio']){
                                $keys[0][$key] = $val[2]; // 大于设定值
                            }else{
                                $keys[1][$key] = $val[2]; // 取最大值
                            }
        ...
        case 'setting':
            if(is_array($_POST['ratio'])){
                for($i=0;$i<=2;$i++){
                    if(isset($_POST['ratio'][$i])&&preg_match('/^\-?\d+(\.\d+)?$|^\-?0+\.\d+$/', $_POST['ratio'][$i])){
                        ...
                        $_POST['ratio'][$i]>=100&&$_POST['ratio'][$i] = 100;
                        $jsssc_config['ratio'][$i] = floor($_POST['ratio'][$i]*100);
                    }
                }
            }else{
                $_POST['mode'] = 'r';
                $jsssc_config['ratio'] = ['5000', '10000'];
            }
            ...
            if(isset($_POST['level'])&&in_array($_POST['level'], array_keys($level))){
                $jsssc_config['level'] = $level[$_POST['level']];
            }else{
                $jsssc_config['level'] = 100;
            }
            if(isset($_POST['check'])&&$_POST['check']=='true'){
                $jsssc_config['check'] = true;
            }else{
                $jsssc_config['check'] = false;
            }
            file_put_contents($config_file, '<?php'.PHP_EOL.'return unserialize(\''.serialize($jsssc_config).'\');');
            $_return['status'] = 'success';
            admin::insert_log($_SESSION['adminid'], '修改了极速时时彩的配置信息');
            break;
        case 'check':
            $_return['status'] = 'failed';
            $tempNum = [];
            if(count($_POST['opencode'])==5){
        ...
        case 'delete':
            $_return['status'] = 'failed';
            $params = array(':id' => isset($_POST['id'])?$_POST['id']:0, ':type' => 'JSSSC');
            ...
            if($stmt->rowCount()>0){
                ...
                admin::insert_log($_SESSION['adminid'], '删除极速时时彩[第'.$rows['value']['qishu'].'期]开奖信息[ID: '.$params[':id'].']');
            }
            break;
        ...
        case 'change':
            ...
            if($stmt->rowCount()>0){
                ...
                if($info['status']==2){
                    ...
                    admin::insert_log($_SESSION['adminid'], '结算极速时时彩[第'.$info['value']['qishu'].'期]，开奖结果：'.implode(',', $info['value']['opencode']));
                    ...
                    $stmt->execute(array(':type' => 'JSSSC', ':qishu' => $info['qishu']));
                    $_return['msg'] = 0;
                    ...
                    admin::insert_log($_SESSION['adminid'], '重算极速时时彩[第'.$info['qishu'].'期]');
                }
                $_return['status'] = 'success';
            }
            break;
        ...
        case 'save':
            ...
            if(empty($_return['msg'])){
                ...
                $stmt->execute([':qishu' => $qishu, ':type' => 'JSSSC']);
                if(isset($_POST['id'])&&!empty($_POST['id'])){
                    ...
                    if(!$rows||$rows['id']!=$_POST['id']){
                        ...
                        $stmt->execute($params)&&admin::insert_log($_SESSION['adminid'], '修改极速时时彩[第'.$_POST['qishu'].'期]开奖信息[ID: '.$_POST['id'].']');
                        $_return['status'] = 'success';
                    }
                }else{
                    if($stmt->rowCount()>0){
                        ...
                        $stmt = $mydata1_db->prepare($sql);
                        $stmt->execute($params)&&admin::insert_log($_SESSION['adminid'], '添加极速时时彩[第'.$_POST['qishu'].'期]开奖信息');
                        $_return['status'] = 'success';
                    }
                }
            }
            ...
            $(".font12:first").on("click", "input[type=button]", function(){
                ...
                $(".font12:first .data-status").on("click", function(){
                    layer.open({
                        ...
                        title: "极速时时彩",
                        content: "lottery_status.php?type=jsssc",
                        area: ['550px', '310px'],
                        shift: 0,
                        scrollbar: false
                    });
                        ...
                        case "modify":
                            ...
                            i.find("tr").last().find("input:first").val("保存"),
                                e.parent().find("img[data-ball]").map(function(e){
                                    s.eq(e).find("option[value="+$(this).data("ball")+"]").attr("selected", true)
                                });
                            break;
                        case "change":
                ...
                <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1" class="font12" bgcolor="#798EB9">
                    <tr>
                        <td align="center" bgcolor="#3C4D82" style="color:#FFF" colspan="2">
                            ...
                            <a id="ssc02" href="lottery_jsssc_data.php" class="menu_curr">极速时时彩</a> -
                    ...
                    <tr>
                        ...
                        <td align="left" bgcolor="#FFFFFF">极速时时彩</td>
                    </tr>
                    <tr>
                        <td align="right" bgcolor="#F0FFFF" width="100">杀率模式：</td>
                        <td align="left" bgcolor="#FFFFFF">
                            <select>
                                <option value="r"<?php echo $jsssc_config['mode']!='s'?' selected="true"':''; ?>>整体杀率</option>
                                <option value="s"<?php echo $jsssc_config['mode']=='s'?' selected="true"':''; ?>>单期杀率</option>
                            </select>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td align="left" bgcolor="#FFFFFF">
                            <select>
                                <option value="s1"<?php echo $jsssc_config['level']=='s1'?' selected="true"':''; ?>>默认</option>
                                <option value="s2"<?php echo $jsssc_config['level']=='s2'?' selected="true"':''; ?>>一级</option>
                                <option value="s3"<?php echo $jsssc_config['level']=='s3'?' selected="true"':''; ?>>二级</option>
                                <option value="s4"<?php echo $jsssc_config['level']=='s4'?' selected="true"':''; ?>>三级</option>
                                <option value="s5"<?php echo $jsssc_config['level']=='s5'?' selected="true"':''; ?>>四级</option>
                                <option value="s6"<?php echo $jsssc_config['level']=='s6'?' selected="true"':''; ?>>五级</option>
                            </select>
                    ...
                    <tr>
                        ...
                        <td align="left" bgcolor="#FFFFFF">最低 <input type="text" size="5" value="<?php echo sprintf('%.2f', $jsssc_config['ratio'][0]/100); ?>" /> % 最高 <input type="text" size="5" value="<?php echo sprintf('%.2f', $jsssc_config['ratio'][1]/100); ?>" /> % <span style="color:red">允许负数，最大 100.00%；如果需要固定杀率，请设置为相同数值</span></td>
                    </tr>
                    <tr>
                        <td align="right" bgcolor="#F0FFFF" width="100">整体杀率：</td>
                        <td align="left" bgcolor="#FFFFFF"><input type="text" size="5" value="<?php echo sprintf('%.2f', $jsssc_config['ratio'][2]/100); ?>" /> % <span style="color:red">允许负数，0 为不启用，最大 100.00%</span>，<a href="javascript:;" class="data-status">查看统计数据</a></td>
                    </tr>
                    <tr>
                        <td align="right" bgcolor="#F0FFFF" width="100">号码规律性：</td>
                        <td align="left" bgcolor="#FFFFFF"><input type="checkbox"<?php echo $jsssc_config['check']?' checked="true"':''; ?> /> 启用 <span style="color:red">* 该功能为实验性功能</span></td>
                    </tr>
                    <tr>
                ...
                <table border="0" align="center" cellspacing="1" cellpadding="5" width="100%" class="font12" style="margin-top:5px;" bgcolor="#798EB9">
                    ...
                    <tr>
                        ...
                        <td align="left" bgcolor="#FFFFFF">
                            ...
                            <select>
                                ...
                                <?php } ?>								</select>
                            <select>
                                <option value="">第四球</option>
                                <?php for($i=0;$i<10;$i++){ ?>									<option value="<?php echo $i; ?>"><?php echo $i; ?></option>
                                <?php } ?>								</select>
                            <select>
                                <option value="">第五球</option>
                                <?php for($i=0;$i<10;$i++){ ?>									<option value="<?php echo $i; ?>"><?php echo $i; ?></option>
                                <?php } ?>								</select>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td align="left" bgcolor="#FFFFFF">
                            ...
                            <a href="lottery_jsssc_datapl.php">批量添加</a>
                        </td>
                    </tr>
                </table>
                <table border="0" align="center" cellspacing="1" cellpadding="5" width="100%" class="font12" style="margin-top:5px;" bgcolor="#798EB9">
                    <tr style="background-color:#3C4D82;color:#FFF">
                        ...
                        <td height="22" align="center"><strong>第四球</strong></td>
                        <td height="22" align="center"><strong>第五球</strong></td>
