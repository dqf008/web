            ...
            $(".font12:first").on("click", "input[type=button]", function(){
                ...
                switch(t.data("action")){
                    case "start":
                        ...
                        !q.match(/^[1-9]\d{11}$/)||parseInt(q.substring(8))>1440||parseInt(q.substring(8))<1 ? alert("请正确填写彩票期号") : (!s.match(/^[1-9]\d*$/)||parseInt(s)<1||parseInt(s)>1440 ? alert("请设定添加期数，最少 1 期，最多 1440 期") : f());
                        break;
                    case "pause":
                            ...
                            n+i>1440&&(r.setDate(parseInt(r.getDate())+Math.floor((n+i)/1440), n-= 1440*Math.floor((n+i)/1440))),
                                ...
                                i+1>s ? (alert("批量添加完成"), t.siblings("input").trigger("click")) : t.data("action")=="pause"&&(
                                    x = m.clone().show(),
                                        ...
                                        $.post("lottery_ffk3_data.php", {action: "random", qishu: y, history: true}, function(d){
                                            if(d["status"]=="success"){
                                                $.each(d["msg"][0], function(i, v){
                                                    ...
                                                    $.post("lottery_ffk3_data.php", {action: "save", qishu: y, opencode: d["msg"][0]}, function(d){
                                                        if(d["status"]=="success"){
