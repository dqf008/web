            ...
            $(".font12:first").on("click", "input[type=button]", function(){
                ...
                switch(t.data("action")){
                    case "start":
                        ...
                        !q.match(/^[1-9]\d{10}$/)||parseInt(q.substring(8))>480||parseInt(q.substring(8))<1 ? alert("请正确填写彩票期号") : (!s.match(/^[1-9]\d*$/)||parseInt(s)<1||parseInt(s)>480 ? alert("请设定添加期数，最少 1 期，最多 480 期") : f());
                        break;
                    case "pause":
                    ...
                    case "continue":
                        ...
                        var q = v.eq(0).val(), s = v.eq(1).val(), r = new Date(), i = parseInt(t.data("i")), y = q.substring(0, 4), m = parseInt(q.substring(4, 6))-1, d = q.substring(6, 8), n = parseInt(q.substring(8)), f = function(){
                            var x;
                            n+i>480&&(r.setDate(parseInt(r.getDate())+Math.floor((n+i)/480), n-= 480*Math.floor((n+i)/480))),
                                ...
                                i+1>s ? (alert("批量添加完成"), t.siblings("input").trigger("click")) : t.data("action")=="pause"&&(
                                    x = m.clone().show(),
                                        ...
                                        $.post("lottery_sfk3_data.php", {action: "random", qishu: y, history: true}, function(d){
                                            if(d["status"]=="success"){
                                                $.each(d["msg"][0], function(i, v){
                                                    ...
                                                    $.post("lottery_sfk3_data.php", {action: "save", qishu: y, opencode: d["msg"][0]}, function(d){
                                                        if(d["status"]=="success"){
            ...
            <td valign="top">
                <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1" class="font12" bgcolor="#798EB9">
                    ...
                    <tr>
                        ...
                        <td align="left" bgcolor="#FFFFFF"><input type="text" size="5" /> 期 <span style="color:red">最多 480 期，超出请多次进行添加</span></td>
                    </tr>
                    <tr>
