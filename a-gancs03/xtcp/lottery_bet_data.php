...
$type = isset($_GET['type'])?$_GET['type']:'JSSC';
!in_array($type, array_keys($xtcp))&&$type = 'JSSC';

$page = array('limit' => 50);
$status = isset($_GET['status'])?$_GET['status']:'1';
!in_array($status, range(1, 7))&&$status = '1';
...
function cancel_order($id){
    ...
    if($stmt->rowCount()>0){
        ...
        if($rows['status']==2){
            ...
            /* 和局已退本金，不处理 */
            $rows['money']==$rows['win']&&$rows['remoney'] = 0;
            $rows['remoney']/= 100;
            $stmt = $mydata1_db->prepare('UPDATE `c_bet_data` SET `status`=2 WHERE `type`=:type AND `id`=:id');
            $stmt->execute($params);
            ...
            admin::insert_log($_SESSION['adminid'], '撤销['.$rows['username'].']'.$xtcp[$type].'期号['.$rows['value']['qishu'].']注单[注单号:'.$rows['id'].',注单金额:'.number_format($rows['money']/100, 2, '.', '').',可赢金额:'.number_format($rows['win']/100, 2, '.', '').',结算状态:' . $status[$rows['status']] . '],退回金额:'.number_format($rows['remoney'], 2, '.', ''));
            return true;
        }
    }else{
        return false;
        ...
        $(document).ready(function(){
            $("a[data-id]").on("click", function(){
                ...
                if(t.data("status")=="2"){
                    ...
                    $.post("<?php echo basename(__FILE__); ?>?type=<?php echo $type; ?>", {action: "cancel", id: t.data("id")}, function(data){
                    ...
                    <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1" class="font12" bgcolor="#798EB9">
                        <tr>
                            <td align="center" bgcolor="#3C4D82" style="color:#FFF">
                                <a id="ssc01" href="lottery_bet_data.php?type=JSSC" class="<?php echo $type=='JSSC'?'menu_curr':'menu_com'; ?>">极速赛车</a>&nbsp;&nbsp;-&nbsp;&nbsp;
                                <a id="ssc02" href="lottery_bet_data.php?type=JSSSC" class="<?php echo $type=='JSSSC'?'menu_curr':'menu_com'; ?>">极速时时彩</a>&nbsp;&nbsp;-&nbsp;&nbsp;
                                <a id="ssc03" href="lottery_bet_data.php?type=JSLH" class="<?php echo $type=='JSLH'?'menu_curr':'menu_com'; ?>">极速六合</a>&nbsp;&nbsp;-&nbsp;&nbsp;
                                <a id="ssc04" href="lottery_bet_data.php?type=FFK3" class="<?php echo $type=='FFK3'?'menu_curr':'menu_com'; ?>">分分快3</a>&nbsp;&nbsp;-&nbsp;&nbsp;
                                <a id="ssc05" href="lottery_bet_data.php?type=SFK3" class="<?php echo $type=='SFK3'?'menu_curr':'menu_com'; ?>">超级快3</a>&nbsp;&nbsp;-&nbsp;&nbsp;
                                <a id="ssc05" href="lottery_bet_data.php?type=WFK3" class="<?php echo $type=='WFK3'?'menu_curr':'menu_com'; ?>">好运快3</a>
                            </td>
                        </tr>
                        <tr>
                            <td align="center" bgcolor="#FFFFFF" style="color:#000">
                                <form action="<?php echo basename(__FILE__); ?>" method="get">
                                    ...
                                    <select name="status"> 
                                        ...
                                        <option value="2" style="color:#0000FF;"<?php echo $status=='2'?' selected="true"':'';?>>已结算(全部)</option>
                                        <option value="5" style="color:#FF0000;"<?php echo $status=='5'?' selected="true"':'';?>>已结算(盈利)</option>
                                        <option value="6" style="color:#00FF00;"<?php echo $status=='6'?' selected="true"':'';?>>已结算(亏损)</option>
                                        <option value="7"<?php echo $status=='7'?' selected="true"':'';?>>已结算(和局)</option>
...
<?php
$page['url'].= '?type='.$type.'&status='.urlencode($status).'&amp;';
...
?>                        <tr onmouseover="this.style.backgroundColor='#EBEBEB'" onmouseout="this.style.backgroundColor='#ffffff'" style="background-color:#FFFFFF;line-height:20px;">
                            ...
                            <td align="center"><?php echo $rows['value']['class'][0].'<br />'.$rows['value']['class'][1]; ?></td>
                            ...
                            <td align="center"><?php echo isset($rows['value']['result'])&&!empty($rows['value']['result'])?$rows['value']['result']:'暂无数据'; ?></td>
