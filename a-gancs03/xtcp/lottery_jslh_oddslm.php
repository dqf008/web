...
$stmt = $mydata1_db->prepare('SELECT `id`, `class`, `value` FROM `c_odds_data` WHERE `type`=:type AND `class` BETWEEN 56 AND 61 ORDER BY `id` ASC');
$stmt->execute(array(':type' => 'JSLH'));
while($rows = $stmt->fetch()){
    $rows['value'] = unserialize($rows['value']);
    $count = count($rows['value']);
    foreach($rows['value'] as $key=>$val){
?>                        <tr style="background-color:#FFFFFF;line-height:20px;" data-id="<?php echo $rows['id']; ?>">
<?php if($key==0){ ?>                            <td height="28" align="center" rowspan="<?php echo $count;?>"><?php echo $class[$rows['class']]; ?></td> 
<?php } ?>                            <td height="28" align="center"><?php echo $count>1?$val['class']:'--'; ?></td> 
