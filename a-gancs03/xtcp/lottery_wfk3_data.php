...
include_once '../../cj/include/wfk3.func.php';
check_quanxian('cpgl');
check_quanxian('cpkj');
...
file_exists($config_file)&&$wfk3Config = include($config_file);
if(isset($wfk3Config['level'])&&in_array($wfk3Config['level'], $level)){
    foreach($level as $key=>$val){
        if($val==$wfk3Config['level']){
            $wfk3Config['level'] = $key;
            break;
        }
    }
}else{
...
if(in_array($action, array('random', 'setting', 'check', 'delete', 'change', 'save'))){
    ...
    switch($action){
        case 'random':
            ...
            $check = isset($wfk3Config['check'])&&$wfk3Config['check'];
            $tempRanking = getRanking();
            if($wfk3Config['mode']=='s'||(isset($wfk3Config['ratio'][2])&&$wfk3Config['ratio'][2]!=0)){
                $keys = $tempArray = [[], []];
                for($i=0;$i<$level[$wfk3Config['level']];$i++){
                    // 随机计算结果
                ...
                if(empty($tempArray[1])){
                    ...
                    if($wfk3Config['mode']=='s'){
                        $wfk3Config['ratio'] = rand($wfk3Config['ratio'][0], $wfk3Config['ratio'][1]);
                        foreach($tempArray[1] as $key=>$val){
                            if($val[2]>=$wfk3Config['ratio']){
                        ...
                        $wfk3Config['ratio'] = $wfk3Config['ratio'][2];
                        $jssc_data = [0, 0, 0];
                        $data_file = '../../cache/wfk3.data.php';
                        file_exists($data_file)&&$jssc_data = include($data_file);
                        foreach($tempArray[1] as $key=>$val){
                            if(round(-10000*($jssc_data[1]+$val[1])/($jssc_data[0]+$val[0]))>=$wfk3Config['ratio']){
                                $keys[0][$key] = $val[2]; // 大于设定值
                            }else{
                                $keys[1][$key] = $val[2]; // 取最大值
                            }
        ...
        case 'setting':
            if(is_array($_POST['ratio'])){
                for($i=0;$i<=2;$i++){
                    if(isset($_POST['ratio'][$i])&&preg_match('/^\-?\d+(\.\d+)?$|^\-?0+\.\d+$/', $_POST['ratio'][$i])){
                        ...
                        $wfk3Config['ratio'][$i] = floor($_POST['ratio'][$i]*100);
                    }
                }
            }else{
                $_POST['mode'] = 'r';
                $wfk3Config['ratio'] = ['5000', '10000', '0'];
            }
            ...
            if(isset($_POST['level'])&&in_array($_POST['level'], array_keys($level))){
                $wfk3Config['level'] = $level[$_POST['level']];
            }else{
                $wfk3Config['level'] = 100;
            }
            if(isset($_POST['check'])&&$_POST['check']=='true'){
                $wfk3Config['check'] = true;
            }else{
                $wfk3Config['check'] = false;
            }
            file_put_contents($config_file, '<?php'.PHP_EOL.'return unserialize(\''.serialize($wfk3Config).'\');');
            $_return['status'] = 'success';
            admin::insert_log($_SESSION['adminid'], '修改了好运快3的配置信息');
            break;
        case 'check':
        ...
        case 'delete':
            $_return['status'] = 'failed';
            $params = array(':id' => isset($_POST['id'])?$_POST['id']:0, ':type' => 'WFK3');
            ...
            if($stmt->rowCount()>0){
                ...
                admin::insert_log($_SESSION['adminid'], '删除好运快3[第'.$rows['value']['qishu'].'期]开奖信息[ID: '.$params[':id'].']');
            }
            break;
        ...
        case 'change':
            ...
            if($stmt->rowCount()>0){
                ...
                if($info['status']==2){
                    ...
                    admin::insert_log($_SESSION['adminid'], '结算好运快3[第'.$info['value']['qishu'].'期]，开奖结果：'.implode(',', $info['value']['opencode']));
                    ...
                    $stmt->execute(array(':type' => 'WFK3', ':qishu' => $info['qishu']));
                    $_return['msg'] = 0;
                    ...
                    admin::insert_log($_SESSION['adminid'], '重算好运快3[第'.$info['qishu'].'期]');
                }
                $_return['status'] = 'success';
            }
            break;
        ...
        case 'save':
            ...
            if(empty($_return['msg'])){
                ...
                $stmt->execute([':qishu' => $qishu, ':type' => 'WFK3']);
                if(isset($_POST['id'])&&!empty($_POST['id'])){
                    ...
                    if(!$rows||$rows['id']!=$_POST['id']){
                        ...
                        $stmt->execute($params)&&admin::insert_log($_SESSION['adminid'], '修改好运快3[第'.$_POST['qishu'].'期]开奖信息[ID: '.$_POST['id'].']');
                        $_return['status'] = 'success';
                    }
                }else{
                    if($stmt->rowCount()>0){
                        ...
                        $stmt = $mydata1_db->prepare($sql);
                        $stmt->execute($params)&&admin::insert_log($_SESSION['adminid'], '添加好运快3[第'.$_POST['qishu'].'期]开奖信息');
                        $_return['status'] = 'success';
                    }
                }
            }
            ...
            $(".font12:first").on("click", "input[type=button]", function(){
                ...
                $(".font12:first .data-status").on("click", function(){
                    layer.open({
                        ...
                        title: "好运快3",
                        content: "lottery_status.php?type=wfk3",
                        area: ['550px', '310px'],
                        shift: 0,
                        scrollbar: false
                    });
                ...
                <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1" class="font12" bgcolor="#798EB9">
                    <tr>
                        <td align="center" bgcolor="#3C4D82" style="color:#FFF" colspan="2">
                            ...
                            <a id="ssc05" href="lottery_sfk3_data.php" class="menu_com">分分快3</a> -
                            <a id="ssc06" href="lottery_wfk3_data.php" class="menu_curr">好运快3</a>
                        </td>
                    </tr>
                    <tr>
                        <td align="right" bgcolor="#F0FFFF" width="100">彩票类别：</td>
                        <td align="left" bgcolor="#FFFFFF">好运快3</td>
                    </tr>
                    <tr>
                        <td align="right" bgcolor="#F0FFFF" width="100">杀率模式：</td>
                        <td align="left" bgcolor="#FFFFFF">
                            <select>
                                <option value="r"<?php echo $wfk3Config['mode']!='s'?' selected="true"':''; ?>>整体杀率</option>
                                <option value="s"<?php echo $wfk3Config['mode']=='s'?' selected="true"':''; ?>>单期杀率</option>
                            </select>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td align="left" bgcolor="#FFFFFF">
                            <select>
                                <option value="s1"<?php echo $wfk3Config['level']=='s1'?' selected="true"':''; ?>>默认</option>
                                <option value="s2"<?php echo $wfk3Config['level']=='s2'?' selected="true"':''; ?>>一级</option>
                                <option value="s3"<?php echo $wfk3Config['level']=='s3'?' selected="true"':''; ?>>二级</option>
                                <option value="s4"<?php echo $wfk3Config['level']=='s4'?' selected="true"':''; ?>>三级</option>
                                <option value="s5"<?php echo $wfk3Config['level']=='s5'?' selected="true"':''; ?>>四级</option>
                                <option value="s6"<?php echo $wfk3Config['level']=='s6'?' selected="true"':''; ?>>五级</option>
                            </select>
                    ...
                    <tr>
                        ...
                        <td align="left" bgcolor="#FFFFFF">最低 <input type="text" size="5" value="<?php echo sprintf('%.2f', $wfk3Config['ratio'][0]/100); ?>" /> % 最高 <input type="text" size="5" value="<?php echo sprintf('%.2f', $wfk3Config['ratio'][1]/100); ?>" /> % <span style="color:red">允许负数，最大 100.00%；如果需要固定比例，请设置为相同数值</span></td>
                    </tr>
                    <tr>
                        <td align="right" bgcolor="#F0FFFF" width="100">整体杀率：</td>
                        <td align="left" bgcolor="#FFFFFF"><input type="text" size="5" value="<?php echo sprintf('%.2f', $wfk3Config['ratio'][2]/100); ?>" /> % <span style="color:red">允许负数，0 为不启用，最大 100.00%</span>，<a href="javascript:;" class="data-status">查看统计数据</a></td>
                    </tr>
                    <tr>
                        <td align="right" bgcolor="#F0FFFF" width="100">号码规律性：</td>
                        <td align="left" bgcolor="#FFFFFF"><input type="checkbox"<?php echo $wfk3Config['check']?' checked="true"':''; ?> /> 启用 <span style="color:red">* 该功能为实验性功能</span></td>
                    </tr>
                    <tr>
                ...
                <table border="0" align="center" cellspacing="1" cellpadding="5" width="100%" class="font12" style="margin-top:5px;" bgcolor="#798EB9">
                    ...
                    <tr>
                        ...
                        <td align="left" bgcolor="#FFFFFF">
                            ...
                            <a href="lottery_wfk3_datapl.php">批量添加</a>
                        </td>
                    </tr>
                </table>
                <table border="0" align="center" cellspacing="1" cellpadding="5" width="100%" class="font12" style="margin-top:5px;" bgcolor="#798EB9">
