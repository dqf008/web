...
check_quanxian('fsgl');
$time = empty($_GET['time'])?'EN':$_GET['time'];
$bdate = $_GET['bdate'];
...
$fsfs = ($fsfs == 0 ? 1 : $fsfs);
$addfs = empty($_GET['addfs'])? 0 :intval($_GET['addfs']);
...
<div id="pageMain">
  ...
  <table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:5px;line-height:20px;" bgcolor="#798EB9">
	  ...
	  <?php 
		...
		$params = array();
		$sqlwhere = $sqlwhere2 = $sqlwhere3 = $sqlwhere4 = '';
		if ($username != ''){
			...
			$sqlwhere3 .= ' and u.username=:username3';
			$sqlwhere4 .= ' and u.username=:username4';
		}
		if ($group != ''){
			$params[':group'] = $group;
			$params[':group2'] = $group;
			...
			$sqlwhere3 .= ' and u.gid=:group3';
			$sqlwhere4 .= ' and u.gid=:group4';
		}
		if ($agent != ''){
			$params[':agent'] = $agent;
			$params[':agent2'] = $agent;
			...
			$sqlwhere3 .= ' and u.top_uid=:agent3';
			$sqlwhere4 .= ' and u.top_uid=:agent4';
		}
		$params[':q_btime'] = $q_btime;
		$params[':q_etime'] = $q_etime;
		$params[':q_btime2'] = $q_btime;
		$params[':q_etime2'] = $q_etime;
		$params[':q_btime3'] = $q_btime;
		$params[':q_etime3'] = $q_etime;
		$params[':q_btime4'] = strtotime($q_btime);
		$params[':q_etime4'] = strtotime($q_etime);
		$sql = 'select uid,username,sum(num) as num,sum(bet_money) as bet_money,sum(yx_bet_money) as yx_bet_money,sum(bet_money-win) as yingli, gid, top_uid from (';
		$sql .= 'select k.uid,k.username,1 as num,k.money as bet_money,if(k.win<>0,k.money,0) as yx_bet_money,case when k.win<0 then 0 when k.win=0 then k.money else k.win end as win, u.gid, u.top_uid from c_bet k left join k_user u on u.uid=k.uid where k.money>0 and k.js=1 and k.addtime>=:q_btime and k.addtime<=:q_etime ' . $sqlwhere2;
		$sql .= ' union all ';
		$sql .= 'select k.uid,k.username,1 as num,k.money as bet_money,if(k.win<>0,k.money,0) as yx_bet_money,case when k.win<0 then 0 when k.win=0 then k.money else k.win end as win, u.gid, u.top_uid from c_bet_3 k left join k_user u on u.uid=k.uid where k.money>0 and k.js=1 and k.addtime>=:q_btime2 and k.addtime<=:q_etime2 ' . $sqlwhere3;
		$sql .= ' union all ';
		$sql .= 'select u.uid,k.username,1 as num,k.money as bet_money,if(k.win<>0,k.money,0) as yx_bet_money,k.win+k.money as win, u.gid, u.top_uid from lottery_data k left join k_user u on k.username=u.username where k.money>0 and k.bet_ok=1 and k.bet_time>=:q_btime3 and k.bet_time<=:q_etime3 ' . $sqlwhere;
		$sql .= ' union all ';
		$sql .= 'select u.uid,u.username,1 as num,k.money/100 as bet_money,if(k.win<>0,k.money/100,0) as yx_bet_money,case when k.win<0 then 0 when k.win=0 then k.money/100 else k.win/100 end as win, u.gid, u.top_uid from c_bet_data k left join k_user u on k.uid=u.uid where k.money>0 and k.status=1 and k.addtime>=:q_btime4 and k.addtime<=:q_etime4 ' . $sqlwhere4;
...
?> 		  
		...
		<td><?=array_key_exists($row["top_uid"],$agent_list)?$agent_list[$row["top_uid"]]:''?></td>
