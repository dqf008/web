...
if (@($_GET['action']) == 'save'){
	...
	$ip = trim($_POST['ip']);
	$params = array(':login_name' => trim($_POST['login_name']), ':login_pwd' => md5(md5($_POST['login_pwd'])), ':quanxian_str' => $quanxian_str, ':about' => trim($_POST['about']), ':ip' => $ip, ':address' => trim($_POST['address']));
	$sql = 'insert into mydata3_db.sys_admin(login_name,login_pwd,quanxian,about,ip,address) values(:login_name,:login_pwd,:quanxian_str,:about,:ip,:address)';
