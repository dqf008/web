...
<tr class="tbtitle"> 
  <td width="13%"><span class="STYLE6">[<?=$_GET['username'];?>]总代理报表查询</span></td> 
  <td width="87%"><table border="0" align="center" cellspacing="0" cellpadding="1" bordercolor="888888" bordercolordark="#FFFFFF" width="98%"> 
	<tr> 
...
&nbsp;&nbsp;&nbsp;&nbsp;投注品种：
	  ...
	  <td width="21%"><div align="right"> 
		  <button type="button" onclick="javascript:location.href='index.php?action=re_guan&kithe=<?=$kithe;?>&username=<?=$guanname;?>'"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="width:100;height:22" ><img src="images/cal_date_picker.gif" width="15" height="12" align="absmiddle" />返回股东</button> 
...
while ($rs = $result->fetch()){
	...
	$zz_sf += -$usersf - $zongsf - $daisf;
	$zong_sf += -$usersf - $zongsf - $daisf;
	$dai_sf += -$usersf - $daisf;
	$paramsSub = array(':kauser' => $rs['dai']);
...
<tr >
    ...
    <td align="center" nowrap="nowrap" bordercolor="cccccc"><?=$rs['dai']?>
        <?=$xm?></td>
    <td align="center" nowrap="nowrap" bordercolor="cccccc"><?=$Rs5['re']?></td>
    <td align="center" nowrap="nowrap" bordercolor="cccccc"><button onclick="javascript:location.href='index.php?action=re_dai&amp;kithe=<?=$kithe?>&amp;guanname=<?=$guanname?>&amp;agentname=<?=$agentname?>&amp;username=<?=$rs['dai']?>&amp;txt9=<?=$_GET['txt9']?>&amp;txt8=<?=$_GET['txt8']?>&amp;class2=<?=$_GET['class2']?>'"  class="headtd4" style="width:80;height:22" ;><font color="0000ff">  <?=$Rs5['sum_m']?></font></button></td>
    ...
    <td height="28" align="center" nowrap="nowrap" bordercolor="cccccc"><?=$Rs6['dai_ds']+$Rs7['dai_ds']?></td>
    ...
    <td align="center" nowrap="nowrap" bordercolor="cccccc"><span class="STYLE5" style="color:<? if ((0-$usersf-$daisf)>0) echo 'black'; else echo 'red'; ?>;"> <?=number_format((0-$usersf-$daisf), 2)?>  </span></td>
    ...
    <td align="center" nowrap="nowrap" bordercolor="cccccc" style="color:<? if ((0-$usersf-$zongsf-$daisf)>0) echo 'black'; else echo 'red'; ?>;"><?=number_format((0-$usersf-$zongsf-$daisf), 2)?>    </td>
  </tr>
  <? }?>
  <tr >
    ...
    <td align="center" nowrap="nowrap" bordercolor="cccccc" bgcolor="#FDF4CA" class="STYLE3" style="color:<? if ($dai_sf>0) echo 'black'; else echo 'red'; ?>;"><?=number_format($dai_sf, 2)?></td>
     <td align="center" nowrap="nowrap" bordercolor="cccccc" bgcolor="#FDF4CA" class="STYLE3"><span class="STYLE3"><?=number_format($z_zongds,2)?></span></td>
   <td align="center" nowrap="nowrap" bordercolor="cccccc" bgcolor="#FDF4CA" class="STYLE3"><span class="STYLE3" style="color:<? if ($z_zongsf-$z_zongds>0) echo 'black'; else echo 'red'; ?>;">    <?=number_format($z_zongsf-$z_zongds,2)?>  </span></td>
