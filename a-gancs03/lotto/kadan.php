...
<table width="100%" border="0" cellspacing="0" cellpadding="5"> 
  <tr class="tbtitle"> 
		...
		<tr> 
		  ...
		  <td align="center" colspan="6"><table width="100%"  border="0" cellspacing="0" cellpadding="0"> 
			  <tr> 
				...
				<td width="80" align="center"><input type="submit" value="搜索" name="B12"   class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="width:60;height:22" ></td> 
				<td>&nbsp;</td> 
			  </tr> 
		  </table></td> 
		</tr> 
	...
	<td width="49%"><div align="right"> 
	  <button type="button" onclick="javascript:location.href='index.php?action=kadan&amp;id=0'"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle" /><font <? if ($id==0) {?> color=ff0000<? }else{?> color=000000<? }?>>所有代理</font></button>&nbsp;
	  ...
	  <button type="button" onclick="javascript:location.href='index.php?action=kadan&amp;id=1'"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle" /><font <?php if ($id == 1){?> color=ff0000<?php }else{?> color=000000<?php }?> >禁止</font></button> 
	</div></td> 
  </tr> 
  <tr > 
...
$sql = 'select count(*) from ka_guan ' . $vvv . ' order by id desc';
$stmt = $mydata2_db->prepare($sql);
$stmt->execute($params);
...
$sql = 'select * from ka_guan  ' . $vvv . '  order by id desc limit ' . $start_limit . ', ' . $perpage;
$result = $mydata2_db->prepare($sql);
$result->execute($params);
while ($image = $result->fetch()){
	$mumu = 0;
	$params = array(':danid' => $image['id']);
	$stmt = $mydata2_db->prepare('Select SUM(cs) As sum_m  From ka_mem Where   danid=:danid order by id desc');
	$stmt->execute($params);
	...
	$params = array(':danid' => $image['id']);
	$stmt = $mydata2_db->prepare('Select Count(ID) As memnum2 From ka_mem Where danid=:danid order by id desc');
	$stmt->execute($params);
	$rsw = $stmt->fetchColumn();
	if ($rsw != ''){
		$memnum2 = $rsw;
	...
	$params = array(':id' => $image['zongid']);
	$stmt = $mydata2_db->prepare('select * from ka_guan where id=:id and lx=2 order by id desc');
	$stmt->execute($params);
	$rsw = $stmt->fetch();
	if ($rsw != ''){
		$mjmj = $rsw['sj'];
...
<tr> 
		  ...
		  <td height="25" align="center" bordercolor="cccccc"> 
		   <button type="button" onclick="javascript:location.href='index.php?action=kamem&amp;id=2&amp;ids2=<?=$image['id'];?>'"  class="headtd4" style="width:80;height:22" ><font color="ff6600"><?=$image['kauser'];?></font></button>  			    </td> 
		  ...
		  <td height="25" align="center" bordercolor="cccccc"> 
		  <button type="button" class=headtd4  onmouseover="this.className='headtd3';window.status='代管理';return true;" onMouseOut="this.className='headtd4';window.status='代管理';return true;"  onClick="javascript:location.href='index.php?action=kadan&ids=<?=$image['guanid'];?>'"><?=$image['guan'];?></button>  			  </td> 
		...
		<?php }?> 
		<tr> 
		<td height="25" colspan="16" bordercolor="cccccc">
