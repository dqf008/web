...
while ($rs = $result->fetch()){
	...
	$result1 = $mydata2_db->prepare('Select sum(sum_m) as sum_m,count(*) as re,sum(sum_m*guan_ds/100*dagu_zc/10) as sum_ds,sum(sum_m*rate*dagu_zc/10) as sum_m3 from ka_tan   where Kithe=:Kithe and lx=0 and  class1=:class1 and  class2=:class2  and class3=:class3');
	...
	$RsB = $result4->fetch();
	if (($rs['class2'] == '二肖') || ($rs['class2'] == '三肖') || ($rs['class2'] == '四肖') || ($rs['class2'] == '五肖') || ($rs['class2'] == '六肖')){
		$params = array(':class1' => $rs['class1'], ':class2' => $rs['class2']);
		$result5 = $mydata2_db->prepare('Select * from ka_bl   where   class1=:class1 and  class2=:class2 ');
	}else{
	...
	if (($rs['class2'] == '四肖') || ($rs['class2'] == '五肖') || ($rs['class2'] == '六肖')){
		$cclass3 = $rs['class2'];
	}else{
		$cclass3 = $rs['class3'];
	}
