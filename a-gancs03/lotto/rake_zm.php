...
if ($_GET['save'] == 'save'){
	if (empty($_POST['bl'])){
		echo "<script>alert('赔率不能为空!');history.back();</script>";
		exit();
	}
	...
	for ($tt = 1;$tt <= 49;$tt++){
		...
		if ($_POST['dx'] == '全部'){
			if ($_GET['ids'] == '正A'){
				...
				$stmt->execute($params);
				$params = array(':rate' => $num, ':blrate' => $num, ':class3' => $tt);
				$stmt = $mydata2_db->prepare('update ka_bl set rate=:rate,blrate=:blrate where class2=\'正B\' and  class3=:class3');
				$stmt->execute($params);
			}else{
				$params = array(':rate' => $_POST['bl'], ':blrate' => $_POST['bl'], ':class2' => $ids, ':class3' => $tt);
				$stmt = $mydata2_db->prepare('update ka_bl set rate=:rate,blrate=:blrate where class2=:class2 and  class3=:class3');
				$stmt->execute($params);
				$params = array(':rate' => $num1, ':blrate' => $num1, ':class3' => $tt);
				$stmt = $mydata2_db->prepare('update ka_bl set rate=:rate,blrate=:blrate where class2=\'正A\' and  class3=:class3');
				$stmt->execute($params);
			}
		}
	...
	for ($tt = 1;$tt <= 53;$tt++){
		...
		if ($_GET['ids'] == '正A'){
			if ($tt <= 49){
				...
				$params = array(':rate' => $num3, ':blrate' => $num3, ':class3' => $_POST['class3_' . $tt]);
				$stmt = $mydata2_db->prepare('update ka_bl set rate=:rate,blrate=:blrate where class2=\'正B\' and  class3=:class3');
				$stmt->execute($params);
			}
		}else if ($tt <= 49){
			$params = array(':rate' => $num2, ':blrate' => $num2, ':class3' => $_POST['class3_' . $tt]);
			$stmt = $mydata2_db->prepare('update ka_bl set rate=:rate,blrate=:blrate where class2=\'正A\' and  class3=:class3');
			$stmt->execute($params);
		}else{
			$params = array(':rate' => $num4, ':blrate' => $num4, ':class3' => $_POST['class3_' . $tt]);
			$stmt = $mydata2_db->prepare('update ka_bl set rate=:rate,blrate=:blrate where class2=\'正A\' and  class3=:class3');
			$stmt->execute($params);
		}
	}
	
	echo "<script>alert('修改成功!');window.location.href='index.php?action=rake_zm&ids=".$ids."';</script>";
	exit();
}
...
if ($_GET['savew'] == 'savew'){
	...
	for ($f = 0;$f < $ss4;$f++){
		if ($mv == 0){
			...
			$stmt = $mydata2_db->prepare('update ka_bl set rate=rate-:rate where class1=\'正码\'  and   class3=:class3');
			$stmt->execute($params);
		}else{
			$params = array(':rate' => $money, ':class3' => $pc4[$f]);
			$stmt = $mydata2_db->prepare('update ka_bl set rate=rate+:rate where class1=\'正码\'  and   class3=:class3');
...
<link rel="stylesheet" href="xp.css" type="text/css"> 
	  ...
	  <table width="100%" border="0" cellspacing="0" cellpadding="5"> 
		...
		<table   border="1" align="center" cellspacing="1" cellpadding="1" bordercolordark="#FFFFFF" bordercolor="f1f1f1" width="99%"> 
		   <form name="form1" method="post" action="index.php?action=rake_zm&act=修改&ids=<?=$ids;?>"> 
		   <tr bordercolor="cccccc" > 
			<td height="28" colspan="20" align="center" nowrap="nowrap" bordercolor="cccccc" bgcolor="#FE773D"><table width="99%" border="0" cellspacing="0" cellpadding="0"> 
			  <tr> 
				...
				<td><div align="right"> 
				  <button type="button" onClick="javascript:location.href='index.php?action=rake_zm&ids=正A';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm1 STYLE='color:<?=$z1color;?>;'>正A</span></button>&nbsp;
				  <button type="button" onClick="javascript:location.href='index.php?action=rake_zm&ids=正B';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:<?=$z2color;?>;'>正B</span></button> 
				</div></td> 
			  </tr> 
			</table></td> 
		  </tr> 
...
<?php for ($I = 1;$I <= 10;$I = $I + 1){?>
      ...
      style="HEIGHT: 18px"  class="input1" maxlength="6" size="4" value="<?=$ShowTable[$I-1][0]?>" name="Num_<?=$I?>" /></td>
                  <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td><a style="cursor:hand" onClick="UpdateRate('MODIFYRATE','Num_<?=$I?>','bl<?=$I-1?>','class1=正码&ids=<?=$ids?>&sqq=sqq&lxlx=1&qtqt=0.05&class3=<?=$ShowTable[$I-1][1]?>');"><img src="images/bvbv_01.gif"   width="19" height="17" border="0"></a></td>
                    </tr>
                    <tr>
                      <td><a style="cursor:hand" onClick="UpdateRate('MODIFYRATE','Num_<?=$I?>','bl<?=$I-1?>','class1=正码&ids=<?=$ids?>&sqq=sqq&lxlx=0&qtqt=0.05&class3=<?=$ShowTable[$I-1][1]?>');"><img src="images/bvbv_02.gif" width="19" height="17" border="0"  ></a></td>
                    </tr>
                  </table></td>
                  <td><input type=checkbox id=lock<?=$I-1?> style="zoom:95%" title="关闭该项" onClick="UpdateRate('LOCK','lock<?=$I-1?>','','class1=正码&ids=<?=$ids?>&sqq=sqq&class3=<?=$ShowTable[$I-1][1]?>&lock='+this.checked);" value="TRUE"  <? if ($ShowTable[$I-1][2]==1){echo "checked";}?>>
                  </input></td>
                </tr>
              </table>
			 ...
			 <img src="images/num<?=$I+10?>.gif" /></td>
              <td height="25" align="center" bordercolor="cccccc"><table border="0" cellspacing="0" cellpadding="0">
                <tr>
                  ...
                  <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td><a style="cursor:hand" onClick="UpdateRate('MODIFYRATE','Num_<?=$I+10?>','bl<?=$I+10-1?>','class1=正码&ids=<?=$ids?>&sqq=sqq&lxlx=1&qtqt=0.05&class3=<?=$ShowTable[$I+10-1][1]?>');"><img src="images/bvbv_01.gif"   width="19" height="17" border="0"></a></td>
                    </tr>
                    <tr>
                      <td><a style="cursor:hand" onClick="UpdateRate('MODIFYRATE','Num_<?=$I+10?>','bl<?=$I+10-1?>','class1=正码&ids=<?=$ids?>&sqq=sqq&lxlx=0&qtqt=0.05&class3=<?=$ShowTable[$I+10-1][1]?>');"><img src="images/bvbv_02.gif" width="19" height="17" border="0"  ></a></td>
                    </tr>
                  </table></td>
                  <td><input type=checkbox id=lock<?=$I+10-1?> style="zoom:95%" title="关闭该项" onClick="UpdateRate('LOCK','lock<?=$I+10-1?>','','class1=正码&ids=<?=$ids?>&sqq=sqq&class3=<?=$ShowTable[$I+10-1][1]?>&lock='+this.checked);"  <? if ($ShowTable[$I+10-1][2]==1){echo "checked";}?>></td>
                </tr>
              </table>
      ...
      style="HEIGHT: 18px"  class="input1" maxlength="6" size="4" value="<?=$ShowTable[$I+20-1][0]?>" name="Num_<?=$I+20?>" /></td>
                  <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td><a style="cursor:hand" onClick="UpdateRate('MODIFYRATE','Num_<?=$I+20?>','bl<?=$I+20-1?>','class1=正码&ids=<?=$ids?>&sqq=sqq&lxlx=1&qtqt=0.05&class3=<?=$ShowTable[$I+20-1][1]?>');"><img src="images/bvbv_01.gif"   width="19" height="17" border="0"></a></td>
                    </tr>
                    <tr>
                      <td><a style="cursor:hand" onClick="UpdateRate('MODIFYRATE','Num_<?=$I+20?>','bl<?=$I+20-1?>','class1=正码&ids=<?=$ids?>&sqq=sqq&lxlx=0&qtqt=0.05&class3=<?=$ShowTable[$I+20-1][1]?>');"><img src="images/bvbv_02.gif" width="19" height="17" border="0"  ></a></td>
                    </tr>
                  </table></td>
                  <td><input type=checkbox id=lock<?=$I+20-1?> style="zoom:95%" title="关闭该项" onClick="UpdateRate('LOCK','lock<?=$I+20-1?>','','class1=正码&ids=<?=$ids?>&sqq=sqq&class3=<?=$ShowTable[$I+20-1][1]?>&lock='+this.checked);"  <? if ($ShowTable[$I+20-1][2]==1){echo "checked";}?>></td>
                </tr>
              </table>
      ...
      style="HEIGHT: 18px"  class="input1" maxlength="6" size="4" value="<?=$ShowTable[$I+30-1][0]?>" name="Num_<?=$I+30?>" /></td>
                  <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td><a style="cursor:hand" onClick="UpdateRate('MODIFYRATE','Num_<?=$I+30?>','bl<?=$I+30-1?>','class1=正码&ids=<?=$ids?>&sqq=sqq&lxlx=1&qtqt=0.05&class3=<?=$ShowTable[$I+30-1][1]?>');"><img src="images/bvbv_01.gif"   width="19" height="17" border="0"></a></td>
                    </tr>
                    <tr>
                      <td><a style="cursor:hand" onClick="UpdateRate('MODIFYRATE','Num_<?=$I+30?>','bl<?=$I+30-1?>','class1=正码&ids=<?=$ids?>&sqq=sqq&lxlx=0&qtqt=0.05&class3=<?=$ShowTable[$I+30-1][1]?>');"><img src="images/bvbv_02.gif" width="19" height="17" border="0"  ></a></td>
                    </tr>
                  </table></td>
                  <td><input type=checkbox id=lock<?=$I+30-1?> style="zoom:95%" title="关闭该项" onClick="UpdateRate('LOCK','lock<?=$I+30-1?>','','class1=正码&ids=<?=$ids?>&sqq=sqq&class3=<?=$ShowTable[$I+30-1][1]?>&lock='+this.checked);"  <? if ($ShowTable[$I+30-1][2]==1){echo "checked";}?>></td>
                </tr>
              </table>
      ...
      style="HEIGHT: 18px"  class="input1" maxlength="6" size="4" value="<?=$ShowTable[$I+40-1][0]?>" name="Num_<?=$I+40?>" /></td>
                  <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td><a style="cursor:hand" onClick="UpdateRate('MODIFYRATE','Num_<?=$I+40?>','bl<?=$I+39?>','class1=正码&ids=<?=$ids?>&sqq=sqq&lxlx=1&qtqt=0.05&class3=<?=$ShowTable[$I+40-1][1]?>');"><img src="images/bvbv_01.gif"   width="19" height="17" border="0"></a></td>
                    </tr>
                    <tr>
                      <td><a style="cursor:hand" onClick="UpdateRate('MODIFYRATE','Num_<?=$I+40?>','bl<?=$I+39?>','class1=正码&ids=<?=$ids?>&sqq=sqq&lxlx=0&qtqt=0.05&class3=<?=$ShowTable[$I+40-1][1]?>');"><img src="images/bvbv_02.gif" width="19" height="17" border="0"  ></a></td>
                    </tr>
                  </table></td>
                  <td><input type=checkbox id=lock<?=$I+40-1?> style="zoom:95%" title="关闭该项" onClick="UpdateRate('LOCK','lock<?=$I+40-1?>','','class1=正码&ids=<?=$ids?>&sqq=sqq&class3=<?=$ShowTable[$I+40-1][1]?>&lock='+this.checked);"  <? if ($ShowTable[$I+40-1][2]==1){echo "checked";}?>></td>
                </tr>
              </table>
...
?>   
            <td height="25" align="center"><?=$ShowTable[$I][1]?></td>
              <td height="25" align="center"><table border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td><input 
      style="HEIGHT: 18px"  class="input1" maxlength="6" size="4" value="<?=$ShowTable[$I][0]?>" name="Num_<?=$I+1?>" /></td>
                  <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td><a style="cursor:hand" onClick="UpdateRate('MODIFYRATE','Num_<?=$I+1?>','bl<?=$I?>','class1=正码&ids=<?=$ids?>&sqq=sqq&lxlx=1&qtqt=0.05&class3=<?=$ShowTable[$I][1]?>');"><img src="images/bvbv_01.gif"   width="19" height="17" border="0"></a></td>
                    </tr>
                    <tr>
                      <td><a style="cursor:hand" onClick="UpdateRate('MODIFYRATE','Num_<?=$I+1?>','bl<?=$I?>','class1=正码&ids=<?=$ids?>&sqq=sqq&lxlx=0&qtqt=0.05&class3=<?=$ShowTable[$I][1]?>');"><img src="images/bvbv_02.gif" width="19" height="17" border="0"  ></a></td>
                    </tr>
                  </table></td>
                  <td><input type=checkbox id=lock<?=$I?> style="zoom:95%" title="关闭该项" onClick="UpdateRate('LOCK','lock<?=$I?>','','class1=正码&ids=<?=$ids?>&sqq=sqq&class3=<?=$ShowTable[$I][1]?>&lock='+this.checked);"  <? if ($ShowTable[$I][2]==1){echo "checked";}?>></td>
                </tr>
              </table>
              <input name="class3_<?=$I+1?>" value="<?=$ShowTable[$I][1]?>" type="hidden" ></td>
              <td height="25" align="center"><span id=bl<?=$I?>><?=$ShowTable[$I][0]?></span></td>
              <td width="4%" align="center"><span id=gold<?=$I?>>0</span></td>
			  <? }?>
	...
	<table   border="1" align="center" cellspacing="1" cellpadding="2" bordercolordark="#FFFFFF" bordercolor="f1f1f1">
          <form action="index.php?action=rake_zm&savew=savew&ids=<?=$ids?>" name="form21" method="post" >
            <tr align="middle">
   ...
   <form action="index.php?action=rake_zm&save=save&ids=<?=$ids?>" name="form21" method="post" ><tr>
...
<SCRIPT language=javascript> 
makeRequest('index.php?action=server&class1=正码&class2=<?=$ids;?>')</script>
<?php 
