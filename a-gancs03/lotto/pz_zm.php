...
if ($_GET['zsave'] == 'zsave'){
	...
	$params = array(':zm' => $_POST['tm'], ':zm1' => $_POST['ttm1']);
	$stmt = $mydata2_db->prepare('update adad set zm=:zm,zm1=:zm1 Where id=1');
	$stmt->execute($params);
}
...
if ($_GET['save'] == 'save'){
	echo "<script>alert('系统已关闭补仓功能!');window.location.href='index.php?action=pz_zm&kithe=".$kithe."';</script>";
	exit();
}
?>
  ...
  if (http_request.readyState == 4) { 
	  ...
	  if (http_request.status == 0 || http_request.status == 200) { 
		 ...
		 for(var i=0;i<arrResult.length-1;i++) { 
			...
			num4 = "<center><button type=button class=headtd4  onmouseover=this.className='headtd3';window.status='正码';return true;onMouseOut=this.className='headtd4';window.status='正码';return true;onClick=ops('"+zmzm1+"',400,700)><font color=000000><b>"+arrTmp[2]+"</b></font></button></center>";//字段num2的值 
			...
			num11 = "<center><button type=button class=headtd4  onmouseover=this.className='headtd3';window.status='正码';return true;onMouseOut=this.className='headtd4';window.status='正码';return true;onClick=ops('"+zmzm2+"',400,700)>"+arrTmp[9]+"</button></center>";//字段num2的值 
			}else{ 
    ...
    <td width="72%" height="25" align="center" bordercolor="cccccc" bgcolor="#FDF4CA"><table width="100%" border="0" cellspacing="3" cellpadding="2">
      <tr>
        ...
        <form name=form55 action="index.php?action=pz_zm&zsave=zsave&kithe=<?=$kithe?>" method=post>
          ...
          <td width="89"><button type="button" onClick="submit()"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="width:80;height:22" ;><img src="images/icon_21x21_copy.gif" align="absmiddle" />走飞计算</button></td>
        </form>
...
<script language="JavaScript">
    function get_member_bet(obj){
        ...
        if(amount > 0){
            ...
            window.open("member_bet_list.php?six_num="+num+"&qishu="+qishu+"&class1=正码",'newwindow', 'height=500, width=800, top=100, left=250, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
        }
    }
</script>