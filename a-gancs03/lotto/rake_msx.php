...
if ($_GET['act'] == '修改'){
	...
	for ($tt = 1;$tt <= $_POST['re'];$tt++){
		...
		$params = array(':adddate' => $text, ':mrate' => $num, ':class1' => $ids, ':class2' => $class2, ':class3' => $class3);
		$stmt = $mydata2_db->prepare('update ka_bl  set adddate=:adddate,mrate=:mrate where class1=:class1 and class2=:class2 and  class3=:class3');
		$stmt->execute($params);
	}
	echo "<script>alert('修改成功!');window.location.href='index.php?action=rake_msx&ids=".$ids."';</script>";
	exit();
}
$params = array(':class1' => $ids);
$result = $mydata2_db->prepare('Select mrate,class3,class2 from ka_bl where class1=:class1   Order By ID');
