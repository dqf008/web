...
if ($_GET['save'] == 'save'){
	...
	for ($i = 1;$i <= 49;$i = $i + 1){
		...
		$params = array(':xr' => $xr, ':class3' => $class3);
		$stmt = $mydata2_db->prepare('update ka_bl set xr=:xr where class2=\'特A\' and  class3=:class3');
		$stmt->execute($params);
		$stmt = $mydata2_db->prepare('update ka_bl set xr=:xr where class2=\'特B\' and  class3=:class3');
		$stmt->execute($params);
	}
...
$result = $mydata2_db->query('Select xr, class3 from ka_bl where class2=\'特A\' order by ID');
$drop_table = array();
$y = 0;
...
<table   border="1" align="center" cellspacing="1" cellpadding="1" bordercolordark="#FFFFFF" bordercolor="f1f1f1" width="99%"> 
	 ...
	 <form name=form1 action=index.php?action=xt_nn&save=save method=post> 
   <tr > 
  ...
  <?php for ($I = 1;$I <= 10;$I = $I + 1){?>    
    ...
    <tr>
      <td height="25" colspan="12" align="center" bordercolor="cccccc">&nbsp;<font color="ff0000">统计修改</font>
        ...
        &nbsp;
      <input name="button2" class="button_c" type="button" onclick="quick0()" value="传送" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
