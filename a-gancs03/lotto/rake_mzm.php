	...
	for ($tt = 1;$tt <= 49;$tt++){
		...
		if ($_POST['dx'] == '全部'){
			if ($_GET['ids'] == '正A'){
				...
				$stmt = $mydata2_db->prepare('update ka_bl set mrate=:mrate where class2=\'正B\' and  class3=:class3');
				$stmt->execute($params);
			}else{
				$params = array(':mrate' => $_POST['bl'], ':class2' => $ids, ':class3' => $tt);
				$stmt = $mydata2_db->prepare('update ka_bl set mrate=:mrate where class2=:class2 and  class3=:class3');
				...
				$stmt = $mydata2_db->prepare('update ka_bl set mrate=:mrate where class2=\'正A\' and  class3=:class3');
				$stmt->execute($params);
			}
		}
...
if ($_GET['savew'] == 'savew'){
	...
	for ($f = 0;$f < $ss4;$f++){
		if ($mv == 0){
			...
			$stmt = $mydata2_db->prepare('update ka_bl set mrate=rate-:rate where class1=\'正码\'  and   class3=:class3');
			$stmt->execute($params);
		}else{
			$params = array(':rate' => $money, ':class3' => $pc4[$f]);
			$stmt = $mydata2_db->prepare('update ka_bl set mrate=rate+:rate where class1=\'正码\'  and   class3=:class3');
