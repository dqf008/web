...
if ($_GET['act'] == '修改'){
	...
	for ($tt = 1;$tt <= 8;$tt++){
		...
		$params = array(':adddate' => $text, ':mrate' => $num, ':class2' => $class2, ':class3' => $class3);
		$stmt = $mydata2_db->prepare('update ka_bl  set adddate=:adddate,mrate=:mrate where class1=\'连码\' and class2=:class2 and  class3=:class3');
		$stmt->execute($params);
	}
	
	echo  "<script>alert('修改成功!');window.location.href='index.php?action=rake_mlm&ids=".$ids."';</script>";
	exit();
}

$result = $mydata2_db->query('Select mrate,class3,class2 from ka_bl where class1=\'连码\'   Order By ID');
$ShowTable = array();
$y = 0;
...
<form name="form1" method="post" action="index.php?action=rake_mlm&act=修改&ids=<?=$ids?>"> <TBODY>
  <TR align=center>
  ...
  <TR>
	...
	<TD align="center" bordercolor="cccccc"><INPUT  style="HEIGHT: 18px"  class="input1" id=lm1 size=10 value='<?=$ShowTable[0][0]?>' name=lm1>
	
	<input  name="class3_1" value="<?=$ShowTable[0][1]?>" type="hidden" >
	<input name="class2_1" value="<?=$ShowTable[0][2]?>" type="hidden" >				</TD>
  </TR>
  <TR>
	<TD width="20%" rowSpan=2 bordercolor="cccccc">二中特</TD>
	<TD width="14%"  height=26 bordercolor="cccccc">中特</TD>
	<TD align="center" bordercolor="cccccc" ><INPUT  style="HEIGHT: 18px"  class="input1" id=lm2 size=10 value='<?=$ShowTable[1][0]?>' name=lm2>
	<input name="class3_2" value="<?=$ShowTable[1][1]?>" type="hidden" >
	<input name="class2_2" value="<?=$ShowTable[1][2]?>" type="hidden" ></TD>
  </TR>
  <TR>
	<TD height=24 bordercolor="cccccc">中二</TD>
	<TD align="center" bordercolor="cccccc" ><INPUT   style="HEIGHT: 18px"  class="input1"  id=lm3 size=10 value='<?=$ShowTable[2][0]?>' name=lm3>
	<input name="class3_3" value="<?=$ShowTable[2][1]?>" type="hidden" >
	<input name="class2_3" value="<?=$ShowTable[2][2]?>" type="hidden" ></TD>
  </TR>
  <TR>
	<TD height=26 colSpan=2 bordercolor="cccccc">特串</TD>
	<TD align="center" bordercolor="cccccc" ><INPUT  style="HEIGHT: 18px"  class="input1" id=lm4 size=10 value=<?=$ShowTable[3][0]?> name=lm4>
	<input name="class3_4" value="<?=$ShowTable[3][1]?>" type="hidden" >
	<input name="class2_4" value="<?=$ShowTable[3][2]?>" type="hidden" ></TD>
  </TR>
  <TR>
	<TD height=26  colSpan=2 bordercolor="cccccc">三全中</TD>
	<TD align="center" bordercolor="cccccc" ><INPUT  style="HEIGHT: 18px"  class="input1" id=lm5 size=10 value='<?=$ShowTable[4][0]?>' name=lm5>
	<input name="class3_5" value="<?=$ShowTable[4][1]?>" type="hidden" >
	<input name="class2_5" value="<?=$ShowTable[4][2]?>" type="hidden" ></TD>
  </TR>
  <TR>
	<TD  rowSpan=2 bordercolor="cccccc">三中二</TD>
	<TD  height=25 bordercolor="cccccc">中二</TD>
	<TD align="center" bordercolor="cccccc" ><INPUT   style="HEIGHT: 18px"  class="input1" id=lm6 size=10 value='<?=$ShowTable[5][0]?>' name=lm6>
	<input name="class3_6" value="<?=$ShowTable[5][1]?>" type="hidden" >
	<input name="class2_6" value="<?=$ShowTable[5][2]?>" type="hidden" ></TD>
  </TR>
  <TR>
	<TD  height=30 bordercolor="cccccc">中三</TD>
	<TD align="center" bordercolor="cccccc" ><INPUT  style="HEIGHT: 18px"  class="input1" id=lm7 size=10 value='<?=$ShowTable[6][0]?>' name=lm7>
	<input name="class3_7" value="<?=$ShowTable[6][1]?>" type="hidden" >
	<input name="class2_7" value="<?=$ShowTable[6][2]?>" type="hidden" ></TD>
  </TR>

<TR>
	<TD height=26  colSpan=2 bordercolor="cccccc">四中一</TD>
	<TD align="center" bordercolor="cccccc" ><INPUT  style="HEIGHT: 18px"  class="input1" id=lm8 size=10 value='<?=$ShowTable[7][0]?>' name=lm8>
	<input name="class3_8" value="<?=$ShowTable[7][1]?>" type="hidden" >
	<input name="class2_8" value="<?=$ShowTable[7][2]?>" type="hidden" ></TD>
  </TR>

  <TR>
	<TD height="30" colspan="3" bordercolor="cccccc"><table width="98" border="0" align="center" cellpadding="0" cellspacing="0">
	  <tr>
