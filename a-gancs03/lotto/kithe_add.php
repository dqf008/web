...
if ($_POST['tv5']=="是"){	
	...
	$params = array(':nn' => $_POST['nn']);
	$result = $mydata2_db->prepare('select count(*) from ka_kithe where nn=:nn   order by id');
	$result->execute($params);
	$row11 = $result->fetchColumn();
	if (0 < $row11){
		echo "<script>alert('对不起！这一“期”已被开过，请重新输入!');history.back();</script>";
		exit();
		exit();
	}
	$result = $mydata2_db->query('select nn from ka_kithe where na=0  order by id');
	$row11 = $result->fetchColumn();
	if (0 < $row11){
		echo "<script>alert('对不起！第[".$row11."]期还未开奖！请先开完奖在添加新的盘口!');history.back();</script>";
		exit();
	}
	
	$result22 = $mydata2_db->query('update ka_bl set gold=0');
	$params = array(':best' => $best, ':nn' => $_POST['nn'], ':nd' => $_POST['nd'], ':kitm' => $_POST['kitm'], ':kizt' => $_POST['kizt'], ':kizm' => $_POST['kizm'], ':kizm6' => $_POST['kizm6'], ':kigg' => $_POST['kigg'], ':kilm' => $_POST['kilm'], ':kisx' => $_POST['kisx'], ':kibb' => $_POST['kibb'], ':kiws' => $_POST['kiws'], ':zfbdate' => $_POST['zfbdate'], ':kitm1' => $_POST['kitm1'], ':kizt1' => $_POST['kizt1'], ':kizm1' => $_POST['kizm1'], ':kizm61' => $_POST['kizm61'], ':kigg1' => $_POST['kigg1'], ':kilm1' => $_POST['kilm1'], ':kisx1' => $_POST['kisx1'], ':kibb1' => $_POST['kibb1'], ':kiws1' => $_POST['kiws1'], ':zfbdate1' => $_POST['zfbdate1'], ':zfb' => $_POST['zfb']);
	$sql = 'INSERT INTO  ka_kithe set best=:best,nn=:nn,nd=:nd,kitm=:kitm,kizt=:kizt,kizm=:kizm,kizm6=:kizm6,kigg=:kigg,kilm=:kilm,kisx=:kisx,kibb=:kibb,kiws=:kiws,zfbdate=:zfbdate,kitm1=:kitm1,kizt1=:kizt1,kizm1=:kizm1,kizm61=:kizm61,kigg1=:kigg1,kilm1=:kilm1,kisx1=:kisx1,kibb1=:kibb1,kiws1=:kiws1,zfbdate1=:zfbdate1,zfb=:zfb,n1=0,n2=0,n3=0,n4=0,n5=0,n6=0,na=0,lx=0 ';
	$stmt = $mydata2_db->prepare($sql);
	$stmt->execute($params);
	echo "<script>alert('盘口添加成功!');window.location.href='index.php?action=kithe';</script>";
	exit();
}
