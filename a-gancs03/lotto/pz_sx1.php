...
if ($_GET['zsave'] == 'zsave'){
	...
	$params = array(':ztws' => $_POST['tm'], ':ztws1' => $_POST['tm1']);
	$stmt = $mydata2_db->prepare('update adad set ztws=:ztws,ztws1=:ztws1  Where id=1');
	$stmt->execute($params);
}
...
if ($_GET['save'] == 'save'){
	echo "<script>alert('系统已关闭补仓功能!');window.location.href='index.php?action=pz_sx1&ids=".$ids."&kithe=".$kithe."';</script>";
	exit();
}
?>
...
while ($image = $result->fetch()){
	echo "<OPTION value=index.php?action=pz_sx1&ids=".$ids."&kithe=".$image['nn'];
	if ($kithe!="") {
		if ($kithe==$image['nn']) {
	...
	echo ">第".$image['nn']."期</OPTION>";
	  ...
	  <form name=form55 action="index.php?action=pz_sx1&zsave=zsave&kithe=<?=$kithe;?>&ids=<?=$ids;?>" method=post> 
		<td width="85" align="right" nowrap><?=$sxtitle;?>：</td> 
		<td width="30"><span class="STYLE2"> 
		  <input name="tm" class="input1" id="tm" value='<?=$ztws;?>' size="10" /> 
		</span></td> 
		<td width="50" align="right" nowrap>退水：</td> 
		<td width="63"><input name="tm1" class="input1" type="text" id="tm1" value="<?=$ztws1;?>" size="5"></td> 
	  ...
	  <td width="697"><div align="right"> 
		...
		<button type="button" onClick="javascript:location.href='index.php?action=pz_sx1&ids=一肖&kithe=<?=$kithe;?>';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:ff0000;'>一肖</span></button> 
		<button type="button" onClick="javascript:location.href='index.php?action=pz_ztws&ids=正特尾数&kithe=<?=$kithe;?>';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:000000;'>正特尾数</span></button> 
...
<form action="index.php?action=pz_sx1&save=save&ids=<?=$ids;?>&kithe=<?=$kithe;?>"  method="post" name="form1" ><?php include 'd1.php';?></form> 
</div> 
