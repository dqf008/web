...
if ($_GET['act'] == '修改'){
	...
	for ($tt = 1;$tt <= 5;$tt++){
		...
		$stmt = $mydata2_db->prepare('update ka_bl  set adddate=:adddate,rate=:rate where class1=\'五行\' and class2=:class2 and  class3=:class3');
		$stmt->execute($params);
	}
	
	echo "<script>alert('修改成功!');window.location.href='index.php?action=rake_wx&ids=".$ids."';</script>";
	exit();
}

$result = $mydata2_db->query('Select rate,class3,class2,locked from ka_bl where class1=\'五行\'   Order By ID');
$ShowTable = array();
$y = 0;
  ...
  { 
	  case "MODIFYRATE":  	  //更新赔率 
		  { 
			  ...
			  if (strResult!="") 
			  { 
				  makeRequest('index.php?action=server&class1=五行&class2=<?=$ids;?>') 
...
.ball_r {font-weight:bold;background:#f1f1f1 url(images/r1.gif) no-repeat center;COLOR: #000000;font-size: 16;
  font-weight: bold;} 
.ball_b {font-weight:bold;background:#f1f1f1 url(images/b1.gif) no-repeat center;COLOR: #000000;font-size: 16;
  font-weight: bold;} 
.ball_g {font-weight:bold;background:#f1f1f1 url(images/g1.gif) no-repeat center;COLOR: #000000;font-size: 16;
  font-weight: bold;} 
--> 
</style> 
...
<table border="1" align="center" cellspacing="1" cellpadding="2" bordercolor="f1f1f1" bordercolordark="#FFFFFF" width="99%"> 
 <form name="form1" method="post" action="index.php?action=rake_wx&act=修改&ids=<?=$ids;?>"> <TBODY> 
	<TR align=center> 
	  <TD height=26 colSpan=3 bordercolor="cccccc" bgcolor="#FDF4CA" >类型</TD> 
	  <TD width="80" bordercolor="cccccc" bgcolor="#FDF4CA" > 
	  赔率/封号</TD> 
	  <TD width="80" bordercolor="cccccc" bgcolor="#FDF4CA" >当前赔率</TD> 
	  <TD width="80" bordercolor="cccccc" bgcolor="#FDF4CA" >下注金额</TD> 
	</TR>
	<?php 
	$bbffx = 25;
	for ($I = 0;$I <= 4;$I = $I + 1){
	...
	?>  
	<TR> 
	  <TD width="80" align="center" bordercolor="cccccc"><?=$ShowTable[$I][2];?></TD> 
	  <TD width="80" align="center" bordercolor="cccccc"><?=$ShowTable[$I][1];?></TD> 
	  <TD align="center" bordercolor="cccccc" bgcolor="f1f1f1"><table align=left> 
		<tr>
		<?php 
		$params = array(':id' => $bbffx);
		$result333 = $mydata2_db->prepare('Select m_number from ka_sxnumber where id=:id order by id');
		$result333->execute($params);
		$image333 = $result333->fetch();
		
		$xxm = explode(',',$image333['m_number']);
		$ssc = count($xxm);
		for ($j = 0;$j < $ssc;$j = $j + 1){
		?>                     
			<td align=middle   height="32" width="32" class="ball_<?=intval($xxm[$j])?>"><img src="images/num<?=intval($xxm[$j])?>.gif" /></td>
		<? } ?>
		</tr>
		</table>
		</TD>
		<TD width="80" align="center" bordercolor="cccccc"><table border="0" cellspacing="0" cellpadding="0">
		  <tr>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
				  <td><a style="cursor:hand" onClick="UpdateRate('MODIFYRATE','lm<?=$I+1?>','bl<?=$I?>','class1=五行&ids=<?=$ShowTable[$I][2]?>&sqq=sqq&lxlx=1&qtqt=0.01&class3=<?=$ShowTable[$I][1]?>');"><img src="images/bvbv_01.gif"   width="19" height="17" border="0"></a></td>
				</tr>
				<tr>
				  <td><a style="cursor:hand" onClick="UpdateRate('MODIFYRATE','lm<?=$I+1?>','bl<?=$I?>','class1=五行&ids=<?=$ShowTable[$I][2]?>&sqq=sqq&lxlx=0&qtqt=0.01&class3=<?=$ShowTable[$I][1]?>');"><img src="images/bvbv_02.gif" width="19" height="17" border="0"  ></a></td>
				</tr>
			</table></td>
			<td><input name="checkbox" type=checkbox id=lock<?=$I?> style="zoom:95%" title="关闭该项" onClick="UpdateRate('LOCK','lock<?=$I?>','','class1=五行&ids=<?=$ids?>&sqq=sqq&class3=<?=$ShowTable[$I][1]?>&lock='+this.checked);" value="TRUE"  <? if ($ShowTable[$I][3]==1){echo "checked";}?>></td>
		  </tr>
		</table></TD>
		<TD width="80" align="center" bordercolor="cccccc"><span id=bl<?=$I?>><?=$ShowTable[$I][0]?></span></TD>
		<TD width="80" align="center" bordercolor="cccccc" ><span id=gold<?=$I?>>0</span></TD>
	  </TR>
