...
if ($_GET['save'] == 'save'){
	echo "<script>alert('系统已关闭补仓功能!');window.location.href='index.php?action=pz_wsl&ids=".$ids."&kithe=".$kithe."';</script>";
	exit();
}
?>
...
if (i!=(arrResult.length-2)){ 
var zmzm1="index.php?action=look&kithe=<?=$kithe;?>&class2="+arrTmp[14]+"&lx=0&id=13&class3="+arrTmp[12];
...
{ 
  ...
  { 
	  case "MODIFYRATE":  	  //更新赔率 
		...
		makeRequest('index.php?action=server_wx&class1=尾数连&class2=<?=$ids;?>&kithe=<?=$kithe;?>') 
...
<tr > 
  <td ></td> 
  <td align="right">    
		<button onClick="javascript:location.href='index.php?action=pz_wsl&ids=二尾连中&kithe=<?=$kithe;?>';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm1 STYLE='color:<?=$z1color;?>;'>二尾连中</span></button> 
		<button onClick="javascript:location.href='index.php?action=pz_wsl&ids=三尾连中&kithe=<?=$kithe;?>';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:<?=$z2color;?>;'>三尾连中</span></button> 
		<button onClick="javascript:location.href='index.php?action=pz_wsl&ids=四尾连中&kithe=<?=$kithe;?>';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:<?=$z3color;?>;'>四尾连中</span></button> 
		<button onClick="javascript:location.href='index.php?action=pz_wsl&ids=二尾连不中&kithe=<?=$kithe;?>';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:<?=$z4color;?>;'>二尾连不中</span></button> 
		<button onClick="javascript:location.href='index.php?action=pz_wsl&ids=三尾连不中&kithe=<?=$kithe;?>';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:<?=$z5color;?>;'>三尾连不中</span></button> 
		<button onClick="javascript:location.href='index.php?action=pz_wsl&ids=四尾连不中&kithe=<?=$kithe;?>';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:<?=$z5color;?>;'>四尾连不中</span></button> 
</td> 
</tr> 
</table> 
...
while ($image = $result->fetch()){
	echo "<OPTION value=index.php?action=pz_wsl&ids=".$ids."&kithe=".$image['nn'];
	if ($kithe!="") {
		 if ($kithe==$image['nn']) {
	...
	echo ">第".$image['nn']."期</OPTION>";
	  ...
	  <form name=form55 action="index.php?action=pz_wsl&zsave=zsave&kithe=<?=$kithe;?>&ids=<?=$ids;?>" method=post> 
...
<form action="index.php?action=pz_wsl&save=save&ids=<?=$ids;?>&kithe=<?=$kithe;?>"    method="post" name="form1" ><?php include 'd1.php';?></form> 
</div> 
...
<SCRIPT language=javascript> 
makeRequest('index.php?action=server_lx&class1=尾数连&class2=<?=$ids;?>&kithe=<?=$kithe;?>')</script>
    <script language="JavaScript">
        function get_member_bet(obj){
            ...
            if(amount > 0){
                ...
                window.open("member_bet_list.php?six_num="+num+"&qishu="+qishu+"&class1=尾数连",'newwindow', 'height=500, width=800, top=100, left=250, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
            }
        }
