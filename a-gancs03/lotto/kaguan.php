...
<table width="100%" border="0" cellspacing="0" cellpadding="5"> 
  <tr class="tbtitle"> 
	...
	<td width="49%">
	<div align="right"> 
	<button type="button" onclick="javascript:location.href='index.php?action=kaguan&amp;id=0'"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ;><img src="images/add.gif" width="16" height="16" align="absmiddle" /><font <? if ($id==0) {?>color=ff0000<? }else{?>color=000000<? }?>>所有股东</font></button>&nbsp;
	<button type="button" onclick="javascript:location.href='index.php?action=kaguan&amp;id=2'"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ;><img src="images/add.gif" width="16" height="16" align="absmiddle" /><font <? if ($id==2) {?>color=ff0000<? }else{?>color=000000<? }?>>开启</font></button>&nbsp;
	<button type="button" onclick="javascript:location.href='index.php?action=kaguan&amp;id=1'"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ;><img src="images/add.gif" width="16" height="16" align="absmiddle" /><font <? if ($id==1) {?>color=ff0000<? }else{?>color=000000<? }?>>禁止</font></button>&nbsp;
	<!--button type="button" onclick="javascript:location.href='index.php?action=guan_add'"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="width:80;height:22" ;><img src="images/add.gif" width="16" height="16" align="absmiddle" />添加股东</button--> 
	</div></td> 
  </tr> 
  <tr > 
...
while ($image = $result->fetch()){
	$mumu = 0;
	$params = array(':guanid' => $image['id']);
	$stmt = $mydata2_db->prepare('Select SUM(cs) As sum_m  From ka_guan Where lx=2 and   guanid=:guanid order by id desc');
	$stmt->execute($params);
	...
	$stmt = $mydata2_db->prepare('Select SUM(sum_m) As sum_m1   From ka_tan Where kithe=:kithe  and   username=:username order by id desc');
	$stmt->execute($params);
	$rsw = $stmt->fetchColumn();
	if ($rsw != ''){
		$mkmk = $rsw;
	...
	$memnum2 = 0;
	$params = array(':guanid' => $image['id']);
	$stmt = $mydata2_db->prepare('Select Count(ID) As memnum From ka_guan Where lx=2 and guanid=:guanid order by id desc');
	$stmt->execute($params);
	$rsw = $stmt->fetchColumn();
	if ($rsw != ''){
		$memnum = $rsw;
	...
	$params = array(':guanid' => $image['id']);
	$stmt = $mydata2_db->prepare('Select Count(ID) As memnum1 From ka_guan Where lx=3 and guanid=:guanid order by id desc');
	$stmt->execute($params);
	$rsw = $stmt->fetchColumn();
	if ($rsw != ''){
		$memnum1 = $rsw;
	...
	$params = array(':guanid' => $image['id']);
	$stmt = $mydata2_db->prepare('Select Count(ID) As memnum2 From ka_mem Where guanid=:guanid order by id desc');
	$stmt->execute($params);
	$rsw = $stmt->fetchColumn();
	if ($rsw != ''){
		$memnum2 = $rsw;
