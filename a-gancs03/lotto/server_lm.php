...
while ($rs = $result->fetch()){
	...
	$RsB = $result4->fetch();
	if ($class2 == '三中二'){
		$params = array(':class1' => $rs['class1'], ':class2' => $rs['class2']);
		$sql = 'Select * from ka_bl   where   class1=:class1 and  class2=:class2 and class3=\'中三\' ';
	}else if ($class2 == '二中特'){
		$params = array(':class1' => $rs['class1'], ':class2' => $rs['class2']);
		$sql = 'Select * from ka_bl   where   class1=:class1 and  class2=:class2 and class3=\'中二\' ';
	}else{
		$params = array(':class1' => $rs['class1'], ':class3' => $rs['class2']);
		$sql = 'Select * from ka_bl   where   class1=:class1 and  class3=:class3 ';
	}
	...
	if ($Rsbl['rate'] != ''){
		$sum_bl[$ii] = '<a style="cursor:hand" onClick="UpdateRate(\'MODIFYRATE\',\'lm\',\'bl' . $ii . '\',\'class1=' . $Rsbl['class1'] . '&ids=' . $Rsbl['class2'] . '&sqq=sqq&lxlx=0&qtqt=0.1&class3=' . $Rsbl['class3'] . '\');"><font color=0000ff>↓</font></a><span id=bl' . $ii . '>' . $Rsbl['rate'] . '</span><a style="cursor:hand" onClick="UpdateRate(\'MODIFYRATE\',\'lm\',\'bl' . $ii . '\',\'class1=' . $Rsbl['class1'] . '&ids=' . $Rsbl['class2'] . '&sqq=sqq&lxlx=1&qtqt=0.1&class3=' . $Rsbl['class3'] . '\');"><font color=ff0000>↑</font></a>';
	}else{
		$sum_bl[$ii] = 0;
	}
...
for ($i = 0;$i < $ii;$i++){
	$sum_sx[$i] = $sum_ma[$i] - $sum_ds[$i] - ($sum_pz1[$i] * (1 - ($lm1 / 100))) - (($sum_ma[$i] - $sum_pz1[$i]) * $sum_mbl[$i]);
}
$b = 0;
...
for ($i = 0;$i < $ii;$i++){
	if ((0 <= $sum_sx[$i] + $ztm_tm) || ($sum_mbl[$i] == 0)){
		...
		$ffxx = (-$sum_sx[$i] - $ztm_tm) / (($sum_mbl[$i] + ($lm1 / 100)) - 1);
	}
	$bl = round($ffxx, 0);
	if (1 <= $ffxx){
		$fg = $fg + 1;
