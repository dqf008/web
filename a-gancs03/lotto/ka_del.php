...
if ($_GET['act'] == '添加'){
	if (($Current_Kithe_Num <= $_POST['key']) || ($Current_Kithe_Num <= $_POST['key1'])){
		echo "<script type='text/javascript'>alert('对不起，当期数据不能删除！');history.back();</script>";
		exit();
	}
	
	if (($_POST['key'] == '') || ($_POST['key1'] == '')){
		echo "<script type='text/javascript'>alert('请输入期数！');history.back();</script>";
		exit();
	}
	
	$params = array(':skithe' => $_POST['key'], ':ekithe' => $_POST['key1']);
	$stmt = $mydata2_db->prepare('Delete from ka_tan where kithe>=:skithe and kithe<=:ekithe ');
	$stmt->execute($params);
	$stmt = $mydata2_db->prepare('Delete from ka_tong where kithe>=:skithe and kithe<=:ekithe ');
	$stmt->execute($params);
		  ...
		  <tr> 
			...
			<td align="center" colspan="6"><table width="100%"  border="0" cellspacing="0" cellpadding="0"> 
				<tr> 
				  ...
				  <td width="100" align="center"><input type="submit" value="确定删除" name="B1"   class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="width:80;height:22" ></td> 
				  <td>&nbsp;</td> 
				</tr> 
			</table></td> 
		  </tr> 
