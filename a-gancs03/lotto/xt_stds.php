...
if ($_GET['save'] == 'save'){
	...
	for ($I = 0;$I <= count($rid);$I = $I + 1){
		$params = array(':drop_value' => $rate[$I], ':drop_unit' => $ratedrop[$I], ':low_drop' => $low_drop[$I], ':ID' => $rid[$I]);
		$sql = 'Update ka_drop Set drop_value=:drop_value,drop_unit=:drop_unit,low_drop=:low_drop where ID=:ID';
...
<?php for ($I = 0;$I <= $drop_count;$I = $I + 1){?>      
	<tr > 
	  <td width="22%" align="center" bordercolor="cccccc" bgcolor="#FDF4CA" ><span class="STYLE2"><strong><?=$drop_table[$I][1];?></strong></span></td> 
	  <td width="23%" align="center" bordercolor="cccccc" ><input type="hidden" name="drop_id[]" value='<?=$drop_table[$I][0];?>' /> 
		  <input type="hidden" name="drop_name[]" value='<?=$drop_table[$I][1];?>' /> 
		  <input size="10" value='<?=$drop_table[$I][2];?>' name="rate[]" /></td> 
	  <td width="28%" align="center" bordercolor="cccccc" ><input  size="10" value='<?=$drop_table[$I][3];?>' name="ratedrop[]" /></td> 
	  <td width="27%" align="center" bordercolor="cccccc"><input  size="10" value='<?=$drop_table[$I][4];?>' name="low_drop[]" /></td> 
	</tr>
<?php }?>   
   <tr > 
	  <td colspan="4" align="center" bordercolor="cccccc" ><button onclick="submit()"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="width:100;height:22" ><img src="images/icon_21x21_copy.gif" align="absmiddle" />确认修改</button> 
	<button onclick="javascript:location.reload();"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="width:80;height:22" ><img src="images/icon_21x21_info.gif" align="absmiddle" />刷新</button></td> 
	</tr> 
  </tbody></form > 
</table> 
