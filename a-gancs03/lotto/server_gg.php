...
$result = $mydata2_db->prepare('select distinct(class3),class1,class2,rate   from   ka_tan where Kithe=:Kithe and  class1=:class1   order by class3 desc');
...
for ($i = 0;$i < $ii;$i++){
	if ((0 <= $sum_sx[$i] + $ztm_tm) || ($sum_bl[$i] == 0)){
		$ffxx = 0;
	}else{
	...
	if (1 <= $ffxx){
		$fg = $fg + 1;
		if ($i == 0){
			$sum_pz[0] = '<button class=headtd4  onmouseover=this.className=\'headtd3\';window.status=\'过关\';return true;onMouseOut=this.className=\'headtd4\';window.status=\'过关\';return true;onclick=show_win(\'' . $class_3[0] . '\',\'' . $bl . '\',\'' . $sum_bl[0] . '\',\'' . $class1 . '\',\'' . $class_2[0] . '\')    ><font color=ff6600>走飞</font>  ' . $bl . '</button>';
		}else{
			$sum_pz[$i] = '<button class=headtd4  onmouseover=this.className=\'headtd3\';window.status=\'过关\';return true;onMouseOut=this.className=\'headtd4\';window.status=\'过关\';return true;onclick=show_win(\'' . $class_3[$i] . '\',\'' . $bl . '\',\'' . $sum_bl[$i] . '\',\'' . $class1 . '\',\'' . $class_2[$i] . '\')    ><font color=ff6600>走飞</font>  ' . $bl . '</button>';
		}
	}else{
		$sum_pz[$i] = '0';
		$sum_pz[$i] = '<button class=headtd4  onmouseover=this.className=\'headtd3\';window.status=\'过关\';return true;onMouseOut=this.className=\'headtd4\';window.status=\'过关\';return true;onclick=show_win(\'' . $class_3[$i] . '\',\'' . $bl . '\',\'' . $sum_bl[$i] . '\',\'' . $class1 . '\',\'' . $class_2[$i] . '\')    ><font color=ff6600>走飞</font>  ' . $bl . '</button>';
...
for ($i = 0;$i < $ii;$i++){
	$blbl .= '##' . $sum_tm[$i] . '@@@' . $sum_re[$i] . '注@@@' . $sum_m[$i] . '@@@' . $sum_ma[$i] . '@@@' . $sum_mb[$i] . '@@@' . round($sum_ds[$i], 2) . '@@@' . round($sum_xx[$i], 2) . '@@@' . round($sum_sx[$i], 2) . '@@@' . $sum_pz[$i] . '@@@' . $sum_pz1[$i] . '@@@' . $sum_bl[$i] . '@@@' . $fg . '@@@' . $sum_tm[$i] . '@@@' . $sum_color[$i] . '@@@' . $class_2[$i] . '@@@' . $class_3[$i] . '###';
}
