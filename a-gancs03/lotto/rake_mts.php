...
if ($_GET['act'] == '修改'){
	...
	for ($tt = 1;$tt <= 5;$tt++){
		...
		$stmt = $mydata2_db->prepare('update ka_bl  set adddate=:adddate,mrate=:mrate where class1=\'头数\' and class2=:class2 and  class3=:class3');
		$stmt->execute($params);
	}
	echo "<script>alert('修改成功!');window.location.href='index.php?action=rake_mts&ids=".$ids."';</script>";
	exit();
}
$result = $mydata2_db->query('Select mrate,class3,class2 from ka_bl where class1=\'头数\'   Order By ID');
$ShowTable = array();
$y = 0;
  ...
  { 
	  case "MODIFYRATE":  	  //更新赔率 
		  { 
			  ...
			  if (strResult!="") 
			  { 
				  makeRequest('index.php?action=server&class1=头数&class2=<?=$ids;?>') 
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
 ...
 <tr >
  ...
  <table border="1" align="center" cellspacing="1" cellpadding="2" bordercolor="f1f1f1" bordercolordark="#FFFFFF" width="99%">
   <form name="form1" method="post" action="index.php?action=rake_mts&act=修改&ids=<?=$ids?>"> <TBODY>
	  <TR align=center>
