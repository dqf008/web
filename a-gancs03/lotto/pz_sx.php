...
if ($_GET['zsave'] == 'zsave'){
	...
	$params = array(':spx' => $_POST['tm'], ':spx1' => $_POST['tm1']);
	$stmt = $mydata2_db->prepare('update adad set spx=:spx,spx1=:spx1  Where id=1');
	$stmt->execute($params);
}
...
if ($_GET['save'] == 'save'){
	echo "<script>alert('系统已关闭补仓功能!');window.location.href='index.php?action=pz_sx&ids=". $ids ."&kithe=".$kithe."';</script>";
	exit();
}
?>
...
if (i!=(arrResult.length-2)){ 
var zmzm1="index.php?action=look&kithe=<?=$kithe;?>&class2=<?=$ids;?>&lx=0&id=5&class3="+arrTmp[12];
...
var zmzm2="index.php?action=look&lx=1&kithe=<?=$kithe;?>&class2=<?=$ids;?>&id=5&class3="+arrTmp[12];
...
{ 
  ...
  { 
   ...
   makeRequest('index.php?action=server_sx&class1=生肖&class2=<?=$ids;?>&kithe=<?=$kithe;?>') 
...
while ($image = $result->fetch()){
	echo "<OPTION value=index.php?action=pz_sx&ids=".$ids."&kithe=".$image['nn'];
	if ($kithe!="") {
		if ($kithe==$image['nn']) {
	...
	echo ">第".$image['nn']."期</OPTION>";
	  ...
	  <form name=form55 action="index.php?action=pz_sx&zsave=zsave&kithe=<?=$kithe;?>&ids=<?=$ids;?>" method=post> 
		<td width="85" align="right" nowrap><?=$sxtitle;?>：</td> 
		<td width="30"><span class="STYLE2"> 
		  <input name="tm" class="input1" id="tm" value='<?=$spx;?>' size="10" /> 
		</span></td> 
		<td width="50" align="right" nowrap>退水：</td> 
		<td width="63"><input name="tm1" class="input1" type="text" id="tm1" value="<?=$spx1;?>" size="5"></td> 
	  ...
	  <td width="697"><div align="right"> 
		...
		<button type="button" onClick="javascript:location.href='index.php?action=pz_sx&ids=特肖&kithe=<?=$kithe;?>';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm1 STYLE='color:<?=$z1color;?>;'>特肖</span></button> 
		<button type="button" onClick="javascript:location.href='index.php?action=pz_sx&ids=二肖&kithe=<?=$kithe;?>';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:<?=$z6color;?>;'>二肖</span></button> 
		<button type="button" onClick="javascript:location.href='index.php?action=pz_sx&ids=三肖&kithe=<?=$kithe;?>';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:<?=$z7color;?>;'>三肖</span></button> 
		 
		 
		<button type="button" onClick="javascript:location.href='index.php?action=pz_sx&ids=四肖&kithe=<?=$kithe;?>';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:<?=$z3color;?>;'>四肖</span></button> 
		<button type="button" onClick="javascript:location.href='index.php?action=pz_sx&ids=五肖&kithe=<?=$kithe;?>';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:<?=$z4color;?>;'>五肖</span></button> 
		<button type="button" onClick="javascript:location.href='index.php?action=pz_sx&ids=六肖&kithe=<?=$kithe;?>';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:<?=$z5color;?>;'>六肖</span></button> 
	  </div></td> 
	</tr> 
  </table></td> 
</tr> 
...
<form action="index.php?action=pz_sx&save=save&ids=<?=$ids;?>&kithe=<?=$kithe;?>"    method="post" name="form1" ><?php include 'd1.php';?></form> 
</div> 
...
<SCRIPT language=javascript> 
makeRequest('index.php?action=server_sx&class1=生肖&class2=<?=$ids;?>&kithe=<?=$kithe;?>')</script>
    <script language="JavaScript">
        function get_member_bet(obj){
            ...
            if(amount > 0){
                ...
                window.open("member_bet_list.php?six_num="+num+"&qishu="+qishu+"&class1=生肖",'newwindow', 'height=500, width=800, top=100, left=250, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
            }
        }
