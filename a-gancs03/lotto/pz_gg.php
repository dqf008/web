...
if ($_GET['zsave'] == 'zsave'){
	...
	$params = array(':zlm' => $_POST['tm'], ':zlm1' => $_POST['tm1']);
	$stmt = $mydata2_db->prepare('update adad set zlm=:zlm,zlm1=:zlm1  Where id=1');
	$stmt->execute($params);
}
...
if ($_GET['save'] == 'save'){
	echo "<script>alert('系统已关闭补仓功能!');window.location.href='index.php?action=pz_gg&ids=".$ids."&kithe=".$kithe."';</script>";
	exit();
}
?>
  ...
  if (http_request.readyState == 4) { 
	  ...
	  if (http_request.status == 0 || http_request.status == 200) { 
		...
		for(var i=0;i<arrResult.length-1;i++){ 
			...
			num4 = "<center onclick='get_member_bet($(this));' data-id='"+c3+"'><button type=button class=headtd4  onmouseover=this.className='headtd3';window.status='过关';return true;onMouseOut=this.className='headtd4';window.status='过关';return true;onClick=ops('"+zmzm1+"',400,700)>"+arrTmp[2]+"</button></center>";//字段num2的值
			}else{num4 = "<center onclick='get_member_bet($(this));' data-id='"+c3+"'>"+arrTmp[2]+"</center>";
			} 
			...
			num11 = "<center><button type=button class=headtd4  onmouseover=this.className='headtd3';window.status='过关';return true;onMouseOut=this.className='headtd4';window.status='过关';return true;onClick=ops('"+zmzm2+"',400,700)>"+arrTmp[9]+"</button></center>";//字段num2的值 
			}else{ 
...
<form action="index.php?action=pz_gg&save=save&ids=<?=$ids;?>&kithe=<?=$kithe;?>"    method="post" name="form1" ><?php include 'd1.php';?></form> 
</div> 
    ...
    <script language="JavaScript">
        function get_member_bet(obj){
            ...
            if(amount > 0){
                ...
                window.open("member_bet_list.php?six_num="+num+"&qishu="+qishu+"&class1=过关",'newwindow', 'height=500, width=800, top=100, left=250, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
            }
        }
