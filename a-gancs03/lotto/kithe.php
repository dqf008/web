...
if ($_GET['act'] == 'del'){
	$result = $mydata2_db->query('select count(*) from ya_kithe');
	$num = $result->fetchColumn();
	if ($num <= 1){
		echo "<script>alert('没有多余的预设,不能删除!');history.back();</script>";
		exit();
	}else{
		$params = array(':yid' => intval($_GET['yid']));
		$stmt = $mydata2_db->prepare('delete from ya_kithe where id=:yid');
		$stmt->execute($params);
		echo "<script>alert('预设删除成功!');window.location.href='index.php?action=kithe';</script>";
	}
}
...
if ($_GET['act'] == '添加'){
	...
	$resultf = $mydata2_db->query('select * from ya_kithe order by nn');
	$ii = 1;
	while ($imagef = $resultf->fetch()){
		$nnd1 = $_POST['nn'] + $ii;
		$params = array(':nn' => $nnd1, ':id' => $imagef['id']);
		$sqlbbb = 'update ya_kithe set nn=:nn where id=:id';
		$stmt = $mydata2_db->prepare($sqlbbb);
		$stmt->execute($params);
		$ii++;
	}
	
	if ($_GET['id'] == ''){
		$params = array(':nn' => $_POST['nn'], ':nd' => $_POST['nd'], ':kitm' => $_POST['kitm'], ':kizt' => $_POST['kizt'], ':kizm' => $_POST['kizm'], ':kizm6' => $_POST['kizm6'], ':kigg' => $_POST['kigg'], ':kilm' => $_POST['kilm'], ':kisx' => $_POST['kisx'], ':kibb' => $_POST['kibb'], ':kiws' => $_POST['kiws'], ':zfbdate' => $_POST['zfbdate'], ':kitm1' => $_POST['kitm1'], ':kizt1' => $_POST['kizt1'], ':kizm1' => $_POST['kizm1'], ':kizm61' => $_POST['kizm61'], ':kigg1' => $_POST['kigg1'], ':kilm1' => $_POST['kilm1'], ':kisx1' => $_POST['kisx1'], ':kibb1' => $_POST['kibb1'], ':kiws1' => $_POST['kiws1'], ':zfbdate1' => $_POST['zfbdate1']);
		$sql = 'insert into ka_kithe(na,nn,nd,kitm,kizt,kizm,kizm6,kigg,kilm,kisx,kibb,kiws,zfbdate,kitm1,kizt1,kizm1,kizm61,kigg1,kilm1,kisx1,kibb1,kiws1,zfbdate1)values(0,:nn,:nd,:kitm,:kizt,:kizm,:kizm6,:kigg,:kilm,:kisx,:kibb,:kiws,:zfbdate,:kitm1,:kizt1,:kizm1,:kizm61,:kigg1,:kilm1,:kisx1,:kibb1,:kiws1,:zfbdate1)';
	}else{
		$params = array(':nn' => $_POST['nn'], ':nd' => $_POST['nd'], ':kitm' => $_POST['kitm'], ':kizt' => $_POST['kizt'], ':kizm' => $_POST['kizm'], ':kizm6' => $_POST['kizm6'], ':kigg' => $_POST['kigg'], ':kilm' => $_POST['kilm'], ':kisx' => $_POST['kisx'], ':kibb' => $_POST['kibb'], ':kiws' => $_POST['kiws'], ':zfbdate' => $_POST['zfbdate'], ':kitm1' => $_POST['kitm1'], ':kizt1' => $_POST['kizt1'], ':kizm1' => $_POST['kizm1'], ':kizm61' => $_POST['kizm61'], ':kigg1' => $_POST['kigg1'], ':kilm1' => $_POST['kilm1'], ':kisx1' => $_POST['kisx1'], ':kibb1' => $_POST['kibb1'], ':kiws1' => $_POST['kiws1'], ':zfbdate1' => $_POST['zfbdate1'], ':id' => $_GET['id']);
		$sql = 'update ka_kithe set nn=:nn,nd=:nd,kitm=:kitm,kizt=:kizt,kizm=:kizm,kizm6=:kizm6,kigg=:kigg,kilm=:kilm,kisx=:kisx,kibb=:kibb,kiws=:kiws,zfbdate=:zfbdate,kitm1=:kitm1,kizt1=:kizt1,kizm1=:kizm1,kizm61=:kizm61,kigg1=:kigg1,kilm1=:kilm1,kisx1=:kisx1,kibb1=:kibb1,kiws1=:kiws1,zfbdate1=:zfbdate1 where id=:id';
	}
	$stmt = $mydata2_db->prepare($sql);
	$stmt->execute($params);
	include_once '../../class/admin.php';
	$message = 'modify the info,nn:' . $_POST['nn'];
...
if ($_GET['t0'] == '是'){
	...
	$sql = 'update ka_kithe set best=1 where id=:id';
	$stmt = $mydata2_db->prepare($sql);
	$stmt->execute($params);
...
if ($_GET['t0'] == '否'){
	$params = array(':id' => $_GET['newsid']);
	$sql = 'update ka_kithe set best=0 where id=:id';
	$stmt = $mydata2_db->prepare($sql);
	$stmt->execute($params);
...
if ($_GET['t1'] == '是'){
	$params = array(':zfb' => $_GET['zfb'], ':id' => $_GET['id']);
	$sql = 'update ka_kithe set zfb=:zfb where id=:id';
	$stmt = $mydata2_db->prepare($sql);
	$stmt->execute($params);
	if ($_GET['zfb'] == 1){
		$params = array(':id' => $_GET['id']);
		$sql = 'update ka_kithe set kitm=1,kizt=1,kizm=1,kizm6=1,kigg=1,kilm=1,kisx=1,kibb=1,kiws=1,zfb=1 where id=:id';
		$stmt = $mydata2_db->prepare($sql);
		$stmt->execute($params);
		...
		$params = array(':id' => $_GET['id']);
		$sql = 'update ka_kithe set kitm=0,kizt=0,kizm=0,kizm6=0,kigg=0,kilm=0,kisx=0,kibb=0,kiws=0,zfb=0 where id=:id';
		$stmt = $mydata2_db->prepare($sql);
		$stmt->execute($params);
...
if ($_GET['svave'] == 'svave'){
	if (!empty($_POST['na']) && ($_POST['na'] != 0)){
		...
		$x6 = Get_sx_Color($vv);
		$params = array(':na' => $_POST['na'], ':x1' => $x1, ':x2' => $x2, ':x3' => $x3, ':x4' => $x4, ':x5' => $x5, ':x6' => $x6, ':sx' => $sx, ':id' => $_GET['id']);
		$sql = 'update ka_kithe set na=:na,x1=:x1,x2=:x2,x3=:x3,x4=:x4,x5=:x5,x6=:x6,sx=:sx where id=:id';
		$stmt = $mydata2_db->prepare($sql);
		$stmt->execute($params);
		$sql22 = 'update ka_bl set gold=0';
		$mydata2_db->query($sql22);
		$resultf = $mydata2_db->query('select count(1) as ycount from ya_kithe');
		$ycount = $resultf->fetchColumn();
		$resultf = $mydata2_db->query('select * from ya_kithe order by nn LIMIT 1');
		while ($imagef = $resultf->fetch())
		{
			$ykaid = $imagef['id'];
			$nn = $imagef['nn'] + $ycount;
			$params = array(':best' => $imagef['best'], ':nn' => $imagef['nn'], ':nd' => $imagef['nd'], ':kitm' => $imagef['kitm'], ':kizt' => $imagef['kizt'], ':kizm' => $imagef['kizm'], ':kizm6' => $imagef['kizm6'], ':kigg' => $imagef['kigg'], ':kilm' => $imagef['kilm'], ':kisx' => $imagef['kisx'], ':kibb' => $imagef['kibb'], ':kiws' => $imagef['kiws'], ':zfbdate' => $imagef['zfbdate'], ':kitm1' => $imagef['kitm1'], ':kizt1' => $imagef['kizt1'], ':kizm1' => $imagef['kizm1'], ':kizm61' => $imagef['kizm61'], ':kigg1' => $imagef['kigg1'], ':kilm1' => $imagef['kilm1'], ':kisx1' => $imagef['kisx1'], ':kibb1' => $imagef['kibb1'], ':kiws1' => $imagef['kiws1'], ':zfbdate1' => $imagef['zfbdate1']);
			$sqlrr = 'INSERT INTO  ka_kithe set best=:best,nn=:nn,nd=:nd,kitm=:kitm,kizt=:kizt,kizm=:kizm,kizm6=:kizm6,kigg=:kigg,kilm=:kilm,kisx=:kisx,kibb=:kibb,kiws=:kiws,zfbdate=:zfbdate,kitm1=:kitm1,kizt1=:kizt1,kizm1=:kizm1,kizm61=:kizm61,kigg1=:kigg1,kilm1=:kilm1,kisx1=:kisx1,kibb1=:kibb1,kiws1=:kiws1,zfbdate1=:zfbdate1,n1=0,n2=0,n3=0,n4=0,n5=0,n6=0,na=0,lx=0 ';
			$stmt = $mydata2_db->prepare($sqlrr);
			$stmt->execute($params);
			$params = array(':nn' => $nn, ':id' => $ykaid);
			$sqlzz = 'update ya_kithe set nn=:nn where id=:id';
			$stmt = $mydata2_db->prepare($sqlzz);
			$stmt->execute($params);
		}
	}
	if (!empty($_POST['n1']) && ($_POST['n1'] != 0))
	{
		$params = array(':n1' => $_POST['n1'], ':id' => $_GET['id']);
		$sql = 'update ka_kithe set n1=:n1 where id=:id';
		$stmt = $mydata2_db->prepare($sql);
		$stmt->execute($params);
	}
	if (!empty($_POST['n2']) && ($_POST['n2'] != 0))
	{
		$params = array(':n2' => $_POST['n2'], ':id' => $_GET['id']);
		$sql = 'update ka_kithe set n2=:n2 where id=:id';
		$stmt = $mydata2_db->prepare($sql);
		$stmt->execute($params);
	}
	if (!empty($_POST['n3']) && ($_POST['n3'] != 0))
	{
		$params = array(':n3' => $_POST['n3'], ':id' => $_GET['id']);
		$sql = 'update ka_kithe set n3=:n3 where id=:id';
		$stmt = $mydata2_db->prepare($sql);
		$stmt->execute($params);
	}
	if (!empty($_POST['n4']) && ($_POST['n4'] != 0))
	{
		$params = array(':n4' => $_POST['n4'], ':id' => $_GET['id']);
		$sql = 'update ka_kithe set n4=:n4 where id=:id';
		$stmt = $mydata2_db->prepare($sql);
		$stmt->execute($params);
	}
	if (!empty($_POST['n5']) && ($_POST['n5'] != 0))
	{
		$params = array(':n5' => $_POST['n5'], ':id' => $_GET['id']);
		$sql = 'update ka_kithe set n5=:n5 where id=:id';
		$stmt = $mydata2_db->prepare($sql);
		$stmt->execute($params);
	}
	if (!empty($_POST['n6']) && ($_POST['n6'] != 0))
	{
		$params = array(':n6' => $_POST['n6'], ':id' => $_GET['id']);
		$sql = 'update ka_kithe set n6=:n6 where id=:id';
		$stmt = $mydata2_db->prepare($sql);
		$stmt->execute($params);
	}
	if (!empty($_POST['na']) && ($_POST['na'] != 0))
...
$nana = 1;
$result = $mydata2_db->query('select * from ka_kithe where na=0 order by nn desc LIMIT 1');
$row = $result->fetch();
$id = $row['id'];
$nn = $row['nn'];
$nd = $row['nd'];
  ...
  <td>
	...
	<table width="100%" border="1" cellpadding="2" cellspacing="2" bordercolor="#ECE9D8"> 
	  ...
	  <tr bordercolor="#ECE9D8"> 
		...
		<td width="89%" height="30" colspan="3" bordercolor="#CCCCCC">
		<?php 
		...
		?>                 <table border="1" cellpadding="3" cellspacing="1" bordercolor="f1f1f1"> 
			<form action="index.php?action=kithe&amp;svave=svave&amp;id=<?=$id?>"  method="post" name="testFrm" id="testFrm">
                    ...
                    <tr>
                      <td height="25" align="center"><input onblur="return CountGold(this,'blur','SP');" onkeyup="return CountGold(this,'keyup');" name="n1" type="text" class="input1"  id="n1" value="<?=$row['n1']?>" size="8" /></td>
                      <? if ($row['n1']<>"0" and $row['n1']<>"") { ?>
                      <td height="25" align="center"><input name="n2" onblur="return CountGold(this,'blur','SP');"  onkeyup="return CountGold(this,'keyup');"  type="text" class="input1"  id="n2" value="<?=$row['n2']?>" size="8" /></td>
                      <? }?>
                      <? if ($row['n2']<>"0" and $row['n2']<>"") { ?>
                      <td height="25" align="center"><input onblur="return CountGold(this,'blur','SP');" onkeyup="return CountGold(this,'keyup');" name="n3" type="text" class="input1"  id="n3" value="<?=$row['n3']?>" size="8" /></td>
                      <? }?>
                      <? if ($row['n3']<>"0" and $row['n3']<>"") { ?>
                      <td height="25" align="center"><input onblur="return CountGold(this,'blur','SP');" onkeyup="return CountGold(this,'keyup');" name="n4" type="text" class="input1"  id="n4" value="<?=$row['n4']?>" size="8" /></td>
                      <? }?>
                      ...
                      <td height="25" align="center">
					   	<? if ($row['zfb']!=0){ echo "请先封盘"; }else{?>
					  	<input onblur="return CountGold(this,'blur','SP');" onkeyup="return CountGold(this,'keyup');" name="n5" type="text" class="input1"  id="n5" value="<?=$row['n5']?>" size="8" />
						<? }?>
					  </td>
                      <? }?>
                      <? if ($row['n5']<>"0" and $row['n5']<>"") { ?>
      ...
      <table width="100%"  border="1" cellpadding="2" cellspacing="2" bordercolor="#ECE9D8">
        <form action="index.php?action=kithe&amp;act=添加&amp;id=<?=$id?>" method="post" name="testFrm" id="testFrm" onsubmit="return SubChk()">
          ...
          <tr>
            ...
            <button type="button" onclick="javascript:location.href='index.php?action=kithe&amp;t1=是&amp;id=<?=$id?>&amp;zfb=<? if ($row['zfb']==0) {?>1<? }else{ ?>0<? }?>'"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="width:150;height:22" ;>
              <? if ($row['zfb']==0 ){ ?>
      ...
      <table width="100%" border="1" align="center" cellpadding="2" cellspacing="2" bordercolor="#ECE9D8">
        ...
        <?
		$resultf = $mydata1_db->query("select * from mydata2_db.ya_kithe order by nn");   
while($imagef = $resultf->fetch()){
	?>
	    <tr>
