...
if ($_GET['fen'] == 'fen'){
	$locked = $_GET['fid'];
	for ($tt = 1;$tt <= 49;$tt++){
		$params = array(':locked' => $locked, ':class3' => $tt);
		$stmt = $mydata2_db->prepare('update ka_bl set locked=:locked where class1=\'特码\' and class3=:class3 ');
		$stmt->execute($params);
	}
}
...
if ($_GET['save'] == 'save'){
	...
	$config_3 = ka_config(3);

	for ($tt = 1;$tt <= 49;$tt++){
		if($config_3>0){
			$num = $_POST['bl'] + $config_3;
			$num1 = $_POST['bl'] - $config_3;
		}
		if ($_POST['dx'] == '全部'){
			if ($_GET['ids'] == '特A'){
				...
				if($config_3>0){
					...
					$stmt = $mydata2_db->prepare('update ka_bl set rate=:rate,blrate=:blrate where class2=\'特B\' and  class3=:class3');
					$stmt->execute($params);
				}
			}else{
				$params = array(':rate' => $_POST['bl'], ':blrate' => $_POST['bl'], ':class2' => $ids, ':class3' => $tt);
				...
				if($config_3>0){
					...
					$stmt = $mydata2_db->prepare('update ka_bl set rate=:rate,blrate=:blrate where class2=\'特A\' and  class3=:class3');
					$stmt->execute($params);
				}
			}
		}
...
if ($_GET['act'] == '修改')
{
	...
	for (;$tt <= 66;$tt++)
	{
		...
		if ($_GET['ids'] == '特A')
		{
			...
			{
				if(ka_config(5)){
					$params = array(':rate' => $num5, ':blrate' => $num5, ':class3' => $_POST['class3_' . $tt]);
					$stmt = $mydata2_db->prepare('update ka_bl set rate=:rate,blrate=:blrate where class2=\'特B\' and  class3=:class3');
					$stmt->execute($params);
				}
			}
		...
		{
			if(ka_config(5)){
				$params = array(':rate' => $num6, ':blrate' => $num6, ':class3' => $_POST['class3_' . $tt]);
				$stmt = $mydata2_db->prepare('update ka_bl set rate=:rate,blrate=:blrate where class2=\'特A\' and  class3=:class3');
				$stmt->execute($params);
			}
		}
...
if ($_GET['savew'] == 'savew'){
	...
	for ($f = 0;$f < $ss4;$f++){
		if ($mv == 0)
		{
			...
			$stmt = $mydata2_db->prepare('update ka_bl set rate=rate-:rate where class1=\'特码\'  and   class3=:class3');
			$stmt->execute($params);
		}else{
			$params = array(':rate' => $money, ':class3' => $pc4[$f]);
			$stmt = $mydata2_db->prepare('update ka_bl set rate=rate+:rate where class1=\'特码\'  and   class3=:class3');
...
$result = $mydata2_db->prepare('Select rate,class3,locked from ka_bl where class2=:class2 Order By id  LIMIT 66');
...
?>
  ...
  <table   border="1" align="center" cellspacing="1" cellpadding="1" bordercolordark="#FFFFFF" bordercolor="f1f1f1" width="99%"> 
  			 <form name="form1" method="post" action="index.php?action=rake_tm&act=修改&ids=<?=$ids;?>">
             <tr > 
                <td height="28" colspan="20" align="center" nowrap="nowrap" bordercolor="cccccc" bgcolor="#0077cc"><table width="99%" border="0" cellspacing="0" cellpadding="0"> 
                  <tr> 
                    <td width="13%"><font color="#FFFFFF"><strong><?=$ids;?>赔率设置</strong></font></td> 
                    <td width="50%"><div align="left"> 
                        <button type="button" onClick="javascript:location.href='index.php?action=rake_tm&ids=<?=$ids;?>&fen=fen&fid=1';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><SPAN id=rtm1 STYLE='color:ff0000;'>全部封号</span></button>&nbsp;
						<button type="button" onClick="javascript:location.href='index.php?action=rake_tm&ids=<?=$ids;?>&fen=fen&fid=0';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><SPAN id=rtm2 STYLE='color:ff0000;'>全部开号</span></button> 
                    </div></td> 
                    <td width="37%"><div align="right"> 
                      <button type="button" onClick="javascript:location.href='index.php?action=rake_tm&ids=特A';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm1 STYLE='color:<?=$z1color;?>;'>特A</span></button>&nbsp;
					  <button type="button" onClick="javascript:location.href='index.php?action=rake_tm&ids=特B';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:<?=$z2color;?>;'>特B</span></button> 
                    </div></td> 
                  </tr> 
                </table></td> 
              </tr> 
