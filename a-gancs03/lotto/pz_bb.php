...
if ($_GET['zsave'] == 'zsave'){
	...
	$params = array(':bb' => $_POST['tm'], ':bb1' => $_POST['tm1']);
	$stmt = $mydata2_db->prepare('update adad set bb=:bb,bb1=:bb1  Where id=1');
	$stmt->execute($params);
}
...
if ($_GET['save'] == 'save'){
	echo "<script>alert('系统已关闭补仓功能!');window.location.href='index.php?action=pz_bb&ids=".$ids."&kithe=".$kithe."';</script>";
	exit();
}
?>
...
if (i!=(arrResult.length-2)){ 
var zmzm1="index.php?action=look&kithe=<?=$kithe;?>&class2=<?=$ids;?>&lx=0&id=6&class3="+arrTmp[12];
num4 = "<center onclick='get_member_bet($(this));' data-id='"+arrTmp[12]+"'><button type=button class=headtd4  onmouseover=this.className='headtd3';window.status='半波';return true;onMouseOut=this.className='headtd4';window.status='半波';return true;onClick=ops('"+zmzm1+"',400,700)>"+arrTmp[2]+"</button></center>";//字段num2的值
...
var zmzm2="index.php?action=look&lx=1&kithe=<?=$kithe;?>&class2=<?=$ids;?>&id=6&class3="+arrTmp[12];
num11 = "<center><button type=button class=headtd4  onmouseover=this.className='headtd3';window.status='半波';return true;onMouseOut=this.className='headtd4';window.status='半波';return true;onClick=ops('"+zmzm2+"',400,700)>"+arrTmp[9]+"</button></center>";//字段num2的值 
}else{ 
...
{ 
  ...
  { 
	  ...
	  makeRequest('index.php?action=server_bb&class1=半波&class2=<?=$ids;?>&kithe=<?=$kithe;?>') 
...
while ($image = $result->fetch()){
	echo "<OPTION value=index.php?action=pz_bb&ids=".$ids."&kithe=".$image['nn'];
	if ($kithe!="") {
		if ($kithe==$image['nn']) {
...
?>         </SELECT></td> 
	  <form name=form55 action="index.php?action=pz_bb&zsave=zsave&kithe=<?=$kithe;?>&ids=<?=$ids;?>" method=post> 
		<td width="85" align="right" nowrap>最大亏损：</td> 
		<td width="30"><span class="STYLE2"> 
		  <input name="tm" class="input1" id="tm" value='<?=$bb;?>' size="10" /> 
		</span></td> 
		<td width="50" align="right" nowrap>退水：</td> 
		<td width="63"><input name="tm1" class="input1" type="text" id="tm1" value="<?=$bb1;?>" size="5"></td> 
...
<form action="index.php?action=pz_bb&save=save&ids=<?=$ids;?>&kithe=<?=$kithe;?>"    method="post" name="form1" ><?php include 'd1.php';?></form> 
</div> 
...
<SCRIPT language=javascript> 
makeRequest('index.php?action=server_bb&class1=半波&class2=<?=$ids;?>&kithe=<?=$kithe;?>')</script>
    <script language="JavaScript">
        function get_member_bet(obj){
            ...
            if(amount > 0){
                ...
                window.open("member_bet_list.php?six_num="+num+"&qishu="+qishu+"&class1=半波",'newwindow', 'height=500, width=800, top=100, left=250, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
            }
        }
