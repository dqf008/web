...
if ($_GET['zsave'] == 'zsave'){
	...
	$params = array(':dx' => $_POST['tm'], ':dx1' => $_POST['tm1']);
	$stmt = $mydata2_db->prepare('update adad set dx=:dx,dx1=:dx1  Where id=1');
	$stmt->execute($params);
}
...
if ($_GET['save'] == 'save'){
	echo "<script>alert('系统已关闭补仓功能!');window.location.href='index.php?action=pz_dd&ids=".$ids."&kithe=".$kithe."';</script>";
	exit();
}
?>
...
if (i!=(arrResult.length-2)){ 
var zmzm1="index.php?action=look&kithe=<?=$kithe;?>&class1=<?=$c1;?>&class2=<?=$ids;?>&lx=0&id=10&class3="+arrTmp[12];
num4 = "<center onclick='get_member_bet($(this));' data-id='"+arrTmp[12]+"'><button type=button class=headtd4  onmouseover=this.className='headtd3';window.status='生肖';return true;onMouseOut=this.className='headtd4';window.status='生肖';return true;onClick=ops('"+zmzm1+"',400,700)>"+arrTmp[2]+"</button></center>";//字段num2的值
...
var zmzm2="index.php?action=look&lx=1&kithe=<?=$kithe;?>&class1=<?=$c1;?>&class2=<?=$ids;?>&id=10&class3="+arrTmp[12];
num11 = "<center ><button type=button class=headtd4  onmouseover=this.className='headtd3';window.status='生肖';return true;onMouseOut=this.className='headtd4';window.status='生肖';return true;onClick=ops('"+zmzm2+"',400,700)>"+arrTmp[9]+"</button></center>";//字段num2的值
}else{ 
...
{ 
  ...
  { 
	  case "MODIFYRATE":  	  //更新赔率 
		...
		makeRequest('index.php?action=server_dd&class1=<?=$c1;?>&class2=<?=$ids;?>&kithe=<?=$kithe;?>') 
...
<tr> 
  <td width="72%" height="25" align="center" bordercolor="cccccc" bgcolor="#FDF4CA"><table width="100%" border="0" cellspacing="1" cellpadding="1"> 
	<tr> 
	  <td width="160" align="left" nowrap>[<?=$c1;?>-<?=$ids;?>]统计[<?=$kithe;?>期]</td> 
	  <td width="63" align="right" nowrap>选择期数： </td> 
	  <td width="38" nowrap><SELECT class=zaselect_ste name=temppid onChange="var jmpURL=this.options[this.selectedIndex].value ;if(jmpURL!='') {window.location=jmpURL;} else {this.selectedIndex=0 ;}"><?php
$result = $mydata2_db->query('select * from ka_kithe order by nn desc');
while ($image = $result->fetch()){
	echo "<OPTION value=index.php?action=pz_dd&ids=".$ids."&kithe=".$image['nn'];
	if ($kithe!="") {
		 if($kithe==$image['nn']) {
	...
	echo ">第".$image['nn']."期</OPTION>";
	  ...
	  <form name=form55 action="index.php?action=pz_dd&zsave=zsave&kithe=<?=$kithe;?>&ids=<?=$ids;?>" method=post> 
		<td width="74" align="right" nowrap>最大亏损：</td> 
		<td width="62"><span class="STYLE2"> 
		  <input name="tm" class="input1" id="tm" value='<?=$dx;?>' size="10" /> 
		</span></td> 
		<td width="45" align="right" nowrap>退水：</td> 
		<td width="34"><input name="tm1" class="input1" type="text" id="tm1" value="<?=$dx1;?>" size="5"></td> 
		<td width="80"><button onClick="submit()"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="width:80;height:22" ><img src="images/icon_21x21_copy.gif" align="absmiddle" />确认修改</button></td> 
	  </form> 
	  <td width="519"><div align="right"> 
		<button onClick="javascript:location.href='index.php?action=pz_dd&kithe=<?=$kithe;?>&ids=特A';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm1 STYLE='color:<?=$z7color;?>;'>特码</span></button> 
		<button onClick="javascript:location.href='index.php?action=pz_dd&kithe=<?=$kithe;?>&ids=正A';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm1 STYLE='color:<?=$z9color;?>;'>正码</span></button> 
	   
		<button onClick="javascript:location.href='index.php?action=pz_dd&kithe=<?=$kithe;?>&ids=正1特';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm1 STYLE='color:<?=$z1color;?>;'>正1特</span></button> 
		<button onClick="javascript:location.href='index.php?action=pz_dd&kithe=<?=$kithe;?>&ids=正2特';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:<?=$z2color;?>;'>正2特</span></button> 
		  <br> 
		<button onClick="javascript:location.href='index.php?action=pz_dd&kithe=<?=$kithe;?>&ids=正3特';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:<?=$z3color;?>;'>正3特</span></button> 
		<button onClick="javascript:location.href='index.php?action=pz_dd&kithe=<?=$kithe;?>&ids=正4特';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:<?=$z4color;?>;'>正4特</span></button> 
		<button onClick="javascript:location.href='index.php?action=pz_dd&kithe=<?=$kithe;?>&ids=正5特';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:<?=$z5color;?>;'>正5特</span></button> 
		<button onClick="javascript:location.href='index.php?action=pz_dd&kithe=<?=$kithe;?>&ids=正6特';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:<?=$z6color;?>;'>正6特</span></button> 
	  </div></td> 
	</tr> 
  </table></td> 
</tr> 
...
<form action="index.php?action=pz_dd&save=save&ids=<?=$ids;?>&kithe=<?=$kithe;?>"    method="post" name="form1" ><?php include 'd1.php';?></form> 
</div> 
...
<SCRIPT language=javascript> 
makeRequest('index.php?action=server_dd&class1=<?=$c1;?>&class2=<?=$ids;?>&kithe=<?=$kithe;?>')</script>
