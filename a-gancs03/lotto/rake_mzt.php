...
if ($_GET['save'] == 'save'){
	...
	for ($tt = 1;$tt <= 49;$tt++){
		if ($_POST['dx'] == '全部'){
			...
			$params = array(':adddate' => $text, ':mrate' => $_POST['bl'], ':class2' => $ids, ':class3' => $tt);
			$stmt = $mydata2_db->prepare('update ka_bl set adddate=:adddate,mrate=:mrate where class2=:class2 and  class3=:class3');
			$stmt->execute($params);
		}
...
if ($_GET['act'] == '修改'){
	...
	for ($tt = 1;$tt <= 58;$tt++){
		...
		$stmt->execute($params);
		if (49 < $tt){
			$params = array(':mrate' => $num, ':class2' => $class22, ':class3' => $class3);
			$stmt = $mydata2_db->prepare('update ka_bl  set mrate=:mrate where class1=\'过关\' and  class2=:class2 and  class3=:class3');
			$stmt->execute($params);
		}
	}
	echo "<script>alert('修改成功!');window.location.href='index.php?action=rake_mzt&ids=".$ids."';</script>";
	exit();
}
...
if ($_GET['savew'] == 'savew'){
	...
	for ($f = 0;$f < $ss4;$f++){
		if ($mv == 0){
			...
			$stmt = $mydata2_db->prepare('update ka_bl set mrate=rate-:rate where class1=\'正特\'  and   class3=:class3');
			$stmt->execute($params);
		}else{
			$params = array(':rate' => $money, ':class3' => $pc4[$f]);
			$stmt = $mydata2_db->prepare('update ka_bl set mrate=rate+:rate where class1=\'正特\'  and   class3=:class3');
...
$result = $mydata2_db->prepare('Select mrate,class3,locked from ka_bl where class1=\'正特\'    and class2=:class2 Order By ID  LIMIT 58');
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
          ...
          <form name="form1" method="post" action="index.php?action=rake_mzt&act=修改&ids=<?=$ids?>">  <tr >
              <td height="28" colspan="10" align="center" nowrap="nowrap" bordercolor="cccccc" bgcolor="#FE773D"><table width="99%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td width="57%"><font color="#FFFFFF"> <strong>
                    <?=$ids?>
                    赔率设置</strong></font></td>
                  <td width="14%" align="right"><span class="STYLE6">选择项目</span></td>
...
?><tr>
	...
	<table   border="1" align="center" cellspacing="1" cellpadding="2" bordercolordark="#FFFFFF" bordercolor="f1f1f1">
          <form action="index.php?action=rake_mzt&savew=savew&ids=<?=$ids?>" name="form21" method="post" >
            <tr align="middle">
   ...
   <form action="index.php?action=rake_mzt&save=save&ids=<?=$ids?>" name="form21" method="post" ><tr>
