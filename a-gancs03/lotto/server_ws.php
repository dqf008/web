...
for ($i = 0;$i < $ii;$i++){
	$sum_sx[$i] = $z_suma - $z_ds - ($z_pz * (1 - ($ws1 / 100))) - (($sum_ma[$i] - $sum_pz1[$i]) * $sum_mbl[$i]);
	if ($class1 == '正特尾数'){
		$sum_sx[$i] = $sum_ma[$i] - $sum_ds[$i] - ($sum_pz1[$i] * (1 - ($ztws1 / 100))) - (($sum_ma[$i] - $sum_pz1[$i]) * $sum_mbl[$i]);
	}
}
$b = 0;
...
for ($i = 0;$i < $ii;$i++){
	if ((0 <= $sum_sx[$i] + $ztm_tm) || ($sum_mbl[$i] == 0)){
		...
		$ffxx = (-$sum_sx[$i] - $ztm_tm) / (($sum_mbl[$i] + ($ws1 / 100)) - 1);
		if ($class1 == '正特尾数'){
			$ffxx = (-$sum_sx[$i] - $ztm_tm) / (($sum_mbl[$i] + ($ztws1 / 100)) - 1);
		}
	}
	$bl = round($ffxx, 0);
	if (1 <= $ffxx){
