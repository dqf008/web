...
$z_zong = 0;
$z_dai = 0;
$params = array(':Kithe' => $kithe, ':dai' => $username);
$result = $mydata2_db->prepare('select distinct(username)   from   ka_tan where Kithe=:Kithe and dai=:dai order by username desc');
$result->execute($params);
$ii = 0;
while ($rs = $result->fetch()){
	$params = array(':Kithe' => $kithe, ':username' => $rs['username']);
	$result1 = $mydata2_db->prepare('Select sum(sum_m) as sum_m,count(*) as re,sum(sum_m*dagu_zc/10) as dagu_zc,sum(sum_m*guan_zc/10) as guan_zc,sum(sum_m*zong_zc/10) as zong_zc,sum(sum_m*dai_zc/10) as dai_zc,sum(sum_m-sum_m*user_ds/100) as sum_st from ka_tan   where Kithe=:Kithe and username=:username');
...
<tr > 
  ...
  <td align="center" nowrap="nowrap" bordercolor="cccccc"><?=$rs['username'].$xm;?></td> 
  <td align="center" nowrap="nowrap" bordercolor="cccccc"><?=$Rs5['re'];?></td> 
  <td align="center" nowrap="nowrap" bordercolor="cccccc">
  <button type="button" onClick="javascript:location.href='index.php?action=x5&kithe=<?=$kithe;?>&username=<?=$rs['username'];?>'"  class="headtd4" style="width:80;height:22" ><font color=ff6600><?=$Rs5['sum_m'];?></font></button></td> 
