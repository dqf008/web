...
$z_xx = 0;
$z_pz = 0;
$params = array(':Kithe' => $kithe, ':class1' => $class1);
...
while ($rs = $result->fetch()){
	...
	$RsB = $result4->fetch();
	$xxm = explode(',', $rs['class3']);
	$ssc = count($xxm);
	$params = array();
	$t_sql = '';
	$j = 0;
	for (;$j < $ssc;$j = $j + 1){
		$params[':class3_' . $j] = $xxm[$j];
		if ($t_sql == ''){
			$t_sql .= 'class3=:class3_' . $j;
		}else{
			$t_sql .= ' or class3=:class3_' . $j;
		}
	}
	$params[':class1'] = $rs['class1'];
	$params[':class2'] = $rs['class2'];
	$result5 = $mydata2_db->prepare('Select * from ka_bl   where   class1=:class1 and class2=:class2 and (' . $t_sql . ')');
	$result5->execute($params);
	$Rsbl = $result5->fetch();
	while ($image = $result5->fetch()){
		if ($image['rate'] < $Rsbl['rate']){
			$Rsbl = $image;
		}
	}
	
	if (($rs['class3'] == '1') || ($rs['class3'] == '3') || ($rs['class3'] == '5') || ($rs['class3'] == '7') || ($rs['class3'] == '9')){
		$sum_color[$ii] = 'ff0000';
	}else if (($rs['class3'] == '2') || ($rs['class3'] == '4') || ($rs['class3'] == '6') || ($rs['class3'] == '8') || ($rs['class3'] == '0')){
		$sum_color[$ii] = '0000FF';
...
for ($i = 0;$i < $ii;$i++){
	$sum_sx[$i] = $sum_ma[$i] - $sum_ds[$i] - ($sum_pz1[$i] * (1 - ($ls1 / 100))) - (($sum_ma[$i] - $sum_pz1[$i]) * $sum_mbl[$i]);
}
$b = 0;
...
for ($i = 0;$i < $ii;$i++){
	if ((0 <= $sum_sx[$i] + $ztm_tm) || ($sum_mbl[$i] == 0)){
		...
		$ffxx = (-$sum_sx[$i] - $ztm_tm) / (($sum_mbl[$i] + ($ls1 / 100)) - 1);
	}
	$bl = round($ffxx, 0);
	if (1 <= $ffxx){
		$fg = $fg + 1;
		if ($i == 0){
			$sum_pz[0] = '<button class=headtd4  onmouseover=this.className=\'headtd3\';window.status=\'正特\';return true;onMouseOut=this.className=\'headtd4\';window.status=\'正特\';return true;onclick=show_win(\'' . $sum_tm[0] . '\',\'' . $bl . '\',\'' . $sum_mbl[0] . '\',\'' . $class1 . '\',\'' . $sum_tm2[0] . '\')    ><font color=ff6600>走飞</font>  ' . $bl . '</button>';
		}else{
			$sum_pz[$i] = '<button class=headtd4  onmouseover=this.className=\'headtd3\';window.status=\'正特\';return true;onMouseOut=this.className=\'headtd4\';window.status=\'正特\';return true;onclick=show_win(\'' . $sum_tm[$i] . '\',\'' . $bl . '\',\'' . $sum_mbl[$i] . '\',\'' . $class1 . '\',\'' . $sum_tm2[$i] . '\')    ><font color=ff6600>走飞</font>  ' . $bl . '</button>';
		}
	}else{
		$sum_pz[$i] = '0';
		$sum_pz[$i] = '<button class=headtd4  onmouseover=this.className=\'headtd3\';window.status=\'正特\';return true;onMouseOut=this.className=\'headtd4\';window.status=\'正特\';return true;onclick=show_win(\'' . $sum_tm[$i] . '\',\'' . $bl . '\',\'' . $sum_mbl[$i] . '\',\'' . $class1 . '\',\'' . $sum_tm2[$i] . '\')    ><font color=ff6600>走飞</font>  ' . $bl . '</button>';
...
for ($i = 0;$i < $ii;$i++){
	$blbl .= $class2 . ' ' . $sum_tm[$i] . '@@@' . $sum_re[$i] . '注@@@' . $sum_m[$i] . '@@@' . $sum_ma[$i] . '@@@' . $sum_mb[$i] . '@@@' . round($sum_ds[$i], 2) . '@@@' . round($sum_xx[$i], 2) . '@@@' . round($sum_sx[$i], 2) . '@@@' . $sum_pz[$i] . '@@@' . $sum_pz1[$i] . '@@@' . $sum_mbl[$i] . '@@@' . $fg . '@@@' . $sum_tm[$i] . '@@@' . $sum_color[$i] . '@@@' . $sum_tm2[$i] . '###';
}
$blbl .= '0@@@<font color=ff6600>' . $z_re . '注</font>@@@<font color=ff6600>' . $z_sum . '</font>@@@<font color=ff6600>' . $z_suma . '</font>@@@<font color=ff6600>' . $z_sumb . '</font>@@@<font color=ff6600>' . $z_ds . '</font>@@@<font color=ff6600>' . $z_xx . '</font>@@@&nbsp;@@@&nbsp;@@@<font color=ff6600>' . $z_pz . '</font>@@@<b><font color=ff0000>' . $ztm_tm . '</font></b>@@@' . $fg . '@@@0###';
echo $blbl;
?>