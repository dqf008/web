...
$result = $mydata2_db->prepare('select class3,class1,class2   from   ka_bl where  class1=:class1  and class2=:class2 order by id');
$result->execute($params);
$ii = 0;
while ($rs = $result->fetch()){
	$params = array(':Kithe' => $kithe, ':class1' => $rs['class1'], ':class2' => $rs['class2'], ':class3' => $rs['class3']);
	$result1 = $mydata2_db->prepare('Select sum(sum_m) as sum_m,count(*) as re,sum(0-sum_m*guan_ds/100*dagu_zc/10) as sum_ds,sum(0-sum_m*rate*dagu_zc/10) as sum_m3 from ka_tan   where Kithe=:Kithe and lx=0 and  class1=:class1 and  class2=:class2  and class3=:class3');
	...
	$result4 = $mydata2_db->prepare('Select sum(sum_m*dagu_zc/10) as sum_moneyb from ka_tan   where  Kithe=:Kithe and  lx=0 and  class1=:class1 and class2=\'正1B\' and class3=:class3');
	...
	$Rsbl = $result5->fetch();
	if ($ii < 49){
		$params = array(':class3' => $rs['class3']);
		$result6 = $mydata2_db->prepare('Select * from m_color   where   id=:class3 order by id Desc');
	...
	if ($ii < 49){
		if ($Rsbl['rate'] != '')
		{
			$sum_bl[$ii] = '<a style="cursor:hand" onClick="UpdateRate(\'MODIFYRATE\',\'lm\',\'bl' . $ii . '\',\'class1=正特&ids=' . $rs['class2'] . '&sqq=sqq&lxlx=0&qtqt=0.5&class3=' . $rs['class3'] . '\');"><font color=0000ff>-</font></a><a style="cursor:hand" onClick="UpdateRate(\'MODIFYRATE\',\'lm\',\'bl' . $ii . '\',\'class1=正特&ids=' . $rs['class2'] . '&sqq=sqq&lxlx=0&qtqt=0.1&class3=' . $rs['class3'] . '\');"><font color=0000ff>↓</font></a><span id=bl' . $ii . '>' . $Rsbl['rate'] . '</span><a style="cursor:hand" onClick="UpdateRate(\'MODIFYRATE\',\'lm\',\'bl' . $ii . '\',\'class1=正特&ids=' . $rs['class2'] . '&sqq=sqq&lxlx=1&qtqt=0.1&class3=' . $rs['class3'] . '\');"><font color=ff0000>↑</font><a style="cursor:hand" onClick="UpdateRate(\'MODIFYRATE\',\'lm\',\'bl' . $ii . '\',\'class1=正特&ids=' . $rs['class2'] . '&sqq=sqq&lxlx=1&qtqt=0.5&class3=' . $rs['class3'] . '\');"><font color=ff0000>+</font></a>';
		}else{
			$sum_bl[$ii] = 0;
		}
	}else if ($Rsbl['rate'] != ''){
...
for ($i = 0;$i < 60;$i++){
	if ($i < 49){
		$sum_sx[$i] = (($z_suma - $z_pz) * (1 - ($zmt1 / 100))) - (($sum_ma[$i] - $sum_pz1[$i]) * $sum_mbl[$i]);
	...
	}else if (($sum_tm[$i] == '合大') || ($sum_tm[$i] == '合小')){
...
for ($i = 0;$i < 60;$i++){
	if ((0 <= $sum_sx[$i] + $ztm_tm) || ($sum_mbl[$i] == 0)){
		...
		$ffxx = ($z_pz - $sum_sx[$i] - $ztm_tm) / (($sum_mbl[$i] + ($zmt1 / 100)) - 1);
	}else if (($sum_tm[$i] == '单') || ($sum_tm[$i] == '双')){
...
$blbl .= '0@@@<font color=ff6600>' . $z_re . '</font>@@@<font color=ff6600>' . $z_sum . '</font>@@@<font color=ff6600>' . $z_suma . '</font>@@@<font color=ff6600>' . $z_sumb . '</font>@@@<font color=ff6600>' . $z_ds . '</font>@@@<font color=ff6600>' . $z_xx . '</font>@@@&nbsp;@@@&nbsp;@@@<font color=ff6600>' . $z_pz . '</font>@@@<b><font color=ff0000>' . $ztm_tm . '</font></b>@@@' . $fg . '###';
echo $blbl;
?>