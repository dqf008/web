...
<tr class="tbtitle"> 
  <td><strong><font color="#FFFFFF">[<?=$username;?>]股东注单查询[<?=$kithe;?>期]</font></strong></td> 
...
<tr > 
  ...
  <TD align="center" bordercolor="cccccc" bgcolor="#FDF4CA" >总代理</TD> 
...
$z_zong = 0;
$z_dai = 0;
$params = array(':Kithe' => $kithe, ':guan' => $username);
$result = $mydata2_db->prepare('select distinct(zong)   from   ka_tan where Kithe=:Kithe and guan=:guan order by zong desc');
$result->execute($params);
$ii = 0;
while ($rs = $result->fetch()){
$params = array(':Kithe' => $kithe, ':zong' => $rs['zong']);
$result1 = $mydata2_db->prepare('Select sum(sum_m) as sum_m,count(*) as re,sum(sum_m*dagu_zc/10) as dagu_zc,sum(sum_m*guan_zc/10) as guan_zc,sum(sum_m*zong_zc/10) as zong_zc,sum(sum_m*dai_zc/10) as dai_zc,sum(sum_m-sum_m*user_ds/100) as sum_st from ka_tan   where Kithe=:Kithe and zong=:zong');
...
echo $xm;?></td> 
  ...
  <button type="button" onClick="javascript:location.href='index.php?action=x3&kithe=<?=$kithe;?>&username=<?=$rs['zong'];?>'"  class="headtd4" style="width:80;height:22" ><font color=ff6600><?=$Rs5['sum_m'];?></font></button>
  </td> 
