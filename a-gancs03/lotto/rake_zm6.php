...
if ($_GET['act'] == '修改'){
	for ($tt = 1;$tt <= 78;$tt++){
		if (empty($_POST['Num_'.$tt])){
			echo "<script>alert('赔率不能为空:".$_POST['Num_'.$tt]." /".$tt ." !');history.back();</script>";
			exit();
		}
	}
	...
	for ($tt = 1;$tt <= 78;$tt++){
		...
		$stmt = $mydata2_db->prepare('update ka_bl  set adddate=:adddate,rate=:rate where class1=\'正1-6\' and class2=:class2 and  class3=:class3');
		$stmt->execute($params);
	}
...
$result = $mydata2_db->query('Select rate,class3,class2,locked from ka_bl where class1=\'正1-6\'   Order By class2,ID');
$ShowTable = array();
$y = 0;
