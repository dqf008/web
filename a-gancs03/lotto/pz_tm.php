...
if ($_GET['zsave'] == 'zsave'){
	...
	$params = array(':tm' => $_POST['tm'], ':tm1' => $_POST['ttm1']);
	$stmt = $mydata2_db->prepare('update adad set tm=:tm,tm1=:tm1 Where id=1');
	$stmt->execute($params);
}
  ...
  if (http_request.readyState == 4) {
	  ...
	  if (http_request.status == 0 || http_request.status == 200) {
		 ...
		 for(var i=0;i<arrResult.length-1;i++) {
			...
			num4 = "<center><button type=button class=headtd4  onmouseover=this.className='headtd3';window.status='特码';return true;onMouseOut=this.className='headtd4';window.status='特码';return true;onClick=ops('"+zmzm1+"',400,700)><font color=#000000><b>"+arrTmp[2]+"</b></font></button></center>";//字段num2的值
			...
			num11 = "<center><button type=button class=headtd4  onmouseover=this.className='headtd3';window.status='特码';return true;onMouseOut=this.className='headtd4';window.status='特码';return true;onClick=ops('"+zmzm2+"',400,700)>"+arrTmp[9]+"</button></center>";//字段num2的值
			}else{
...
?>
	  ...
	  <form name=form55 action="index.php?action=pz_tm&zsave=zsave&kithe=<?=$kithe;?>&ids=<?=$ids;?>" method=post>
		...
		<td width="95"><button type="button" onClick="submit()"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="width:80;height:22" ><img src="images/icon_21x21_copy.gif" align="absmiddle" />走飞计算</button></td>
	  </form>
...
<script language="JavaScript">
    function get_member_bet(obj){
       ...
       if(amount > 0){
           ...
           window.open("member_bet_list.php?six_num="+num+"&qishu="+qishu+"&class1=特码",'newwindow', 'height=500, width=800, top=100, left=250, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
       }
    }
</script>
