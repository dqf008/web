...
if ($_GET['save'] == 'save'){
	echo "<script>alert('系统已关闭补仓功能!');window.location.href='index.php?action=pz_ztws&ids=".$ids."&kithe=".$kithe."';</script>";
	exit();
}
?>
...
if (i!=(arrResult.length-2)){ 
var zmzm1="index.php?action=look&kithe=<?=$kithe;?>&class2=<?=$ids;?>&lx=0&id=72&class3="+arrTmp[12];
num4 = "<center><button type=button class=headtd4  onmouseover=this.className='headtd3';window.status='正特尾数';return true;onMouseOut=this.className='headtd4';window.status='正特尾数';return true;onClick=ops('"+zmzm1+"',400,700)>"+arrTmp[2]+"</button></center>";//字段num2的值 
...
var zmzm2="index.php?action=look&lx=1&kithe=<?=$kithe;?>&class2=<?=$ids;?>&id=72&class3="+arrTmp[12];
num11 = "<center><button type=button class=headtd4  onmouseover=this.className='headtd3';window.status='正特尾数';return true;onMouseOut=this.className='headtd4';window.status='正特尾数';return true;onClick=ops('"+zmzm2+"',400,700)>"+arrTmp[9]+"</button></center>";//字段num2的值 
}else{ 
...
{ 
  ...
  { 
	  case "MODIFYRATE":  	  //更新赔率 
	   ...
	   makeRequest('index.php?action=server_ws&class1=正特尾数&class2=<?=$ids;?>&kithe=<?=$kithe;?>') 
...
while ($image = $result->fetch()){
	echo "<OPTION value=index.php?action=pz_ztws&ids=".$ids."&kithe=".$image['nn'];
	if ($kithe!="") {
		if ($kithe==$image['nn']) {
	...
	echo ">第".$image['nn']."期</OPTION>";
	  ...
	  <form name=form55 action="index.php?action=pz_ztws&zsave=zsave&kithe=<?=$kithe;?>&ids=<?=$ids;?>" method=post> 
	  ...
	  <td width="697"><div align="right"> 
		...
		<button type="button" onClick="javascript:location.href='index.php?action=pz_sx1&ids=一肖&kithe=<?=$kithe;?>';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:000000;'>一肖</span></button> 
		<button type="button" onClick="javascript:location.href='index.php?action=pz_ztws&ids=正特尾数&kithe=<?=$kithe;?>';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:ff0000;'>正特尾数</span></button> 
...
<form action="index.php?action=pz_ztws&save=save&ids=<?=$ids;?>&kithe=<?=$kithe;?>"    method="post" name="form1" ><?php include 'd1.php';?></form> 
</div> 
...
<SCRIPT language=javascript> 
makeRequest('index.php?action=server_ws&class1=正特尾数&class2=<?=$ids;?>&kithe=<?=$kithe;?>')</script> 
