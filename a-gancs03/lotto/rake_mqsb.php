...
if ($_GET['act'] == '修改'){
	...
	for ($tt = 1;$tt <= 4;$tt++){
		...
		$stmt = $mydata2_db->prepare('update ka_bl  set adddate=:adddate,mrate=:mrate where class1=\'七色波\' and class2=:class2 and  class3=:class3');
		$stmt->execute($params);
	}
	
	echo "<script>alert('修改成功!');window.location.href='index.php?action=rake_mqsb&ids=". $ids ."';</script>";
	exit();
}

$result = $mydata2_db->query('Select mrate,class3,class2 from ka_bl where class1=\'七色波\'   Order By ID');
$ShowTable = array();
$y = 0;
...
<form name="form1" method="post" action="index.php?action=rake_mqsb&act=修改&ids=<?=$ids?>"><TBODY>
	  <TR align=center>
