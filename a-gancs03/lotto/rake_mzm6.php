...
if ($_GET['act'] == '修改'){
	...
	for (;$tt <= 42;$tt++)
	{
		...
		$stmt = $mydata2_db->prepare('update ka_bl  set adddate=:adddate,mrate=:mrate where class1=\'正1-6\' and class2=:class2 and  class3=:class3');
		$stmt->execute($params);
	}
	
	echo "<script>alert('修改成功!');window.location.href='index.php?action=rake_mzm6&ids=".$ids."';</script>";
	exit();
}
$result = $mydata2_db->query('Select mrate,class3,class2 from ka_bl where class1=\'正1-6\'   Order By ID');
$ShowTable = array();
$y = 0;
  ...
  { 
	  case "MODIFYRATE":  	  //更新赔率 
		  { 
			  ...
			  if (strResult!="") 
			  { 
				  makeRequest('index.php?action=server&class1=七色波&class2=<?=$ids;?>') 
...
<form name="form1" method="post" action="index.php?action=rake_mzm6&act=修改&ids=<?=$ids?>"><TBODY>
		  <TR align=center>
