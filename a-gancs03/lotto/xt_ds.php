...
if ($_GET['save'] == 'save'){
	...
	for ($I = 0;$I <= count($ygid);$I = $I + 1){
		$params = array(':yg' => $yg[$I], ':ygb' => $ygb[$I], ':ygc' => $ygc[$I], ':ygd' => $ygd[$I], ':xx' => $xx[$I], ':xxx' => $xxx[$I], ':ID' => $ygid[$I]);
		$sql = 'Update ka_guands Set yg=:yg,ygb=:ygb,ygc=:ygc,ygd=:ygd,xx=:xx,xxx=:xxx where ID=:ID';
		$stmt = $mydata2_db->prepare($sql);
		$stmt->execute($params);
		$params = array(':yg' => $yg[$I], ':ygb' => $ygb[$I], ':ygc' => $ygc[$I], ':ygd' => $ygd[$I], ':xx' => $xx[$I], ':xxx' => $xxx[$I], ':ds' => $ds[$I]);
		$sql = 'Update ka_quota Set yg=:yg,ygb=:ygb,ygc=:ygc,ygd=:ygd,xx=:xx,xxx=:xxx where ds=:ds';
		$stmt = $mydata2_db->prepare($sql);
		$stmt->execute($params);
	}
...
<?php 
$t = 0;
$result = $mydata2_db->query('select * from  ka_guands where lx=0 and style=\'六合彩\' order by id');
while ($rst = $result->fetch()){
	if ($rst['ds'] == '五行'){
		continue;
	}
	$t++;
...
<?php }?> 	  
  <tr> 
	<td height="20" colspan="7" align="center" bordercolor="cccccc"><button onclick="submit()"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="width:100;height:22" ><img src="images/icon_21x21_copy.gif" align="absmiddle" />确认修改</button> 
